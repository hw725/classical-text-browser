...
| `src/core/page_survey.py` | **쪽 훑어보기**(D-126). 방향은 **코드**가 잰다 — 잉크 투영의 자기상관(글줄 간격의 주기성)으로 «바로 섰다/누웠다»(속몽구 40판정 전부 정답; 비전 모델은 반반이라 투영이 모름일 때만 그 답을 쓴다). 비전 모델에는 «무슨 글인가»(활자·판본·손글씨·훈점·한글·백지, 목록)만 답받고, 코드가 «지금 회전과 다른 쪽»을 구간으로 묶어 회전을 제안하고(누운 쪽의 90/270은 화면이 미리보기로 확인) 종류를 **설치된 엔진**에 대응시킨다(활자 → NDLOCR, 판본 → 古典籍 Lite, 손글씨 → 古典籍 Full, 훈점 → みんなで翻刻, 한글 → LLM 비전; 없으면 다음 후보). 한 쪽에 종류가 둘 이상(한글+훈점)이면 엔진을 고르지 않고 «영역을 나눠 영역별로 OCR»하라고 따로 돌려준다. 저장·실행은 사람이 누른다 — 라우트는 llm_ocr의 `rotation/suggest`, 화면은 뷰어 툴바 「훑어보기」 |
