...
| `src/core/page_format.py` | **판식을 좌표에서 읽는다**(D-120). 열 사이가 유난히 벌어진 자리를 판심으로 보고, 행마다 `body`·`pansim`·`margin` 라벨을 붙인다. **판심이 쪽의 절반 이상에서 잡히고 좌우 열 수가 일정한 책에서만** 판정하고, 아니면 아무것도 빼지 않는다. 두주는 «본문 열의 0.25배보다 작고 윗변에서 시작하지도 않는» 행 — 0.6배로 자르면 내려쓴 별행 표제가 함께 빠진다. 발견기는 `body`만 센다. 浩齋에서 잰 «반엽 10행»이 KORMARC 300▼b의 「10行20字」와 같다 |
