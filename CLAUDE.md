...
| `src/core/rule_talk.py` | **사람이 말한 것을 규칙 변경으로 옮긴다**(D-121 두 번째 입구). 모델은 정해진 칸 이름과 연산만 쓰고, 아는 칸이 아니면 코드가 되돌린다. **옮기지 못한 말은 반드시 돌려준다** — 범위·조건은 지금 규칙으로 표현할 수 없고, 비슷한 칸으로 바꿔치기하면 사람은 말했다고 여기는데 시스템은 다른 일을 한다. 어휘는 전문에서 몇 번 나오는지 세어 함께 준다(0번이면 이 책의 말이 아니다). 저장하지 않는다 — rule_preview가 재고 사람이 승인한다 |
