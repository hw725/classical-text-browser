...
이 파일은 Codex 용 입구이고, **아래 네 절은 여기에 복제하지 않고 CLAUDE.md 를 가리킨다.**
두 벌을 두면 한쪽만 고쳐진다 — 실제로 그렇게 됐고(2026-09-22 교차검토), head-repo 도
같은 날 같은 이유로 쌍둥이 방식을 버렸다.

| CLAUDE.md 의 절 | 무엇이 있나 |
|---|---|
| **「파일 다루기 — 되풀이하지 말 것」** | **되돌릴 수 없는 손상을 막는 규칙들.** 원자적 JSON 저장 · 다권본 PDF 열기 · 쪽 이미지와 회전 · L2 bbox 배율 · 화면 이스케이프 · `127.0.0.1` · 검증 서버 격리 · `.bat` 는 ASCII · **배포 진입점의 `core.console.force_utf8_console()`**(D-130) · `.ps1` 은 UTF-8 BOM · LLM 뒤 저장은 다시 읽어 병합 · 주석 좌표는 코드포인트 |
| 「추출 모드 관련 모듈」 | 논문 스캔본 경로에만 쓰이는 모듈들 |
| 「OCR 품질 모듈」 | 프롬프트 조립 · 교정 사다리 · 경계 규칙 · 판독 계획 · 구조 판정 |
| 「코어 스키마 모듈」 | D-128 커넥톰 규약 — 데이터 판 각인 · id 장부 · 승격 무게 · 범위 · 부호 |

**고치기 전에 `docs/maintenance.md` 를 본다** — 되돌릴 수 없는 것, 자동 테스트가 못 잡는
자리, 릴리스 절차의 정본이다.
...
└── routers/
    ...
    ├── documents.py     ← 문헌 CRUD/페이지/교정/서지/파서 + 텍스트레이어 진단·가져오기·입히기 + 권 추가·회전 + 찍은 자리·규칙 제안 (45 라우트)
...
- 프론트(static/)가 약 4.2만 줄 — index.html 약 4.9천 줄 단일 파일, workspace.css 약 7.9천 줄,
  JS 32개. 테스트 83파일 — 그중 화면 JS를 node로 돌리는 것 2(`tests/test_annotation_editor_js.py`,
