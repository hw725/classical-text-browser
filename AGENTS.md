...
- 코드 총 83,846줄 중 프론트(static/)가 38,827줄 — index.html 4,762줄 단일 파일,
  workspace.css 7,460줄, JS 에디터 20여 개. 테스트 35파일은 전부 백엔드, 프론트 테스트 0, CI 없음.
  (2026-07-26 실측)
...
| 2 | `src/app/static/` 프론트 모놀리스 | 테스트 0·CI 없음, index.html 4,762줄 수정 회귀 감지 불가. D-055의 작업 모드는 jsdom 일회성 하네스로 22건 실측했으나 **그 하네스는 저장소에 없다** — 정식 프론트 스모크 테스트는 여전히 미결(D-053 착수 조건) |
