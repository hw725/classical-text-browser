...
TOC_MARKERS = ("目錄", "目次", "總目", "目録", "卷之", "第一卷", "卷第")
...
def detect_toc_pages(
    ...
    """
    candidates = []
    for page in sorted(pages)[:search_first]:
        if toc_page_score(pages[page], max_title_chars) >= threshold:
            candidates.append(page)
    if not candidates:
        return []
    # 첫 후보에서 이어지는 연속 구간만 (중간에 끊기면 그 뒤는 본문일 확률이 높다)
    run = [candidates[0]]
...
def extract_toc_entries_rule(pages: dict[int, list[str]], toc_pages: list[int]) -> list[TocEntry]:
    ...
    for page in toc_pages:
        for i, raw in enumerate(pages.get(page, [])):
            t = raw.strip()
            if not t or any(t == m or t.endswith(m) for m in ("目錄", "目次", "總目", "目録")):
                continue
            ...
            entries.append(
                TocEntry(title=t, level=level, page_hint=hint, source_page=page, source_line=i)
            )
    return entries
...
async def extract_toc_entries_llm(
    ...
    """LLM에 목차 쪽 텍스트를 넘겨 항목을 구조화한다. 실패하면 규칙 추출로 물러난다.

    출력: (항목, {"method": "llm"|"rule", "provider", "model", "is_toc", "error"})
    """
    body = "\n".join(
        f"[{page}쪽]\n" + "\n".join(t for t in pages.get(page, []) if t.strip())
        for page in toc_pages
    )
    prompt = (
        "다음은 고서 앞부분 몇 쪽의 OCR 텍스트입니다. 목차인지 판단하고 항목을 JSON으로 뽑으세요.\n"
        '형식: {"is_toc": true, "entries": '
        '[{"title": "...", "level": 1|2, "page_hint": "..."|null}, ...]}\n\n' + body
    )
    meta: dict = {"method": "rule", "provider": None, "model": None, "is_toc": None, "error": None}
    try:
        kwargs = {
        ...
        if entries or meta["is_toc"] is False:
            return entries, meta
    except Exception as e:  # noqa: BLE001 — LLM 실패는 규칙 추출로
        meta["error"] = f"{type(e).__name__}: {e}"
    return extract_toc_entries_rule(pages, toc_pages), meta
