...
def resolve_citation_context(
    ...
    full_page_text = page_data.get("text", "")

    # 블록별 텍스트 추출: 서버의 패턴을 따라 전체 텍스트를 블록 단위로 분할해야 하지만,
    # L4 텍스트가 블록별이 아닌 페이지 전체로 저장되어 있으므로,
    # 서버의 corrected_text API가 blocks[]를 반환하는 것처럼 블록 텍스트를 가져온다.
    # 여기서는 간이하게 전체 텍스트에서 범위를 추출한다.
    # 주의: block_id에 따라 블록별 텍스트를 분리하는 것은 서버 API를 통해 처리.
    # resolve 함수에서는 전체 텍스트를 사용한다.
    full_block_text = full_page_text
    original_text = ""
    if full_page_text and start <= end < len(full_page_text):
        original_text = full_page_text[start:end + 1]
    elif full_page_text and start < len(full_page_text):
        original_text = full_page_text[start:]
    ...
    try:
        ...
        if marks_list and full_page_text:
            # 전체 텍스트에 표점을 적용한 뒤 마크 범위를 추출.
            # render_punctuated_text는 전체 블록 텍스트 기준이므로,
...
def format_citation(
    ...
    author = bib.get("creator", "")
    book_title = bib.get("title", "")
    ...
    if punctuated_text:
        if citation:
            citation += f" : {punctuated_text}"
        else:
            citation = punctuated_text
...
def export_citations(
    ...
    citations = []
    for ctx in contexts:
        citation = format_citation(ctx, include_translation=include_translation)
        if citation:
