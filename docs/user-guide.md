...
개요를 세웁니다. 기사를 누르면 원본 이미지의 그 자리에 **시작 행이 점선**으로 표시되고,
...
1. 사이드바 **「내용」 ▸ 「자동 트리」**. 목차·해제·들여쓰기·위치를 합쳐 개요를 세웁니다.
   ...
   - 해제(解題)가 있으면 편성 인덱스 「규칙」의 **해제·참고 텍스트**에 통째로 붙여 넣으세요(2만 자여도 됩니다).
...
- 편성 인덱스의 「경계 제안」은 날짜·표제 어휘·형식으로 글이 바뀌는 자리를 훑어 보여 줄 뿐이고,
