...
1. 사이드바 **「내용」 ▸ 「자동 트리」**. 이 책의 확정본에서 되풀이되는 표지를 스스로 찾아
   ...
   - **구조를 통째로 묻기** — 표지가 제각각이라 ②의 규칙이 못 잡는 책(문집·잡록)에 씁니다. 권의 확정본 전문에
     ...
     **«LLM 구조»** 표지를 달고 체크된 채 서고, 규칙 후보와 같은 자리면 그 후보에 표지만 보탭니다. 보내기 전에
     ...
     순서로 되는 첫 것). 해제(解題)는 ② 「참고·억제」의 해제 칸에 통째로 붙여 넣으면(2만 자여도 됩니다)
     「더 묻기…」가 읽습니다 — 창 맨 위에 «해제 N자» 또는 «해제 없음»이 뜨고 「해제 넣기」(이미 있으면 「해제 고치기」)가 그 칸으로 데려갑니다.
   - **② 규칙 고르기** — 전문에서 센 규약 후보가 횟수·보기와 함께 체크박스로 뜹니다(판심·엽수처럼 쪽마다
     ...
     어휘는 `^有`처럼 `^`를 앞에, 기호 한 글자는 그대로), 억제 목록·표제 최대 글자수는 「참고·억제」에 있습니다.
   ...
   - **③ 후보 고르기** — 체크 = 경계가 될 것. **행을 누르면 고르고, Shift로 범위, Ctrl로 하나씩** 골라
     위 줄에서 **역할·깊이를 한꺼번에** 바꿉니다(「바꾸기」). 「체크」·「해제」·「억제」도 고른 행 전부에 듭니다.
