...
1. [ZIP 내려받기](https://github.com/hw725/classical-text-browser/archive/refs/heads/main.zip) → 원하는 곳에 압축을 풉니다.
2. 푼 폴더의 **`install.bat`을 두 번** 누릅니다. 검은 창이 뜨고 필요한 것(Python·Git·uv)을
   알아서 받습니다. 중간에 **글자 인식 엔진을 함께 깔지** 묻습니다 — 모르면 그냥 Enter(본체만).
   한글 논문과 글자가 든 PDF는 본체만으로 다 됩니다.
3. 「설치가 끝났습니다」가 보이면 창을 닫습니다.

### ② 켠다

**`start_server.bat`을 두 번** 누릅니다. 브라우저가 열리고 **「처음 설정」** 안내가 뜹니다.
...
- **스캔본**: 오른쪽 세로 줄 **레이아웃** 탭 ▸ 「자동 분석」 ▸ **교정** 탭 ▸ 「OCR 실행」.
  결과가 교정 창에 뜹니다. 틀린 글자는 그 자리에서 고치고 **저장**.

### ⑤ 개요를 세운다

왼쪽 사이드바 **「내용」** ▸ **「자동 트리」**. 목차·해제·들여쓰기를 읽어 **묶음 > 기사 > 조각**
개요를 세웁니다. 기사를 누르면 표점·현토·번역·주석 편집기가 그 기사로 맞춰집니다.

![편성 — 자동 트리와 경계](screenshots/08_composition.jpg)
...
컴퓨터에 이미 설치된 것은 건너뜁니다. 본체만이면 1~2분, 엔진까지 고르면 5분쯤 걸립니다.
...
> **편성은 문헌의 것입니다(v1.3, D-097).** 문헌과 권만 고르면 편성 탭이 열리고 제안·적용·경계
> 넣기가 다 됩니다 — 해석 저장소를 고르지 않아도 됩니다. 한 문헌에 해석 저장소가 여럿이어도
> 편성은 하나를 함께 씁니다. 해석 저장소가 필요해지는 것은 **표점 탭부터**입니다(문헌을 고르면
...
> 체크한 것만 단위가 됩니다. 談草·筆談처럼 이 문헌만의 표제 어휘는 「규칙 → 고급 규칙」에 적으면
> 문헌 설정으로 저장됩니다. 무엇을 적을지 모르겠으면 **«해제·본문에서 뽑기»**를 누르세요 — 해제와
> 본문의 짧은 행을 보고 후보를 찾아 줍니다. 누른 후보만 칸에 붙고, 저장은 「저장하고 다시 제안」으로
> 합니다. 卷·集·附錄 같은 묶음은 적지 않아도 됩니다 — 「卷之一」처럼 卷 이름으로 끝나는 행은 알아서 잡습니다. 권 전체 확정본이 없으면 OCR 패널의 「권 전체 OCR」로 먼저 돌리세요.
> 개행 없이 「○七日…○八日…」처럼 열 중간에서 날이 바뀌는 일기(澹齋日錄류)도 같은 단추로 잡습니다 —
> 후보에 「n자째」가 붙고, 만든 단위는 행 중간에서 잘립니다.
>
> 해제(解題)나 서지 설명이 있으면 「규칙」의 **해제·참고 텍스트**에 붙여 넣으세요 — 한국고전종합DB 해제처럼
> 2만 자가 넘어도 **통째로** 넣으면 됩니다. 저장은 그대로 하고, LLM에 넘길 때만 권별 내용이 있는 데를 골라
> 간추립니다(해제는 생애 이야기가 앞에 길고 권별 서술이 뒤에 있어서, 앞에서 자르면 필요한 데가 사라집니다).
> 목차 감지(LLM)와 표제 어휘 뽑기가 이것을 참고합니다. 사이드바의 섹션 머리(문헌 목록·서지정보·해석 저장소·내용)는 누르면 접힙니다.
>
> 편성 탭을 열면 **지금 저장된 경계**가 먼저 보입니다 — 사이드바 「내용」과 같은 것이고, 어느 쪽에서
> 눌러도 같은 기사가 골라집니다. 「경계 제안」은 새로 훑어 보는 것일 뿐이고, 적용하지 않으면
> 아무것도 바뀌지 않습니다.
>
...
> 표시됩니다 — ▲▼로 옮겨 주세요.
>
> 쪽·행·글자를 세지 않아도 됩니다. **「＋ 경계 넣기」를 펼치면 곧바로 «찍기»가 켜집니다** — 원본 이미지나
> 교정 텍스트에서 자리를 누르면 붉은 점선과 그 자리의 글자 여덟 자가 보이고 「넣기」가 켜집니다. 맞으면
> 역할(묶음/기사/조각)만 고르고 「넣기」를 누르세요. 글자 번호는 행 길이로 어림한 것이라 협주가 섞인
> 행에서는 한두 자 어긋날 수 있어, 넣기 전에 확인하게 되어 있습니다. 찍을 수 없는 쪽(OCR 행 좌표가
> 없는 쪽)은 «숫자로 적기»를 펼쳐 쪽·행·글자를 직접 적습니다.
>
...
>
> 편성 탭 맨 아래 **「단위 손보기」**는 «경계를 적용한 뒤, 기사 하나를 골라 문단으로 나누는» 자리입니다.
> 권 전체 단위가 목록에 뜨니 고르기만 하면 됩니다. 카드를 누르면 가운데에 창이 뜨고, 나눌 자리에
> `===`를 넣으면 **그 기사 안이** 문단(조각)으로 나뉩니다 — 새 기사는 생기지 않습니다.
> 별도의 기사로 나누려면 사이드바 「내용」의 «＋ 경계 넣기»를 쓰세요. 카드에 「⚠ 아래 단위 N개를 품음」이 붙어 있으면
> 그 단위의 깊이가 얕아 뒤따르는 기사들을 삼키고 있다는 뜻입니다 — 사이드바 「내용」에서 ⇥로 깊이를
> 한 단 내리거나 역할을 «묶음»으로 바꾸세요.
>
> **글 단위가 잘 안 보이는 문헌이면** 쪽마다 단위 하나로 시작하세요(같은 자리의 「자동 편성」 —
...
**위에서 아래로 작업 순서**입니다. 지금 있는 탭을 다시 누르면 오른쪽 패널이 접혀 원본
이미지가 화면 전체를 씁니다(왼쪽 아이콘 줄과 같은 규칙). 다른 탭을 누르면 펴집니다.
...
| | 교감 모드 | 추출 모드 |
|---|---|---|
| 모드 탭(오른쪽 세로 줄) | 10개 전부 | **열람·레이아웃·교정** |
...
**설정(⚙) ▸ 앱 업데이트 ▸ 「새 판 확인」**. 새 판이 있으면 무엇이 바뀌었는지 보여 주고
**「받기」**로 받습니다. GitHub에 들어갈 일이 없습니다.

- 서고는 앱 폴더 밖에 있으므로 영향이 없습니다. 키·이체자 사전·설정도 서고에 있어 그대로입니다.
- 받은 뒤에는 **서버를 껐다 켜고 브라우저를 하드 새로고침**(Ctrl+Shift+R)합니다.
- 고친 파일이 앱 폴더에 남아 있으면 받지 않고 알려 줍니다(덮어쓰면 그 수정이 사라지므로).
- 서고 형식이 바뀌는 판은 붉게 표시됩니다 — 그런 판은 받기 전에 [9.5 백업](#95-서고-백업)을 한 번 하세요.
- ZIP으로 받아 설치한 경우(Git 사본이 아닌 경우)에는 새 판 ZIP을 받아 폴더를 덮어씁니다.
