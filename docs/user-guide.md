...
> **v1.2.0 이전에 만든 텍스트 레이어 PDF는 모두 다시 만들어야 합니다.**
> 텍스트가 495×694pt 쪽의 왼쪽 아래 구석에 2pt 크기로 박혀 있어
> 드래그도 검색도 되지 않습니다(D-068). 문헌을 열고
> 「텍스트 레이어 PDF 다시 만들기」를 한 번 눌러 주세요.
> 앱을 켜 둔 채로 갱신했다면 **서버를 껐다 켜야** 고침이 적용됩니다.
>
> 이제는 만든 PDF를 앱이 **다시 열어 재 봅니다.** 글자 크기, 텍스트가 덮은 넓이,
