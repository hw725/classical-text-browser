...
전체, «37-60»), **모델**(글의 종류를 볼 비전 모델. «자동»은 되는 프로바이더 순서), 그리고 범위를 고칠 때마다
«N쪽 — 비전 모델 N번 + PaddleOCR M번»이 바로 밑에 뜹니다. 「훑어보기」를 누르면 시작하고, 끝나면 «회전이 다른
...
레이아웃 탭에서 영역을 나눈 뒤 영역마다 엔진을 골라 「선택 블록 OCR」로 읽으세요. 보내기 전에 «N쪽·호출 K번»을
묻고, 범위를 적으면 그 쪽만 봅니다.
