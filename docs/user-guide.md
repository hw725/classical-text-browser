...
- **「LLM 교정(선별)」** — 신뢰도가 낮은 블록, 협주·방주, 한글을 못 읽는 엔진의 결과만
  ...
  2단계의 기준은 «두 단계의 답이 같은가»입니다 — 같으면 자동 수용, 다르면 두 답을
  나란히 보여 주고 사람이 고릅니다. 어느 블록을 왜 골랐는지 초안 목록에 이유가 붙습니다.
...
- 초안 목록의 각 블록에는 단계(1단계·2단계), **엔진 결과 → 교정본**(두 단계가 다르면
  둘 다), 일치율, `[?]`(불확실 글자) 수가 보입니다. **「적용」**을 누른 블록만 L4(교정
  텍스트)에 들어가고, 들어간 블록은 「적용됨」으로 표시됩니다. 초안은 쪽을 옮겼다 돌아와도
  다시 보입니다.
- **「검토할 쪽」** — 이 권에서 사람이 볼 초안이 남은 쪽 목록입니다. 쪽을 누르면 그 쪽으로
  갑니다. 일괄 OCR을 돌린 뒤 어디를 열어야 하는지 여기서 봅니다.
- 권 단위 일괄 OCR(추출 패널)에서는 **「LLM 교정」**을 「선별만」이나 「전체」로 켤 수
  있습니다. 기본은 끔입니다 — 켜면 쪽마다 LLM 호출이 나갑니다. 켜면 쪽마다 위 사다리가
  돌고, 자동 수용된 블록은 L4에 들어가며, 남은 블록은 완료 메시지와 「검토할 쪽」에
  셉니다.
