...
1. 사이드바 **「내용」 ▸ 「자동 트리」**. 이 책의 확정본에서 되풀이되는 표지를 스스로 찾아
   ...
   - 답이 틀렸을 때만 **「자세히 · 고치기」**를 펼치세요. 신호마다 횟수·보기가 체크박스로 뜹니다(판심·엽수처럼
     쪽마다 같은 자리에 오는 것은 뺀 채로). **켜진 것만 펼쳐져 있고**, 세기는 했지만 점수가 낮아 켜지 않은 것은
     「센 것 N줄 더」로 접혀 있습니다 — 접힌 줄은 규칙이 아니라 근거이고, 체크하면 그대로 규칙이 됩니다. 체크를 바꾸고 **「후보 보기」**로 후보를 다시 보거나,
   ...
   - 해제(解題)가 있으면 신호 목록 아래 「참고·억제」의 **해제·참고 텍스트**에 통째로 붙여 넣으세요
     (2만 자여도 됩니다). 해제는 「LLM에 묻기」만 읽고, 셋 다 읽습니다. 창을 열면 맨 위에 «해제 N자를 함께
     보냅니다» 또는 «해제 없음»이 뜨고, **「해제 넣기」**를 누르면 창이 닫히며 그 칸으로 데려갑니다
     (붙여 넣는 곳은 거기 한 군데뿐입니다).
