...
> **지금은 화면에 없습니다(2026-09-17).** 아래 단추 셋(「LLM 교정(선별)」·「정밀 판독(선택)」·
> 추출 패널의 「LLM 교정」)은 화면에서 내렸습니다. 각 단계는 돌지만 단계 사이의 이음 —
> 1단계에서 떨어진 블록을 2단계로 올리기, 두 단계의 답 견주기, 일괄 뒤 사람이 볼 목록 — 이
> 아직 없고, 정밀 판독이 앞선 초안을 덮어쓰는 결함이 있습니다. 서버 쪽 기능은 남아 있고,
> 이음을 구현하면 되살립니다. 아래 설명은 그때의 동작입니다.
