...
| 하단 | 톱니바퀴 | **설정** — 처음 설정(마법사) · 서고 경로·최근 서고 · LLM 연결(키·ChatGPT 로그인) · 앱 업데이트 · 백업 · 휴지통 |
...
- 받은 뒤에는 **서버를 껐다 켭니다**(`start_server.bat`을 다시).
...
- 다른 주소·포트로 띄웠다면 같은 카드의 주소 칸에 `http://127.0.0.1:11434`처럼 적고 저장하세요.
- 터미널에서 `ollama list`가 되는지 봅니다 — 안 되면 Ollama 자체가 안 떠 있는 것입니다.
