...
2. 푼 폴더의 **`install.bat`을 두 번** 누릅니다. 검은 창이 뜨고 Git·uv를 알아서 받고, 앱 전용
   Python 3.12는 uv가 받아 옵니다. 중간에 **글자 인식 엔진을 함께 깔지** 묻습니다 — 모르면 그냥 Enter(본체만).
...
- **스캔본**: 오른쪽 세로 줄 **레이아웃** 탭 ▸ 「자동감지」 ▸ 「모든 블록 OCR」 ▸ **교정** 탭에서 손봅니다.
...
이어서 **Ollama가 깔려 떠 있으면 기본 비전 모델 `gemma4:e4b`(약 5GB)를 받습니다.** Ollama가 없으면
건너뜁니다. 앱을 켠 뒤에도 비전 모델이 하나도 없으면 같은 모델을 저절로 받기 시작합니다.
...
1. 사이드바 **「내용」 ▸ 「자동 트리」**. 목차·해제·들여쓰기·위치를 합쳐 개요를 세웁니다.
   - 목차가 있는 책은 목차 항목만 고릅니다. 「卷之一」처럼 卷 이름으로 끝나는 행은 묶음이 됩니다.
   - 해제(解題)가 있으면 편성 탭 「규칙」의 **해제·참고 텍스트**에 통째로 붙여 넣으세요(2만 자여도 됩니다).
   - 談草·筆談처럼 이 문헌만의 표제 어휘는 「규칙 ▸ 고급 규칙」에 적습니다. 모르겠으면
     **「해제·본문에서 뽑기」**가 후보를 찾아 줍니다.
...
3. 새 경계는 **「＋ 경계 넣기」**. 펼치면 «찍기»가 켜져 원본 이미지나 교정 텍스트에서 자리를
   ...
   역할을 고른 뒤 「넣기」.
4. 기사 하나를 문단으로 나누려면 편성 탭 맨 아래 **「단위 손보기」**에서 기사를 골라 나눌 자리에
   `===`를 넣습니다. 새 기사는 생기지 않습니다 — 기사를 나누는 것은 「경계 넣기」입니다.

- 편성 탭의 「경계 제안」은 날짜·표제 어휘·형식으로 글이 바뀌는 자리를 훑어 보여 줄 뿐이고,
  「선택 적용」하기 전까지 아무것도 바뀌지 않습니다. 적용은 **체크 상태가 곧 트리**입니다.
- 글 단위가 잘 안 보이는 문헌은 「자동 편성」(쪽마다 단위 하나)으로 시작해 구조가 보이는
  곳부터 쪼개면 됩니다. 단위를 나중에 바꿔도 표점·번역은 제자리에 남습니다.
...
|------|-----------|----------|-------------|------|
| 1 | Ollama (로컬 모델) | Ollama 설치 → 카드의 「모델 받기」(없으면 저절로 받습니다) | O | **무료** (이 PC에서 돕니다) |
| 1 | Ollama (`:cloud` 모델) | 위에 더해 카드의 「로그인」 | O | **구독 한도 소모** (금액은 0) |
...
| **모델 없음** | Ollama는 떠 있는데 비전 모델이 없습니다 | 기본 모델을 저절로 받기 시작합니다. 다른 모델은 「모델 받기」 |
...
- 다른 자리에 두려면 설정 탭의 **「새 서고」** ▸ 폴더 선택 창에서 빈 폴더를 고릅니다.
...
오프라인 OCR은 기본 설치에 들어 있는 PaddleOCR을 쓰거나, 고서라면 NDL古典籍OCR-Lite, 근현대 일본어 자료라면 NDLOCR-Lite를 **설정 ▸ 처음 설정 ▸ 글자 인식**의 「설치」로 더하세요. GPU가 있다면 NDL古典籍OCR Full([7-A.6-2](#7-a6-2-gpu로-돌리기--별도-환경-venv-gpu-선택))로 최고 품질을 얻을 수 있습니다. LLM을 오프라인에서 쓰려면 Ollama 로컬 모델을 설치하세요.
