...
- 왼쪽 아이콘 줄에서 **해석 저장소 전용 패널 5개가 숨습니다** — 검증 결과·의존 추적·
  엔티티·비고·인용 양식. 서고 브라우저·Git 이력·설정은 그대로 남습니다.
...
3. **추출** 탭으로 돌아옵니다. 버튼 위에 이렇게 뜹니다:
   ...
   > 실행하면 1쪽이 돕니다 — 1쪽은 레이아웃을 고쳐 다시 (12쪽)
...
> «이미 처리한 쪽도 다시»가 켜져 있어 1쪽이 다시 돕니다 (지정한 범위 12쪽).
...
> 지정한 1쪽은 이미 처리됐습니다. 다시 돌리려면 «이미 처리한 쪽도 다시»를 켜세요.
...
LLM Vision이, 위치 찾는 일은 PaddleOCR이 맡는 분업입니다. 설치돼 있으면
자동으로 쓰이고, 없으면 조용히 예전 방식으로 돌아갑니다.

```bash
uv sync --extra paddleocr    # 설치해 두면 자동으로 쓰입니다
```
