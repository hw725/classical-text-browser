...
방향을 코드가 재고(모델은 방향을 자주 틀려서 쓰지 않습니다) 비전 모델은 «무슨 글인가»만 답합니다. 화면이
«37~52쪽(16쪽)을 0°로» 같은 구간을 **그 쪽을 그렇게 돌려 보인 채** 하나씩 확인받아 저장하고, 누운 쪽은 90°인지
270°인지 코드가 못 가리므로 아니라고 하면 반대쪽을 한 번 더 보입니다.
같은 훑어보기가 **쪽마다 알맞은 OCR 엔진**도 권합니다 — 근현대 활자는 NDLOCR, 고서 판본은 古典籍 Lite, 손글씨는
古典籍 Full, 훈점·오쿠리가나가 달린 쪽은 みんなで翻刻OCR, 한글이 있으면 LLM 비전(설치된 것 중에서만). 권한 구간을
고르면 교감 탭 「권 전체 OCR」의 쪽 범위·엔진·「된 쪽도 다시」가 채워지고, 그 단추를 누르면 그 쪽들만 그 엔진으로
읽습니다. **한 쪽에 한글과 훈점이 함께 있는 것처럼 종류가 섞인 쪽**은 엔진 하나로 읽을 수 없어 따로 알립니다 —
...
2. 설정:
   - **엔진**: 드롭다운에서 선택 — 없는 엔진은 «(사용 불가)»로 흐리게 보이고, 이유는 마우스를 올리면 뜹니다.
     ...
     다시 읽을 수 있습니다(덮기 전 백업을 남깁니다). 어느 쪽에 어느 엔진이 맞는지는 뷰어의 「훑어보기」가 권합니다(§5.0)
