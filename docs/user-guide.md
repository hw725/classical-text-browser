...
**다시 돌리기 직전의 결과가 한 벌 남습니다.** 되돌릴 수 있는 쪽에는
검수 목록에 **「되돌리기」** 버튼이 뜹니다.

**OCR 결과와 교정 텍스트가 함께 되돌아옵니다.** 다시 돌리면 손으로 고친
교정도 덮이기 때문에, 하나만 되돌리면 어긋난 상태가 됩니다.

되돌린 뒤 한 번 더 되돌리면 원래대로 옵니다 — **두 상태를 오가며 비교**할 수
있습니다. 다만 남는 것은 **한 세대**뿐이라, 세 번 돌린 뒤 첫 번째로 가는 것은
안 됩니다.
