...
답합니다(모델은 방향을 자주 틀려서 방향에는 쓰지 않습니다). 끝나면 «회전이 다른 구간 N개: 37~52쪽 → 0° …
모두 저장할까요?» 한 번만 묻습니다. PaddleOCR이 없어 판정이 «추정»인 구간만 그 쪽을 돌려 보인 채 따로 묻습니다.
PaddleOCR 판정은 쪽당 두 번이고 GPU로 돕니다. 실측 3쪽 20초로, 대부분은 비전 모델 호출 시간입니다(보내기 전
창에 횟수가 뜹니다). 182쪽 전체보다 범위를 적어 의심되는 구간만 훑는 편이 빠릅니다.
