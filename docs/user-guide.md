...
○권점·표제 어휘)를 스스로 찾아 **깊이 있는 개요**(1단 > 2단 > 3단 …)를 세웁니다 — 무엇으로 세웠는지 알림에 적힙니다.
...
| 4 | + くずし字 엔진도 | 붓으로 흘려 쓴 초서·변체가나 고문서까지(みんなで翻刻OCR) |

나중에 바꿔도 됩니다 — 앱 안 **설정 ▸ 처음 설정 ▸ 글자 인식**의 「설치」 단추. 터미널 명령은 필요 없습니다.

마지막 5단계에서 **글자 인식 모델(약 240MB)을 미리 받습니다.** 처음 한 번뿐이고, 여기서 받아 두면
첫 OCR이 바로 됩니다. 4번을 골랐으면 5-1단계에서 みんなで翻刻OCR 모델(약 290MB)도 받아 둡니다.
이 단계가 실패해도 설치는 끝나며, 첫 OCR 때 다시 받습니다(그때는 몇 분 걸릴 수 있습니다).
...
uv sync --extra honkoku        # みんなで翻刻OCR — くずし字(초서·변체가나) 고문서, 약 20MB + 모델 290MB(첫 사용 때 받음)
...
> 개요(깊이 있는 트리)를 세우는 가장 빠른 길은 「내용」의 **자동 트리**입니다. 행마다 ⇤⇥로 깊이,
...
- 교정 인덱스는 확정본이 비어 있으면 OCR 결과로 저절로 채웁니다 — 따로 누를 단추가 없습니다.
...
작업을 하다 보면 같은 인물·개념이 이름만 달리한 채 둘로 서는 일이 생깁니다.
엔티티 목록에서 개념 행의 **「합치기」**를 누르고 남길 개념을 고르면 됩니다.

> **지우지 않고 합칩니다.** 흡수된 개념의 기록은 그대로 남고 상태만
> 「deprecated」가 됩니다. 이미 논문 각주나 내보낸 사전에 그 옛 ID를 적어 두셨더라도
> **참조가 끊기지 않습니다** — 옛 ID로 찾으면 「지금은 이쪽을 보세요」가 함께 나옵니다.
> 목록에서도 합쳐진 개념 옆에 「→ 새 ID」 표시가 붙습니다.

승격된 개념을 열면 **「왜 이 개념이 올라왔는가」**가 위쪽에 적혀 있습니다.
출처의 «개수»가 아니라 «무게»로 재기 때문에, 스치듯 한 번씩 나온 출처가 아무리
많아도 승격 근거가 되지 않습니다.
...
### 5.0 옆으로 스캔된 책 — 먼저 회전을 저장하세요

족자·가로형처럼 쪽이 누워 스캔된 책은 **OCR 전에** 세워야 합니다. 엔진은 글자가 바로 선 것을 전제하므로
누운 채로 돌리면 세로 열이 가로 줄로 잡히고 글자마다 90° 돌아가 인식이 깨집니다.

1. 뷰어 툴바의 **↺ ↻**로 쪽이 바로 설 때까지 돌립니다(임시 — 화면만 돕니다).
2. 그 옆에 나타나는 **「저장」**을 누릅니다. 회전은 **권의 속성**으로 저장되고, 그 뒤로 OCR·레이아웃 감지·
   LLM 판독·텍스트 레이어 PDF가 모두 돌린 이미지를 씁니다. 다른 쪽·다음에 열 때도 그대로입니다.
3. 이미 OCR·레이아웃 결과가 있는 권이면 «OCR·레이아웃 결과가 있는 N쪽은 좌표계가 어긋나므로 다시 돌려야 합니다
   (지우지는 않습니다). 계속할까요?»라고 묻고, 저장 뒤에 «N쪽은 다시 OCR해야 합니다»라고 알립니다.
   저장하면 옛 결과는 좌표계가 달라 OCR이 그 쪽의 레이아웃을 쓰지 않고 다시 잡으라고 합니다 —
   「권 전체 OCR」을 다시 돌리세요.

임시 회전(저장 안 함)은 읽기 편하게 돌려 보는 용도이고, 라벨에 «저장 90° +90°»처럼 저장된 것과 임시 것이
따로 보입니다. 읽기 보조선은 단추(또는 R)를 누를 때마다 **가로선 → 세로선 → 끔**으로 돕니다.

**책 중간에 회전이 다른 구간이 있으면**(접은 그림·가로 표·뒤집힌 낙장) 「저장」 옆의 **쪽 범위 칸**에 «37-52»처럼
적고 저장하세요. 그 쪽들만 그 회전을 갖고 나머지는 권의 회전을 따릅니다(라벨에 «저장 0°(쪽 범위)»). 칸을 비우고
권 전체로 저장하면 쪽 범위는 지워집니다. 구간이 어디인지 모르면 레이아웃 탭 「권 전체 OCR」의 **「돌아간 쪽은
세워서」**(§5.2, 모델 없이 코드가 잼)가 OCR 직전에 찾아 묻습니다. 글의 종류에 따라 엔진까지 고르고 싶으면
**「판독 계획 LLM」**을 누르세요(**GPU 환경에서만** 보입니다 — 바탕화면 아이콘으로 켠
서버. 쪽마다 OCR을 두 번 돌려 CPU에서는 한 시간이 걸리기 때문입니다) — 쪽마다 글줄의
방향은 코드가 재고(투영으로 «섰다/누웠다», PaddleOCR 점수 비교로 180°와 좌우까지), 비전 모델은 «무슨 글인가»만
답합니다(모델은 방향을 자주 틀려서 방향에는 쓰지 않습니다). 누르면 창이 하나 뜹니다 — **쪽 범위**(비우면 권
전체, «37-60»), **모델**(글의 종류를 볼 비전 모델. 기본은 `kimi-k3:cloud` — 표본 10쪽 벤치마크에서 종류 정답
7/10·쪽당 1.5초로 가장 나았습니다. «자동»은 되는 프로바이더 순서), 그리고 범위를 고칠 때마다
«N쪽 — 비전 모델 N번 + PaddleOCR M번»이 바로 밑에 뜹니다. 「계획 세우기」를 누르면 같은 창에 진행 막대가 «3/20쪽 —
9쪽 고서 판본 → ndlkotenocr»처럼 쪽마다 올라가고, 끝나면 «회전이 다른
구간 N개: 37~52쪽 → 0° … 모두 저장할까요?» 한 번만 묻습니다. PaddleOCR이 없어 판정이 «추정»인 구간만 그 쪽을
돌려 보인 채 따로 묻습니다. PaddleOCR 판정은 쪽당 두 번이고 GPU로 돕니다. 실측 3쪽 20초로, 대부분은 비전 모델
호출 시간입니다. 결과 알림에 어느 모델이 답했는지(«gemma4:cloud») 적힙니다. 182쪽 전체보다 범위를 적어 의심되는
구간만 훑는 편이 빠릅니다.
같은 판독 계획이 **쪽마다 알맞은 OCR 엔진**도 권합니다 — 근현대 활자는 NDLOCR, 고서 판본은 古典籍 Lite, 손글씨는
古典籍 Full, 훈점·오쿠리가나가 달린 쪽은 みんなで翻刻OCR, 한글이 있으면 LLM 비전(설치된 것 중에서만). 권한 구간은
**계획**으로 레이아웃 탭 「권 전체 OCR」 옆에 «판독 계획대로 · 1~36쪽 활자 → NDLOCR · 37~52쪽 훈점 →
みんなで翻刻»처럼 뜨고, 켜진 채 그 단추를 누르면 **쪽마다 계획의 엔진으로 한 번에** 돕니다(「지우기」로 끕니다.
화면을 새로 열면 사라집니다). **한 쪽에 한글과 훈점이 함께 있는 것처럼 종류가 섞인 쪽**은 엔진 하나로 읽을 수 없어 따로 알립니다 —
레이아웃 탭에서 영역을 나눈 뒤 영역마다 엔진을 골라 「선택 블록 OCR」로 읽으세요. 다만 **활자+한글**(한글 논문)과
**판본+훈점**(훈점본)은 한 엔진(LLM 비전·みんなで翻刻)이 함께 읽으므로 섞였다고 하지 않습니다. 모델 목록에서
«○ (은퇴한 모델)»로 흐려진 것은 Ollama가 서비스를 끝낸 클라우드 모델이라 고를 수 없습니다.

### 5.0-1 くずし字 고문서 — みんなで翻刻OCR

붓으로 흘려 쓴 초서·변체가나 문서에는 **みんなで翻刻OCR**(橋本雄太, CC BY 4.0)의 Python 이식
[honkoku-ocr-py](https://github.com/mkpoli/honkoku-ocr-py)를 엔진으로 고를 수 있습니다.
설치 때 4번을 골랐거나 `uv sync --extra honkoku`·앱 안 「설치」로 깔면 OCR 엔진 목록에 「みんなで翻刻OCR」이 나옵니다.
모델 약 290MB는 설치 4번이면 설치 때 미리 받고, 다른 길로 깔았으면 첫 사용 때
받습니다(저장 위치는 `HONKOKU_OCR_MODELS`). 행 검출은 NDL古典籍OCR-Lite와 같은 모델이라 두 엔진의 읽기를
행 단위로 견줄 수 있습니다. 저장되는 글은 **원문 글자**뿐입니다 — 후리가나·返り点·送り仮名 같은 훈독 보조는
빼고, みんなで翻刻 기법 그대로의 표기(Koji)는 L2 행의 `koji` 칸에 따로 남습니다. CPU만 있으면 한 쪽에 1분 남짓
걸리고(실측 85초, 인식기가 대부분), CUDA가 있으면 몇 초입니다(`HONKOKU_OCR_DEVICE=cuda`). 한글은 인식하지
못합니다.
...
2. 설정:
   - **엔진**: 드롭다운에서 선택 — 없는 엔진은 «(사용 불가)»로 흐리게 보이고, 이유는 마우스를 올리면 뜹니다.
     「권 전체 OCR」 옆의 **쪽 범위 칸**(«37-52»·«3,7,10-12»)과 **「된 쪽도 다시」**로 일부 쪽만 다른 엔진으로
     다시 읽을 수 있습니다(덮기 전 백업을 남깁니다). 뷰어의 「판독 계획」(§5.0)이 만든 **계획**이 있으면 그 옆에
     «판독 계획대로»가 뜨고, 켜진 채 누르면 쪽마다 계획의 엔진으로 돕니다(쪽 범위를 적으면 그 쪽만, 엔진은
     계획대로). **「돌아간 쪽은 세워서」**(기본 켬)가 켜져 있으면 OCR 직전에 옆으로 누운 쪽을 코드가 찾아
     «37~52쪽 → 90°»처럼 구간으로 묻고, 확인하면 그 쪽 범위의 회전을 저장한 뒤 OCR합니다 — 한 PDF에 세로 본문과
     가로로 돌린 쪽이 섞여 있을 때 이것 하나면 됩니다(모델을 부르지 않고 CPU에서도 됩니다. GPU에 PaddleOCR이
     있으면 90°·270°까지 대개 확정하고, 못 가렸거나 CPU면 첫 쪽을 돌려 보여 주며 묻습니다). 취소하면 회전
     없이 그대로 OCR합니다. 단, 뷰어가 다른 권을 보고 있으면 미리보기를 띄울 수 없어 **확정된 구간만** 묻고
     추정 구간은 건너뜁니다 — 그 권을 뷰어에 띄운 뒤 다시 누르세요
     - **NDL古典籍OCR TrOCR ver2** (Full) — 최고 품질, GPU 필요 (별도 환경 — [7-A.6-2](#7-a6-2-gpu로-돌리기--별도-환경-venv-gpu-선택))
     - **NDL古典籍OCR-Lite** — 고전적 전용, CPU OK (설정 ▸ 처음 설정 ▸ 글자 인식 ▸ 「설치」)
     - **NDLOCR-Lite** — 근현대 일본어 자료 (같은 자리에서 「설치」)
     - **みんなで翻刻OCR** — くずし字(초서·변체가나) 고문서, CPU OK (같은 자리에서 「설치」 — 이 길로 깔면 첫 사용 때 모델 290MB)
...
### 5.2-1 LLM 교정 사다리와 정밀 판독

엔진 OCR을 먼저 돌린 뒤, **다시 볼 곳만** LLM에 넘기고, 그래도 확실치 않은 곳만 더 비싼
단계로 올리는 **사다리**입니다(설계 결정 D-082). LLM은 이미지와 **엔진이 읽은 결과**를 함께
받아 교정하므로 백지에서 읽을 때보다 지어내는 여지가 적습니다. 결과는 L2를 덮지 않고
**교정 초안**으로 저장됩니다.

- **「LLM 교정(선별)」** — 신뢰도가 낮은 블록, 협주·방주, 한글을 못 읽는 엔진의 결과만
  골라 **1단계**(추론 끔)로 다시 읽습니다. 엔진 결과와 거의 같고 불확실 표시가 없으면
  자동 수용이고, 그렇지 않은 블록만 **2단계**(앞뒤 문맥·추론 켬)로 자동으로 올라갑니다.
  2단계의 기준은 «두 단계의 답이 거의 같은가(90% 이상)»입니다 — 거의 같고 불확실·판독
  불가 글자가 없으면 자동 수용, 아니면 두 답을 나란히 보여 주고 사람이 고릅니다. 불확실
  글자(`[?]`)와 판독 불가(`□`)는 교정본 안에서 붉게 표시됩니다. 어느 블록을 왜 골랐는지
  초안 목록에 이유가 붙습니다.
- **「정밀 판독(선택)」** — 선택한 블록 하나를 곧장 2단계로 읽습니다. **앞뒤 블록과 앞뒤
  쪽의 확정본**을 문맥으로 붙여 **추론을 켠 채** 읽으므로 행초·흘림체처럼 자형만으로 안
  풀리는 곳에 씁니다. 추론 예산은 답변 예산에 더해지고, 출력이 잘리면 추론을 끄고 한 번
  더 시도합니다(D-083). 같은 쪽의 다른 블록 초안은 그대로 남습니다.
- 초안 목록의 각 블록에는 단계(1단계·2단계), **엔진 결과 → 교정본**(두 단계가 다르면
  둘 다), 일치율, `[?]`·`□` 수가 보입니다. 쪽 단위 실행에서는 **「적용」**을 누른 블록만
  L4(교정 텍스트)에 들어가고, 들어간 블록은 「적용됨」으로 표시됩니다. 초안은 쪽을 옮겼다
  돌아와도 다시 보입니다. OCR을 다시 돌려 글자가 바뀌면 그 전의 초안은 «오래된 것»으로
  표시되고 적용할 수 없습니다 — 「LLM 교정(선별)」을 다시 누르세요. 교정 편집기에 저장하지
  않은 수정이 있으면 「적용」이 막힙니다(먼저 저장하세요 — 적용 뒤에 저장하면 교정이 지워집니다).
- **「검토할 쪽」** — 이 권에서 사람이 볼 초안이 남은 쪽 목록입니다(볼 것·자동 수용됐는데
  아직 적용 안 된 것·오류·적용 뒤 답이 바뀐 것). 쪽을 누르면 그 쪽으로 갑니다. 일괄 OCR을
  돌린 뒤 어디를 열어야 하는지 여기서 봅니다.
- 권 단위 일괄 OCR(추출 패널)에서는 **「LLM 교정」**을 「선별만」이나 「전체」로 켤 수
  있습니다. 기본은 끔입니다 — 켜면 블록마다 1~2회(2단계는 추론 켬) LLM 호출이 나가고,
  비용 줄에 그 사실이 적힙니다. 켜면 쪽마다 위 사다리가 돌고, 자동 수용된 블록은 사람의
  클릭 없이 L4에 들어가며, 남은 블록은 완료 메시지와 「검토할 쪽」에 셉니다.
...
1. **교정** 인덱스 선택
2. 확정본이 아직 없는 쪽은 OCR 결과가 **저절로** 교정 편집기에 들어옵니다(이미 고친 글은 덮지 않습니다.
   글을 다 지우고 저장하면 다음에 열 때 OCR 결과로 다시 채워집니다)
...
**목적**: 권 전체 확정본을 **깊이 있는 개요**(1단 卷·集, 2단 글, 3단 문단·협주 — 깊이는 제한 없음)로 나눕니다.
...
1. 사이드바 **「내용」 ▸ 「자동 트리」**. 이 책의 확정본에서 되풀이되는 표지를 스스로 찾아
   ...
   - 편성 인덱스는 **①②③ 한 흐름**입니다(D-122). 탭을 열면 ①이 스스로 전문을 세어 **「찾은 규약: …」**
     한 줄을 보이고, ②에 규칙 후보가 체크박스로, ③에 경계 후보가 섭니다. 단추는 넷뿐입니다 —
     ①「말로 넣기」·「더 묻기…」, ②「후보 보기」, ③「적용」.
   - **① 이 책은 어떻게 나뉘나** — 아는 것을 한 줄 쓰고 「말로 넣기」. 목차·제목이 없으면 그것부터
     말하고(«목차와 제목이 없다»), 경계 표지가 무엇인지 이어서 씁니다(«기사는 ○ 뒤의 날짜로 시작한다»,
     «제목 행은 ○○로 끝난다»). 옮긴 것은 ②의 체크에 바로 들어가고, 옮기지 못한 말(「권2부터는…」처럼
     범위·조건이 든 말)은 그 아래 붉게 그대로 보입니다 — 비슷한 칸으로 바꿔치기하지 않습니다. 어휘는
     전문에서 몇 번 나오는지가 함께 적히고, 0번이면 이 책의 말이 아닙니다. 「더 묻기…」는 목차 항목
     구조화·표제 어휘 뽑기·시작 표지 묻기(표본 범위 고르기)·**구조를 통째로 묻기**를 고르는 창입니다. 모델을 부르는
     것은 이 둘뿐이고, 이미지는 보내지 않습니다(앞의 셋은 텍스트 조각만, 「구조를 통째로 묻기」는 권 전문 텍스트).
   - **구조를 통째로 묻기** — 표지가 제각각이라 ②의 규칙이 못 잡는 책(문집·잡록)에 씁니다. 권의 확정본 전문에
     행 번호를 붙여 보내고, 모델은 «새 글이 시작하는 행·제목·깊이»를 **번호로 가리키기만** 합니다 — 글을 고쳐 쓰지
     않으므로 이미지 위 점선·앵커·되돌리기가 그대로 됩니다. 실제 행이 아닌 답은 버리고, 제목이 그 행의 글자가 아니면
     행의 글자로 바꿉니다(창을 닫으면 «가리킨 N자리 중 M개가 실제 행과 맞아…»로 셈이 보입니다). 맞은 것은 ③에
     **«LLM 구조»** 표지를 달고 체크된 채 서고, 규칙 후보와 같은 자리면 그 후보에 표지만 보탭니다. 보내기 전에
     선택지 옆에 «권 전문 N행·M자·호출 K번»이 뜹니다 — 긴 권은 7,000자 묶음으로 나눠 묶음마다 한 번 부릅니다.
     저장은 여전히 「적용」 한 곳입니다. **모델에 따라 답이 갈립니다** — 실측(속몽구 권4, 225행)에서 Ollama
     `kimi-k3:cloud`는 13초에 표제 행 10편 중 8편을 맞혔고, `glm-5.3:cloud`는 답 대신 영문 추론을 써 내려가
     실패, 로컬 `gemma4:e4b`는 빈 답이었습니다. «LLM 실패»가 뜨면 「모델」에서 다른 것을 골라 다시 물으세요. 어느 모델이 도는지는 두 단추 옆 **「모델」** 드롭다운이 정합니다(«자동»이면 Ollama → Gemini …
     순서로 되는 첫 것). 해제(解題)는 ①의 칸에 함께 붙여 넣습니다(2만 자여도 됩니다) — 같은 칸에 «어떻게 나뉘는지»를 한 줄씩
     쓰고 「말로 넣기」를 누르면 나뉨을 말한 문장만 규칙으로 옮기고 나머지는 참고로 읽습니다(옮기지 못한 말은 접혀
     보입니다). 「더 묻기…」 창 맨 위에 «①의 글 N자를 함께 읽습니다»가 뜹니다. 따로 두던 해제 칸은 없앴습니다.
   - **판정 모델로 고르기** — 같은 「더 묻기…」 창의 또 다른 항목입니다. 글을 만들지 않고 **판정만**
     돌려주는 모델(TypeSafe Jev)에게 행마다 «여기서 새 글이 시작합니까»를 묻고 0~1 확률을 받습니다.
     총목이 있으면 **목차 항목이 본문 어느 행인지**도 후보 몇 개 중에서 고르게 합니다(«없음»도 선택지에
     있어 본문에 없는 항목은 붙이지 않습니다). 목차가 답하는 것은 권·集(층위 1~2)이고 본문 판정이 답하는
     것은 그 안의 낱글이라, 본문 후보는 목차 경계 **아래 층**으로 내려갑니다. 체크하면 선택지 옆에
     «N행 · 질문 N개 · 호출 N번 · 약 $0.0X»가 먼저 뜹니다(입력만 청구됩니다). 답은 ③에 **«판정 모델»**·
     **«목차·판정 모델»** 표지를 달고 서고, **규칙 후보를 거르지 않습니다** — 규칙으로 걸러 내면 규칙이
     못 보는 책에서 함께 눈이 멉니다(실측: 문집에서 찾는 비율 0.88 → 0.18). 쓰려면 TypeSafe API 키가
     필요하고, 없으면 그렇게 알립니다. 한문·한국어는 이 모델이 스스로 «정확도가 낮다»고 밝히는 언어이니
     **답을 그대로 믿지 말고 ③에서 골라** 쓰세요.
   - **두 길이 얼마나 다른가** — 판정 모델을 돌리면 ①의 한 줄 아래에 «규칙이 찾은 자리 N · 모델이 찾은
     자리 M — 두 길이 많이 다릅니다(8.4배, 합의 67%)»가 섭니다. **판정이 아니라 모름의 표시입니다** —
     어느 쪽이 틀렸는지는 이 값이 말해 주지 않으므로 아무것도 자동으로 바꾸지 않습니다. 많이 다르면
     표본 몇 쪽을 먼저 보세요.
   - **모델 후보 상위 N개** — ③ 위의 손잡이입니다. 확률 문턱(0.7 같은 숫자) 대신 **몇 개까지 볼지**로
     묻습니다 — 좋은 문턱이 책마다 달라서입니다(일기류와 문집이 서로 달랐습니다). 손잡이를 줄이면 확률이
     낮은 모델 후보가 목록에서 빠지고 체크도 풀립니다(**버리는 것이 아니라 가리는 것**이라 다시 늘리면
     돌아옵니다). 규칙 후보는 이 손잡이와 무관하게 늘 보입니다.
   - **② 규칙 고르기** — 전문에서 센 규약 후보가 횟수·보기와 함께 체크박스로 뜹니다(판심·엽수처럼 쪽마다
     같은 자리에 오는 것은 뺀 채로). 켜진 것만 펼쳐져 있고 점수가 낮은 것은 「센 것 N줄 더」에 접혀 있습니다 —
     체크하면 규칙이 됩니다. 「목차」 줄의 체크가 목차 사용 스위치입니다. 어휘를 직접 더할 수 있고(행 첫머리
     어휘는 `^有`처럼 `^`를 앞에, 기호 한 글자는 그대로), 억제 목록·표제 최대 글자수는 「억제·글자수」에 있습니다.
     체크를 바꾸면 ③의 「적용」이 잠기고 **「후보 보기」**로 다시 세웁니다(저장하지 않습니다). 전에 저장한
     설정이 이번 답과 다르면 ①에 「권고대로」 단추가 나옵니다.
   - **③ 후보 고르기** — 이 권에 **저장된 경계가 먼저** «지금 경계» 표지를 달고 체크된 채로 서고, 규칙이
     새로 찾은 후보는 그 사이에 체크 없이 섭니다. 그래서 닫았다 열어도 사이드바 「내용」과 같은 것을 보고, 아무것도
     안 건드리고 「적용」하면 아무 일도 없습니다. 체크 = 경계가 될 것 — 저장된 행의 체크를 빼면 「적용」 때
     지워집니다(손으로 넣은 것도). **행을 누르면 그 행 바로 아래에 도구**(역할·깊이·「바꾸기」·「☑ 경계로」·
     「☐ 경계에서 빼기」·「억제」)가 뜨고, Shift로 범위·Ctrl로 하나씩 여러 행을 잡으면 «고른 N행에» 한꺼번에 듭니다.
     ①②③은 머리를 누르면 접히고, 접은 것은 이 브라우저가 기억합니다(①을 접어도 «찾은 규약» 한 줄은 남습니다).
     ③ 머리의 «바뀐 것»은 저장된 경계와 견주어 «새로 세울 자리 N · 지울 자리 M»을 체크를 바꿀 때마다 다시 셉니다. 목차에는 있으나 본문에서 못 찾은 제목은 ③ 위에 노랗게 뜹니다 — 제목을 누르면 비슷한
     행이 보이고 「여기서 시작」으로 그 자리를 후보에 넣습니다(체크된 채·«목차·찾아 넣음»). 비슷한 행이 없으면
     사이드바 「경계 넣기」로 직접 넣습니다.
     「전체」는 보이는 후보 전부를 체크·해제합니다. 규칙을 바꿔 다시 세우면 머리에 **«바뀐 것 — 새로 잡히는
     자리 N · 빠지는 자리 M»**과 그 자리들이 먼저 보입니다 — 저장된 규칙(없으면 권고)으로 센 후보와 견준
     것입니다. 체크·역할·깊이는 자리로 기억하므로 다시 세워도 남습니다.
   - **「적용」** 하나가 ②의 규칙을 저장하고 체크한 후보로 경계를 세웁니다. 지울 경계가 있으면 **실제 수**를
     묻습니다(«경계 584개를 세우고, 지금 경계 중 2개를 지웁니다»). 전에 제안으로 만든 경계 중 체크에 없는 것은
     정리되고 손으로 넣은 경계는 남습니다 — 「손으로 넣은 경계도 정리」를 켜면 그것도 지웁니다. Git으로 되돌릴
     수 있습니다.
   - 확정본(L4)이 없는 권은 ①에 붉게 «확정본(L4)이 없어 후보를 만들 수 없습니다»가 뜹니다 — 「권 전체 OCR」을 먼저.
2. 행마다 손봅니다 — **⇤⇥** 깊이, **▲▼** 시작 행(옮기기 전에 어디서 어디로인지 묻습니다),
   **×** 앞 단위에 합치기.
   - 단위의 종류는 **깊이 하나**입니다. 「묶음/기사/조각」이라는 역할 이름은 2026-09-11에 화면에서 뺐습니다 —
     깊이가 트리의 모양(어디서 끝나는지)을 다 정하고, 역할이 동작을 바꾸는 데가 없었기 때문입니다.
     1단은 굵게, 3단부터는 흐리게 보입니다.
3. 새 경계는 **「＋ 경계 넣기」**. 펼치면 «찍기»가 켜져 원본 이미지나 교정 텍스트에서 자리를
   누르면 됩니다 — 쪽·행·글자를 세지 않습니다. 붉은 점선과 그 자리 글자 여덟 자를 확인하고
   깊이를 적은 뒤 「넣기」.
4. 기사 하나를 문단으로 나누려면 사이드바 「내용」에서 그 기사를 고른 뒤 편성 인덱스 맨 아래 **「단위 손보기」**의
   「쪼개기」를 누르고(고른 기사가 제목에 보입니다. 아래 목록에서 카드를 눌러도 됩니다) 나눌 자리에
   `===`를 넣습니다. 새 기사는 생기지 않습니다 — 기사를 나누는 것은 「경계 넣기」입니다.

- 편성 인덱스는 「적용」 전까지 아무것도 저장하지 않습니다 — 「후보 보기」도 「말로 넣기」도 화면의 규칙만
  바꿉니다. 「적용」은 **체크 상태가 곧 트리**입니다 — 손으로 넣은 경계는 남습니다(「손으로 넣은 경계도 정리」를
  켜지 않는 한).
- 규칙은 **확정본(L4)이 있는 쪽**만 읽습니다. 고서 흐름의 「권 전체 OCR」이 확정본을 채우므로 보통은
  그대로 되고, 일부 쪽에만 확정본이 있으면 사이드바 「자동 트리」가 «확정본(L4)이 있는 쪽은 N/M쪽입니다»라고
  알리고 편성 인덱스 ①은 «확정본 N쪽»만 보입니다 — 나머지 쪽은
  교정 인덱스에서 그 쪽을 한 번 열면 확정본이 채워집니다 — 그 뒤 다시 세우세요. 단위를 나중에 바꿔도 표점·번역은
  제자리에 남습니다.
...
| 범주 | 인물·지명·사건·시간/시기·물건·기록/문헌·예술품·음식·의복·개념·문법 가운데 하나 | 인물 |
| 범위 | 일반 용법인지, 이 글에서만 특별히 쓰였는지 | 일반 용법 |
| 해설 | 독음 주의·문법적 구실·이 문맥의 깊은 해석(학술적 보충) | 「戎」은 «융»으로 읽는다 … |
| 출전 | 참고문헌 | 《晉書》 列傳 第十三 |

LLM은 글의 규모에 맞추어 **15개 내외**를 고르고, 뜻이 뻔한 허사는 빼되 읽기를 좌우하는 문법 요소는
«문법»으로 넣습니다. 주석 유형(색·필터)은 범주에서 정해집니다 — 인물 → 인물, 지명 → 지명, 기록/문헌 →
서명, 문법 → 문법, 나머지 → 용어.
...
### 7.5 내보낸 결과에 찍히는 「코퍼스」 표시

내보내기에는 `ctb-corpus:7153819ae0de7333` 같은 짧은 표시가 **자동으로** 붙습니다.
입력하실 것은 없습니다.

무엇에 쓰는가 — 논문에 인용문이나 사전 항목을 실은 뒤, 몇 달이 지나 원문을 더
교정했다고 해 봅시다. 그때 「내가 인용한 것은 **어느 시점** 교정본인가」를
답할 수 있어야 합니다. 이 표시가 그 시점을 가리킵니다. 각주에 그대로 옮겨 적으시면
됩니다.
...
| 산출물 | 어디에 |
|---|---|
| JSON 스냅샷 · 사전 내보내기 | 파일 안 `corpus_version` 항목 |
| 인용 내보내기 | 복사되는 글의 첫 줄 |
| 경계 CSV | 맨 오른쪽 **「코퍼스」 열** |
| 검색 가능한 PDF | 파일 속성의 「키워드」 |

> **「커밋되지 않은 편집 있음」이 함께 찍힌다면** 저장하지 않은 작업이 남아 있다는
> 뜻입니다. 그 상태로 내보낸 결과는 나중에 똑같이 되살릴 수 없습니다 — 먼저
> 저장(버전 기록)하신 뒤 다시 내보내시면 표시가 깨끗해집니다.
...
- `.venv-gpu`의 **PaddleOCR GPU 워커 ping** — 서버가 실제로 쓰는 워커(같은 `.venv-gpu` 파이썬, torch 차단)를
  띄워 GPU로 뜨는지 봅니다. «✓ paddle GPU 워커 ping (…, GPU)»면 PaddleOCR이 GPU로 돕니다. 안 뜨면 CPU로
  내려간다고 «참고»로 알립니다
