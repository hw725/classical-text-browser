...
**「훑어보기 LLM」**을 누르세요(**GPU 환경에서만** 보입니다 — 바탕화면 아이콘으로 켠
...
«N쪽 — 비전 모델 N번 + PaddleOCR M번»이 바로 밑에 뜹니다. 「훑어보기」를 누르면 같은 창에 진행 막대가 «3/20쪽 —
...
구간만 훑는 편이 빠릅니다.
같은 훑어보기가 **쪽마다 알맞은 OCR 엔진**도 권합니다 — 근현대 활자는 NDLOCR, 고서 판본은 古典籍 Lite, 손글씨는
古典籍 Full, 훈점·오쿠리가나가 달린 쪽은 みんなで翻刻OCR, 한글이 있으면 LLM 비전(설치된 것 중에서만). 권한 구간은
**계획**으로 레이아웃 탭 「권 전체 OCR」 옆에 «훑어보기 계획대로 · 1~36쪽 활자 → NDLOCR · 37~52쪽 훈점 →
...
2. 설정:
   - **엔진**: 드롭다운에서 선택 — 없는 엔진은 «(사용 불가)»로 흐리게 보이고, 이유는 마우스를 올리면 뜹니다.
     ...
     다시 읽을 수 있습니다(덮기 전 백업을 남깁니다). 뷰어의 「훑어보기」(§5.0)가 만든 **계획**이 있으면 그 옆에
     «훑어보기 계획대로»가 뜨고, 켜진 채 누르면 쪽마다 계획의 엔진으로 돕니다(쪽 범위를 적으면 그 쪽만, 엔진은
