...
3. 이미 OCR·레이아웃 결과가 있는 권이면 «N쪽은 다시 OCR해야 합니다 (지우지는 않습니다)»라고 묻습니다.
...
`uv sync --extra honkoku`로 깔면 OCR 엔진 목록에 「みんなで翻刻OCR」이 나오고, 첫 사용 때 모델 약 290MB를
...
1. 사이드바 **「내용」 ▸ 「자동 트리」**. 이 책의 확정본에서 되풀이되는 표지를 스스로 찾아
   ...
   - **① 이 책은 어떻게 나뉘나** — 아는 것을 한 줄 쓰고 「말로 넣기」. 목차·제목이 없으면 그것부터
     ...
     「더 묻기…」가 읽습니다 — 창 맨 위에 «해제 N자» 또는 «해제 없음»이 뜨고 「해제 넣기」가 그 칸으로 데려갑니다.
   ...
   - **「적용」** 하나가 ②의 규칙을 저장하고 체크한 후보로 경계를 세웁니다. 지울 경계가 있으면 **실제 수**를
     ...
     수 있습니다.
   - 확정본(L4)이 없는 권은 ①에 붉게 «확정본이 없어 후보를 만들 수 없습니다»가 뜹니다 — 「권 전체 OCR」을 먼저.
