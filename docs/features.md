...
- **경계 제안** — 권 전체 확정본에서 날짜·표제 어휘·형식·卷 표제를 보고 후보를 찍어 줍니다.
  체크한 것만 단위가 됩니다.
- **자동 트리** — 목차·해제·들여쓰기·위치를 합쳐 묶음 > 기사 > 조각의 개요를 한 번에 세웁니다.
...
- 편성은 **원본 저장소**의 일입니다. 문헌만 고르면 되고, 한 문헌에 해석 저장소가 여럿이면
  같은 편성을 함께 씁니다. 해석 저장소는 표점 탭부터 씁니다.
...
| **편성** | 글 단위의 경계 정하기 — 경계 제안·자동 트리·찍기. 본문은 저장하지 않고 확정본에서 잘라 오므로 교감이 자동 반영 |
...
| **처음 설정·업데이트** | 처음 설정 마법사(서고 → 글자 인식 엔진 → AI 연결), API 키·엔진 설치·ChatGPT 로그인·Ollama 로그인과 모델 받기를 전부 화면의 단추로(D-102·D-106·D-107), 앱 안에서 새 판 받기(D-103) |
