...
## B-009 화면 JS 의 전역 함수 이름이 여섯 겹친다 (2026-09-22 실측) — **닫힘**

> **닫힘 (2026-09-22).** 여섯 다 없앴다. `KNOWN` 이 비었고 `test_no_new_shadowing` 이
> 새 겹침을 막는다. 두 묶음으로 나눠 고쳤다:
>
> - **`_render*` 셋** (`b92711b`) — 지는 쪽 이름을 `_renderHyeontoAnnList`·
>   `_renderTransSourceText`·`_renderTransStatusSummary` 로. 고친 뒤 재니
>   `#trans-source-text` 0 → **1,660자**, `#hyeonto-ann-list` 0 → **3건**,
>   대조군 `#ann-source-text` 1,656 그대로, 콘솔 오류 0. **1,656 이 아니라 1,660 인 것이
>   증거다** — 심어 둔 현토 3건(4자)이 들어간 값이라, 번역 탭이 «자기 것»을 그린다는 뜻이다.
>   덤으로 역방향 오염도 사라졌다(번역 탭을 열 때 `#ann-source-text` 가 8자 → 0자).
> - **이스케이프 셋** — **재고 고쳤다.** node 로 각 구현을 돌려 무엇을 막는지 봤다:
>   `_escapeHtml` 다섯은 **동작이 같았고**(전부 `div.textContent → innerHTML`, git-graph 만
>   널 가드) 가려도 결과가 같아 위험이 낮았다. `_escHtml` 은 variant-manager·batch-correction
>   이 `"` 를, `_escAttr` 은 variant-manager 가 `'`·`>` 를 안 막았다 — **둘 다
>   annotation-editor 의 엄격한 판에 가려 «우연히» 안전했다.** 그래서 이름만 떼지 않고
>   **같은 수준으로 올려서** 뗐다(이름만 바꿨으면 회귀다).
>
> **「이기는 판을 엄격하게」는 겹침이 있는 동안만 유효한 방어였다.** 겹침을 없애는 순간
> 각 파일이 제 것을 쓰므로, 지는 판을 함께 올리지 않으면 그 화면에서 속성이 뚫린다.
>
> 그리고 `test_quotes_are_escaped` 가 **큰따옴표만 보고 있었다** — 속성 헬퍼에서 `'` 를
> 도로 빼도 초록이었다(깨뜨려 보고 찾았다). 작은따옴표 단언을 더했다.
...
③ **현토 쪽도 확정했다.** 처음 쟀을 때는 이 서고에 현토가 0건이라 「없어서 빈 것」과
구분되지 않았다. 그래서 복사본 서고에 **API 로 현토 3건을 심고** 다시 쟀다
(화면을 더듬지 않고 라우트를 직접 불렀다 — 무엇을 왜 썼는지가 남는다).

| 무엇 | 값 |
|---|---|
| `/pages/2/hyeonto?block_id=…` 응답 | HTTP 200, **현토 3건** |
| `hyeontoState.annotations.length` | **3** (편집기가 들고 있다) |
| `#hyeonto-ann-list` | **보임 true, 0자** |
| `#hyeonto-ann-count` | 처음 값 «0» 그대로 |
| 같은 순간 `#ann-list` | 34자 |
| 콘솔 오류 | 0건 |

**받아 놓고 안 그린다.** 「못 받아서 빈 것」이 아니다 — 편집기 상태에는 3건이
들어 있는데 자기 목록은 0자이고, 주석 탭 목록만 채워진다.

**주의:** 현토 탭의 `annotations` 는 **L7 주석이 아니라 현토 표기**다
(`hyeontoState.annotations` ← `/pages/{n}/hyeonto`). L7 주석을 심어도 이 자리는
움직이지 않는다 — 재려는 사람이 여기서 헷갈리기 쉽다.

콘솔 오류 0건. 즉 **예외 없이 조용히 빗나간다.**

재는 스크립트는 스크래치패드의 `b009_measure.py`(읽기 전용)와 `b009_hyeonto.py`(네트워크·편집기 상태까지 본다), 현토를 심는 것은 `plant_hyeonto.py` 다 — 셋 다 **복사본 서고**에만 쓴다.

**고친 뒤의 판정 기준:** `b009_measure.py` 를 그대로 돌려 `#trans-source-text` 글자수가 **0 → 1,656**, `#hyeonto-ann-list` 가 **0자 → 3건**이 되면 고쳐진 것이다.
