...
- **자기검증은 한 방향으로만 선다.** 후보를 고르는 자(`toc_candidates`)와 검증하는 자
  ...
  **놓침은 서지로 쟀다**(2026-09-22, 호출 0건): `db/unyangjip_works.csv`의 «권» 열은 ITKC
  ...
  (`eval_boundary_judge.py biblio`). 남은 것은 **양성 팔이 n=1**이라는 것 — 창이 겹치는 쌍을
  설계로 20~30개 심어야 「보수적이라 안 고르는 것」까지 배제된다. 그리고 n≈84라 **최소 검출
  단위가 1.2%p**이므로 위의 「0건」은 「0%」가 아니라 **「1.2% 미만」**이다.
