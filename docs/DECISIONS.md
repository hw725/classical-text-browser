...
HEAD는 **64건**이었다(직접 세었다: `git archive`로 두 트리를 풀어 각각 검사).
...
고치는 방법은 한 가지가 아니었다 — 코드 줄은 `ruff format`, 주석·docstring은 접기,
코드 안의 긴 문자열은 암시적 이어붙이기(값은 그대로), **시험이 비교하는 문자열은
손대지 않고 그 문자열 하나만 면제**. 마지막 것이 요점이다: 줄 길이를 맞추려고
검사 대상을 바꾸면 본말이 뒤집힌다.

접는 데도 함정이 둘 있었다. ruff는 글자 수가 아니라 **표시 폭**으로 잰다(한자·한글은
...
「파일 전체를 끈다」로 가면 과하다: ruff는 그 진단을 **닫는 따옴표 줄로 옮겨** 보므로
