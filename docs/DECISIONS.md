...
3. **Relation 층은 좁은 질의 어휘로만 연다.** neuPrint는 노드 17,087·엣지 46,700 규모를
   `fetch_neurons`·`fetch_adjacencies` 둘로만 노출한다. 임의 순회를 열어주지 않는 것이 핵심이다 —
   그래서 스키마가 커져도 사용처가 터지지 않는다. 「데이터 관리가 복잡하다」는 증상은 대개 저장 구조가
   아니라 **접근 경로가 무한히 열려 있어서** 생긴다. 이것은 사용자 부담을 늘리는 것이 아니라 **줄이는**
   방향이다 — 쓰는 사람이 매번 「어떻게 물어야 하나」를 정하지 않아도 된다.
   `core-schema-v1.3.md` §6.1 `Predicate Rules (Core Stability Rule)`이 서술어 축에서 이미 같은 일을
   하므로 짝이 맞는다 — 그쪽은 «무엇을 말할 수 있는가»를 좁히고, 이쪽은 «무엇을 물을 수 있는가»를 좁힌다.
