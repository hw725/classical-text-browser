...
실측하면 표준 설치는 **약 830MB**다(paddle 377 · cv2 121 · pymupdf 51 · 나머지).

**결정 1 — 「가벼운 설치」는 만들지 않는다.** (D-071에서 실측 후 뒤집음)

처음에는 `--no-install-package paddlepaddle paddleocr`로 320MB 경로를 안내했다.
**되돌렸다.** 그것은 D-055가 기본 번들로 못박은 것을 정확히 빼는 짓이었고,
OCR·PDF를 제외한 나머지 전부가 127MB라 무엇을 빼도 830MB 중 2%다.
갈림길은 기본 안이 아니라 **기본(828MB)과 고서 GPU(+4.5GB)** 사이에 있다.
...
- 830MB는 **개발 의존(pytest·ruff) 포함 실측**이다. 사용자 설치는 조금 더 작다.
...
**맥락**: 「설치가 무겁다」를 고치려다 세 번 헛짚었다. 마지막에 실물을 재고서야
무엇이 무거운지, 무엇이 문제인지가 갈렸다.
...
`FROM csp-csp:latest`로 고정돼 있었다. **개발자 PC에만 있는 개인 이미지**라
다른 사람은 GPU로 표점을 쓰려 해도 **빌드 자체가 불가능**했다. README도
「전제: 사용자의 NLP 작업 이미지가 이미 있다」로 시작했다.
...
`docker-compose.yml`의 `csp_huggingface_cache: external: true`도 같은 문제였다 —
그 볼륨이 없는 PC에서는 **볼륨을 찾을 수 없다**며 기동이 실패했다.
...
**결정 4 — 「가벼운 설치」를 만들지 않는다.**

`--no-install-package paddlepaddle paddleocr`로 320MB 경로를 만들었다가 되돌렸다.
그것은 **정확히 빼면 안 되는 것을 빼는** 짓이었다(D-055). 그리고 OCR·PDF를 뺀
나머지 전부가 127MB라, 다른 무엇을 빼도 830MB 중 2%다. **가벼운 설치는 성립하지
않는다** — 갈림길은 기본(828MB)과 고서 GPU(+4.5GB)이지 기본 안에 있지 않다.

**근거 — 왜 네 번이나 틀렸나 (기록해 둔다)**

1. paddleocr를 뺐다 → 명시적으로 기본 번들로 정한 것을 뺐다
2. 「뺄 게 없다」 → 기본 설치 안에서만 재고 extras를 안 봤다
3. 표점 27.6GB를 우리 몫으로 셌다 → Dockerfile 주석과 README를 안 읽었다
4. 표점 출처가 없다고 했다 → 세 문서에 이미 있었다

공통 원인 하나다. **재거나 읽기 전에 결론부터 냈다.** 용량은 추측할 것이 아니라
`du`로 재는 것이고, 구조는 짐작할 것이 아니라 Dockerfile을 여는 것이다.
