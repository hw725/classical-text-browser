...
- **알려진 한계**: `llm_vision`은 bbox를 반환하지 않으므로(`llm_ocr_engine.py`)
  텍스트를 **원본 글자 자리에 놓을 수 없다.** 대신 모든 줄을 왼쪽 여백에서 시작해
  세로로 균등 배치한다(실측: x=40 고정, y 간격 19.2pt).

  그래서 검색 결과의 형광은 **한 줄 크기로 정확히 뜨지만 그 자리에 글자가 없다.**
  「위치가 대략적」이 아니라 **「위치가 틀렸다」**가 정확한 서술이다. 원본이 중앙
  정렬이어도 형광은 왼쪽에 뜬다. 살아나는 것은 복사·검색·구조 분석·참고문헌 추출이고,
  잃는 것은 «어디에 있는지»다. 산출물 메타데이터(`producer`)에 `page-approximated`로
  명시해 이 PDF만 보고도 알 수 있게 한다.

  이 PC는 Python 3.13이라 좌표를 주는 유일한 한글 엔진 PaddleOCR을 쓸 수 없다
  (`registry.py`). 그 엔진이 가능해지면 **코드 변경 없이** 줄 단위 제자리 배치로 승격된다.
...
**검증**: `uv run python -m pytest` — **527 passed, 1 skipped**
(기존 490 + 신규 37: `tests/test_text_layer_pdf.py` 10, `tests/test_lite_mode_api.py` 27).
