...
1. **덮어쓰기 직전에 L2를 한 벌 남긴다** (`ocr/l2_backup.py`).
   `<문헌>/L2_ocr/.backup/{part}_page_NNN.json` — L2_ocr 안에 두므로 문헌을
   옮기거나 지울 때 함께 따라간다. **한 세대만** 남긴다.
   Git으로 이력을 다 남기지 않는 이유: L2는 쪽마다 수십~수백 KB이고 돌릴
   때마다 통째로 바뀐다. 실제로 필요한 것은 «방금 돌리기 직전»뿐이다.
...
- 사용자 표현 그대로다 — «그냥 로컬에다가 json으로 백업해두는 편이 심플».
  Git에 넣으면 저장소가 부풀고, 정작 필요한 것은 한 세대뿐이다.
...
**검증**: `uv run python -m pytest` — **631 passed** (신규 11).
백업은 되돌리기·자리바꿈·한 세대 유지·빈 결과 제외·백업 없을 때 False까지
고정했다. 라우트는 배치 2회 실행 후 `has_backup`이 False → True로 바뀌고
