...
## D-098: 내용 트리는 「문헌 > 권 > 단위」 — 경계에서 Work를 뗀다 (B-004)
...
**Work 엔티티 자체는 남긴다.** 만들기·목록·자동 생성 API는 그대로다. 다만 단위가 그것을
...
## D-100: 교환 형식의 정본은 스키마 하나 — 문서·구현·검증이 갈라지지 않게 (B-005)
...
- `POST /api/app/update` — `git pull --ff-only` → `uv sync`
