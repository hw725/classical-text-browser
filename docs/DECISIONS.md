...
**같은 날 되던 것을 깨뜨렸다가 고쳤다** — 사용자가 «되던 걸 안 되게 만든 건 아니지?»라고 물어 실제로 돌려 보니
1단계가 0건이었다. 원인 둘. (1) v2 답이 길어져(항목당 630자) Ollama 기본 문맥 창 4,096에 걸려 2,883토큰에서
잘렸고, 잘린 JSON을 파서가 통째로 버렸다(옛 프롬프트는 4,900자에 8건으로 간신히 들어갔다). num_predict를
8,192로 올려도 소용없었다 — 문맥 창이 상한이다. `_gen_options`가 «프롬프트 + 답»이 4,096을 넘으면 num_ctx도
함께 올리고(`DICT_MAX_TOKENS` 8,192), 파서는 잘린 답에서 완성된 항목만 건진다(`_recover_truncated_items`).
(2) 기존 항목을 참고로 보여 주자 모델이 type에 범주 이름(«Place»)을 그대로 적어 화면의 유형과 어긋났다 —
`type_for`가 아는 유형만 받고 아니면 범주에서 정한다. 작은 로컬 모델(gemma4:e4b)이 «15개 내외»를 채우려
「唐高宗」을 唐·高·宗으로 쪼개기도 해서 프롬프트에 «표제어는 어휘 단위, 15개는 상한»을 더했다. 고친 뒤 실제
경로(속몽구 복사본, gemma4:e4b): 한 단위 11건 — 海中三島·魏徴·漢明帝…, 범주·범위·해설 다 채워 저장됨.
...
**덧붙임 (2026-09-17) — 화면에서 내렸다.** 구현을 결정문과 대조하니 단계(선별·1단계·
2단계·적용)는 돌지만 **이음**이 없었다: ① 1단계 불합격 블록이 2단계로 자동으로 올라가지
않는다(일괄은 모드 하나로 끝나고, 화면은 모드를 보내지도 않아 늘 1단계). ② 2단계 수용
기준 「1·2단계 일치」가 없다 — 두 모드 모두 앵커와만 견준다. ③ 정밀 판독이 같은 쪽의
앞선 초안 파일을 통째로 덮어쓴다(가짜 엔진으로 재현). ④ 일괄 뒤 불합격 블록을 사람에게
보이는 자리가 없다 — 초안은 실행 직후에만 그려지고 쪽을 옮기면 다시 읽지 않는다.
⑤ 표본 감사는 「전체」 모드 말고는 없다. 그래서 「LLM 교정(선별)」·「정밀 판독(선택)」·
...
`text_agreement` ≥ 0.9·`[?]`·`□` 없음)이고 1단계 답은 `stage1`에 남아 화면이 두 답을
나란히 보인다. 1단계 답 없이 곧장 정밀 판독을 누르면 앵커 기준(`accept_basis="anchor"`).
⑤ 표본 감사는 아직 없다 — 「전체」 모드가 대신한다.

**같은 날 교차검증(Claude 리뷰 셋) 뒤 고친 것.** ㉠ 표의 2단계 수용 기준 「또는 `[?]`로 자리
표시」는 **구현하지 않는다** — `[?]`·`□`가 하나라도 있으면 1단계도 2단계도 수용하지 않고
3단계(사람)로 보낸다(1단계가 `□`를 세기만 하고 안 보던 비대칭도 없앴다). 자리는 화면에서
글자 단위로 표시한다(`_correctedTextNode`). 표는 그대로 두고 이 문단이 우선한다. ㉡ 초안은
어느 L2를 보고 만든 것인지 지문(`l2_fingerprint`, sha1)을 적는다 — OCR을 다시 돌린 뒤의 초안은
`draft_is_stale`이 잡아 화면은 「적용」을 감추고, `apply_draft`는 거부(409)하며, 검토 목록에서
빠지고, 새 실행은 그 위에 병합하지 않는다. 일괄에서 후보가 0이면 옛 초안 파일을 지운다.
㉢ 회계 불변식 `pending + accepted + errors + applied = blocks`: 빈 답은 오류, 적용 뒤 다시
읽어 답이 달라진 블록은 충돌(`applied_text` 대조)로 pending, 깨진 초안 파일은 목록에 «깨짐»으로
선다. 검토 목록과 일괄의 `review_pages`는 같은 잣대 `needs_human`(pending·errors·accepted)을
쓴다. ㉣ 2단계가 터지면 저장된 1단계 초안을 돌려주고(`stage2_error`) 일괄은 디스크의 초안을
회계에 넣는다. ㉤ 일괄 회계가 중첩 함수 안의 import 때문에 NameError로 조용히 0이 되던 것을
고쳤고, 그 경로를 `tests/test_lite_mode_api.py`가 가짜 비전 엔진으로 잰다. ㉥ 사다리로 돈
초안은 `run_mode="ladder"`를 남긴다(`mode`는 마지막 단계). ㉦ 추출 패널의 비용 줄과 옵션
라벨이 «블록마다 1~2회(2단계는 추론 켬)»를 고지한다.

**2차 리뷰(수정분 재검토) 뒤 더 고친 것.** ㉧ 지문은 L2 파일 전체가 아니라 «블록 id + 앵커
글자»의 sha1이다 — 회전·좌표·다른 블록이 바뀌어도 앵커가 그대로면 초안은 유효하다. 지문이
없는 초안(이전 형식)은 **낡은 것으로 본다**(실서고에 0건이라 잃을 것이 없고, 보수적인 쪽이
안전하다). ㉨ 낡은 초안은 검토 목록에서 감추지 않고 `stale`로 올린다(깨짐과 같은 잣대).
㉩ 충돌 블록(적용 뒤 답이 바뀐 것)에 「적용」을 누르면 L4의 옛 적용문(`applied_text`)을
새 답으로 바꾼다 — 자리를 못 찾으면 «전에 적용했다»는 기록만으로 성공이라 하지 않고
`not_found`. ㉪ 전체 2단계 실패(`stage2_error`)는 초안 파일에 남고 일괄 경고에도 오르며,
블록 단위로 2단계가 한 번 터진 블록은 다시 올리지 않는다(사람에게 둔다). ㉫ `eval_cer`도
낡은 초안은 재지 않는다. ㉬ 화면: 경고 머리글은 한 줄 제한 없이 보이고, 불확실·판독불가
글자는 공백을 보존한 채 글자 단위로 붉게 표시하며, 편집기에 저장 안 한 수정이 있으면
「적용」을 막는다(적용 뒤 저장이 교정을 지우기 때문).

**동시성 (2026-09-19).** 초안 파일을 «읽고 → 고치고 → 쓰는» 곳(`run_correction`의 병합 저장·
`apply_draft`·`mark_applied`·`discard_draft`·일괄의 L4 채우기)을 쪽 단위 `RLock`(`draft_lock`)으로
묶었다. `run_correction`은 LLM을 기다린 뒤 잠금 안에서 파일을 **다시 읽어** 병합한다(D-127과
같은 규칙) — 정밀 판독이 도는 동안 사람이 「적용」을 눌러도 `applied_blocks`가 남는다. 일괄의
L4 채우기와 적용 라우트는 executor 스레드에서 잠금을 잡는다(이벤트 루프 스레드에서 RLock을
잡으면 같은 스레드의 다른 코루틴을 막지 못하고, 기다리면 서버가 멈춘다). 한 프로세스 안의
잠금이다 — CLI는 교정 패스를 부르지 않으므로 프로세스 간 잠금은 두지 않았다. 둘이 각각 막는
것이 다르다: «LLM을 기다리는 동안 들어온 적용»은 저장 직전의 다시 읽기가 막고, «두 적용이
같은 순간에 읽고 쓰는» 창은 잠금이 막는다 — 잠금을 빼고 두 블록을 여섯 스레드로 동시에
적용하면 20회 중 20회 `applied_blocks`가 유실됐고, 잠금을 두면 0회였다(2026-09-19 실측,
`TestConcurrency`가 두 시나리오를 고정한다).
...
3. **잘림 감지 → 사고 끈 재시도.** Ollama `done_reason=="length"`, Anthropic
   ...
   그래도 비면 실패로 남긴다. **OCR 경로에는 thinking 필드를 본문으로 쓰는 폴백을
   두지 않는다**(D-074 유지). Anthropic은 `type=="text"` 블록만 모아 텍스트로 삼는다.

   > **정정(2026-09-22).** 이 문단이 한때 「어떤 경우에도 두지 않는다」로 적혀 있었고
   > `CLAUDE.md`도 「어디에도 없다」로 받아 적었다. **사실이 아니다** — Ollama
   > 프로바이더에는 폴백이 셋 있다(`providers/ollama.py` 텍스트·스트리밍·비전).
   > `response`가 비고 `thinking`만 찼을 때 그것을 본문으로 쓴다. 사고에 `num_predict`를
   > 다 쓴 reasoning 모델의 방어책이고, 비전 경로는 `allow_thinking_fallback=False`로
   > 끌 수 있다(OCR이 그렇게 부른다). 다른 세 프로바이더에는 없다.
   >
   > 막으려던 것은 **OCR이 사고문을 판독 결과로 저장하는 것**이었는데, 문장이 전역
   > 금지처럼 적혀 그 문서를 믿은 호출자가 사고문을 결과로 받게 됐다. 범위를 적지 않은
   > 금지는 지킬 수 없는 약속이 된다(Codex 교차검증 지적).
...
### 덧붙임(2026-09-11) — 역할(묶음/기사/조각)은 화면에서 물러났다

사용자: 「어차피 깊이를 숫자로 재니까 묶음/기사/조각 이 용어 없애도 되지 않을까」. 코드를 보니 맞았다 — 단위의
범위·끝·트리 모양은 전부 깊이가 정하고(같은 깊이 이상의 다음 경계가 끝), 역할이 동작을 바꾸는 곳은 「단위
손보기」의 «아래 단위를 품음» 경고 하나뿐이었다. 나머지(③의 「기사 2단」, 사이드바 역할 단추, 편집기의 «#12
기사 · 제목», 쪼개기의 «조각»)는 이름표였다. 위 «사용자 결정 1»(깊이와 역할을 뗀다)은 «3단에 기사가 오는 책»을
위한 것이었지만, 그 뒤 그 뜻을 쓰는 기능이 생기지 않았으므로 사용자가 배워야 할 개념 하나만 남아 있었다.

- 화면: ③은 「2단」만, 행 옆 도구는 깊이만, 사이드바 역할 단추와 「경계 넣기」의 역할 칸 제거, 편집기 목록은
  «#12 · 제목», 「단위 손보기」·쪼개기 창은 «단위 안을 한 단 깊게 나눕니다», 「지금 경계」 요약은 «1단 1 · 2단 9».
  행의 모양(1단 굵게·3단↑ 흐리게·머리표)은 깊이로만 정한다.
- 파일: `role` 칸은 지우지 않는다(스키마·옛 서고 호환). 화면이 값을 보내지 않으므로 서버가 깊이로 어림해
  채운다(`role_for_level`). 화면은 저장된 role을 읽지 않는다 — 옛 서고에서 사람이 정해 둔 역할이 있어도 깊이로만
  보인다. `PUT …/boundaries/{id}`의 role 변경 API는 남아 있되 화면에서 부르지 않는다.
- 잃는 것: 3단에 «글»이 오는 책에서 그것이 문단이 아님을 화면이 말해 주지 못한다. 시스템이 둘을 다르게 다루는
  데가 없으므로 기능 손실은 없다. 다시 필요해지면 role 칸이 그대로 있다.
...
### 덧붙임(2026-09-10) — 서버 재시작은 도는 배치를 죽이고, 화면은 그것을 «끝남»처럼 보였다

`start_server.bat`은 `--reload`라 `src/`의 파이썬 파일이 바뀌면 서버가 스스로 다시 뜬다(2026-09-03).
그 재시작이 **도는 중인 권 전체 OCR을 끊는다** — 실측: honkoku 배치가 1~16쪽을 2~4분씩 읽고 17쪽의
레이아웃까지 만든 14:33:56 직후, 14:36:13의 코드 편집으로 서버가 다시 떠 배치가 사라졌다. 화면은
스트림이 «complete» 없이 닫히면 진행 막대만 지우고 아무 말도 하지 않아, 사용자는 «한 바퀴 다 돌았는데
16쪽뿐»으로 보고 다른 엔진으로 다시 돌렸다. 고침: 교감·추출 두 배치 화면이 «완료» 없이 끊기면
«N/M쪽(p쪽)까지 보고 멈췄습니다 — 서버가 다시 떴을 수 있습니다. 같은 엔진으로 다시 누르면 이어서
돕니다»라고 경고한다. 재시작 자체를 막지는 않는다 — 개발 PC 밖에서는 `src/`가 바뀌는 일이 없다.
개발 중에는 실서고 `L2_ocr`에 몇 분 안에 쓰인 파일이 있으면 `src/` 편집을 미룬다(메모리에 남김).
...
### 보정 2 (2026-09-08, Codex 2회차 — 층계 바깥의 층)

1회차가 발견기 안쪽을 봤다면 이번은 그 바깥 — 라우터·표본·판심·화면 문구다. Codex는 또 크레딧이 끊겼고
(140,812 토큰), 남긴 실패 테스트 여덟을 코드와 대조해 전부 받았다. 네 책 재현은 그대로다.

- **판심 판정이 층을 섞으면 틀렸다.** 「쪽 안 같은 자리」를 원본 행 번호로 견줬는데, L4는 빈 행을 세고 L2는
  세지 않는다. 확정본이 일부 쪽에만 있는 문헌(浩齋 93쪽본)에서 같은 물리 자리가 다른 번호가 되어 판심이
  안 걸렸다. 이제 «그 쪽에서 비어 있지 않은 몇 번째 행인가»로 견준다(`_page_ordinals`). 원본 번호는 좌표라
  고치지 않는다.
- **사람이 적은 목차 쪽은 항목을 못 읽어도 목차다.** 전에는 항목 추출이 실패하면 목차가 없던 일이 되어 그
  쪽의 짧은 행을 본문 규약으로 셌다. 1단의 뜻(목차를 배우지 않는다)에 어긋난다.
- **목차 대조는 한곳에서.** 라우터의 `_toc_signal_for`가 코어 `toc_signal`을 통째로 베껴 두고 있었다(대조·
  본문 제외·decisive 판정). 코어에 `toc_pages` 인자를 주고 라우터는 요청 필드만 옮긴다.
- **모델 오답이 500이 됐다.** `{"patterns": 1}`처럼 목록이 아닌 답에 `[:8]`을 걸어 TypeError가 그대로 올라갔다.
  이제 «patterns가 목록이 아닙니다»로 알린다. 문자열 답은 조용히 «못 찾음»이 되던 것도 오류로 바꿨다.
- **규칙 저장 실패도 500이었다.** `save_segmentation_rules`의 docstring은 «ValidationError는 호출한 쪽이
  400으로 바꾼다»고 적어 두었는데 자동 트리만 그러지 않았다. 어느 필드가 스키마와 어긋났는지 짚어 400으로 준다.
- **«앞뒤 한 행» 표본은 JSON 한 줄로.** 「앞 ／ ▶후보 ／ 뒤」는 원문에 같은 글자가 있으면 경계가 모호하다.
  `{"before","candidate","after"}`로 보낸다 — 글자 수가 약 1.5배가 되지만 이 범위는 기본이 아니고, 보내기 전에
  «몇 줄·몇 자»를 화면이 먼저 보인다(D-117 4단).
- **목차로 결정한 책에 «표지를 찾지 못했다»고 적었다.** 1단은 저장할 텍스트 규칙이 없는 것이지 실패가 아니다.

내가 함께 고친 것(Codex 지적 밖):
- 모듈 머리말이 아직 D-117의 «신호 종류» 표를 싣고 있었다 — 이 결정과 정면으로 어긋나는 설명이 코드 안에 남아
  있었다. 발견기와 판정으로 다시 썼다.
- 2단 문턱이 이름 없는 `0.4`였다(3단은 `_STAGE3_KEEP`). `_STAGE2_KEEP`으로 이름과 근거를 준다 — 기호의 이체
  (○ 217·◯ 158)를 함께 켜야 해서 3단보다 너그럽다.
- 요약 줄의 날짜 꼴 순서가 **실행마다 달랐다**(`sorted(set(...), key=-len)`은 길이가 같으면 집합 순서를 따른다).
  «긴 것 먼저, 같으면 사전순»으로 못 박았다.

### 보정 3 (2026-09-08, Codex 3회차 — 쓰는 길·목차)

앞 두 번이 «찾는» 쪽을 봤다면 이번은 «쓰는» 쪽이다 — 경계는 원본 저장소의 정본이라 여기서 잃으면
커밋을 되돌리는 수밖에 없다. Codex는 네 번째로 크레딧이 끊겼고(155,832 토큰), 남긴 실패 시험 여섯을
대조해 전부 받았다. 소스만 되돌려 여섯이 실제로 실패함을 확인했다.

- **앵커를 다시 찾을 때 행갈음을 넘지 못했다.** `anchor_text`는 개행을 빼고 저장하는데(`anchor_text_at`)
  `rematch`는 개행이 든 원문에서 그대로 찾았다. 그래서 확정본을 고쳐 행갈음이 달라지면 앵커가 «없다»가
  되어 경계가 통째로 `stale`이 됐다 — 다시 찾으라고 만든 기능이 정작 행이 바뀌었을 때 듣지 않았다.
  개행을 뺀 문자열에서 찾고 원문 오프셋 표로 자리를 되돌린다.
- **경계를 옮길 때는 중복 검사가 없었다.** `insert_boundary`는 «같은 자리·같은 층위»를 막지만
  (D-092, 운양집 77 → 148의 원인) `move_boundary`에는 그 검사가 없어, ▲▼로 다른 경계 위에 올려놓으면
  같은 자리에 경계가 둘 생겼다. 이제 거절하고 400으로 «다른 행으로 옮기거나 그 경계를 먼저 지우세요».
- **적용이 반쯤 되면 있던 경계만 잃었다.** `/segmentation/apply`는 바꿔치기로 **먼저 지우고** 넣는다.
  구간 하나를 확정본에서 못 찾으면 지우기는 다 되고 넣기는 덜 된 채로 저장됐다. 이제 하나라도 못 찾으면
  아무것도 쓰지 않고 400으로 되돌린다(제안한 뒤에 확정본이 바뀌면 행 번호가 어긋난다는 안내와 함께).
- **해제가 개행 없이 붙으면 통째로 버려졌다.** `reference_excerpt`는 문단 단위로 고르는데, 웹에서 복사한
  해제는 문단이 하나라 그 하나가 예산을 넘어 전부 탈락했다 — 권별 서술이 뒤에 있는 해제에서 정작 필요한
  데가 사라졌다. 예산을 넘는 문단만 문장으로 쪼갠다.
- **행이 갈린 표제를 목차가 못 맞췄다.** 두 행을 이었을 때 표제와 **정확히** 같으면 첫 행을 경계로 삼고
  다음 항목은 둘째 행 뒤부터 찾는다. 대조 점수도 다시 재지 않고 후보의 점수를 쓴다.
- 「목차 항목 구조화」도 저장 전 해제를 쓴다(「시작 표지 묻기」와 같은 계약) — 모달의 «해제 N자를 함께
  보냅니다»가 세 선택지 모두에서 참이 된다.

내가 함께 고친 것:
- **「전부 적용해 새로 세우기」가 묻지 않고 다 지웠다.** 이 단추는 `replace="all"` — 이 권의 살아 있는
  경계를 손으로 넣은 것까지 지운다. 사이드바 「자동 트리」는 «단위 N개를 지웁니다. 계속할까요?»를 묻는데
  여기만 묻지 않았다. 같은 확인을 붙이고, 손으로 넣은 것도 지워진다는 말을 더했다.

### 雲養集 목차 9/102 — 알고리즘이 아니라 자료다 (2026-09-08 실측, D-089 후속)

D-119 표의 «목차 9/102 약함»을 직접 재 보았다. **정렬을 빼고 순서를 무시해도 천장이 21이다.**

| 유사도 문턱 | 본문 어딘가에 그만큼 닮은 행이 있는 항목 |
|---|---|
| 1.00 | 6 / 102 |
| 0.80 이상 | 12 / 102 |
| 0.60 이상 | 21 / 102 |

세 가지가 겹쳤다. ① **목차는 集 전체의 것이고 이 책은 1책뿐이다** — 「第三卷」「第十一卷」「第十五卷」
「第十六卷」의 본문이 여기 없다. ② **양쪽 OCR이 같은 제목을 다르게 읽었다** — 목차 「撃努集」 ↔ 본문
「撃蒼集」, 「松屋維詠」 ↔ 「松屋雑詠」, 「従斎集」 ↔ 「健斎集」. ③ 같은 제목이 목차에 두 번 나오는데
(「昇平館集」·「續昇平館集」) 본문 행은 하나뿐이라 순서를 지키는 정렬이 하나만 놓을 수 있다.

즉 대조기는 «있는 것» 12개 중 9개를 찾았다. 문턱을 낮추면 몇 개를 더 얻지만 ②의 OCR 오독과 ①의 없는
권을 아무 데나 붙일 위험이 같이 커진다. 정답 색인이 없으므로 **고치지 않고 한계로 적어 둔다** — 이 책에서
목차가 규약이 되지 못하는 것은 옳은 판단이고, 층계는 그래서 3단으로 내려간다.

## D-120: 판식(版式)은 글자가 아니라 좌표에서 읽는다 — 판심·두주는 종이의 규약이다

**날짜:** 2026-09-08
**상태:** 확정
**관련:** D-116(규약 도출)·D-117(층계)·D-119(발견기), KORMARC 300▼b

### 무엇이 있었나

발견기(D-119)는 «되풀이되는 문자열»만 본다. 그래서 종이의 규약에 두 번 걸렸다.

- **판심제를 못 잡는다.** 쪽마다 장차가 붙어 「浩齋辰巳日錄二」「…三」으로 달라지니 되풀이가 아니다.
  浩齋辰巳日錄 두 판 모두 판심 목록(`furniture`)이 **비어 있었다**.
- **두주를 표제로 센다.** 난외의 작은 주 「同日」「四月十三日」이 행 첫머리 어휘 후보로 올라왔다.
- 게다가 `furniture`는 **글자가 정확히 같아야** 걸린다. 天津談草의 판권 한 줄이 OCR에 따라
  「京城鍾路…」「京城縫路…」 등 여섯으로 갈려 여섯 항목으로 등록됐고, 일곱 번째 오독은 통과한다.

그런데 그 셋은 **자리와 크기로 한눈에 다르다.** 실측(2026-09-08, 浩齋65의 L2 좌표):
본문은 키가 같은 열이 139픽셀 간격으로 서고, 판심 자리만 678~1194픽셀(4.3배)이 벌어지며,
두주는 높이 47~100으로 본문 열(1730)의 **0.03~0.05배**다.

### 결정

`src/core/page_format.py`가 좌표에서 판식을 읽어 행마다 라벨(`body`·`pansim`·`margin`)을 붙인다.
발견기는 `body`만 세고, 화면은 무엇을 뺐는지 한 줄로 말한다. **행을 지우지는 않는다.**

1. **판심 = 열 사이가 유난히 벌어진 자리.** 본문 열의 중심을 늘어놓고 간격의 중앙값을 잰 뒤,
   그 2.5배를 넘는 자리를 접은 자리로 본다. 반엽 하나씩 찍은 스캔은 벌어진 데가 없어 None이 된다.
2. **«판식이 일정한 책»에서만 판정한다.** 판심이 쪽의 절반 이상에서 잡히고, 좌우 열 수의 으뜸 값이
   그 쪽들의 4분의 1 이상이어야 한다. 통과하지 못하면 **아무 라벨도 붙이지 않는다.**
   자신 없을 때 잠자코 있는 쪽이, 본문을 두주로 잘못 빼는 것보다 낫다.
3. **두주는 «아주 작고, 본문처럼 윗변에서 시작하지도 않는» 행.** 크기 문턱은 본문 열의 0.25배다 —
   0.6배로 자르면 D-117의 내려쓴 별행 표제(시집)와 天津談草의 「天津奉使縁起」가 함께 빠진다.
   실제로 첫 판이 그렇게 만들어져 天津談草 본문의 22%, 雲養集의 48%를 난외로 몰아냈다.
4. **코드에 책마다의 값은 없다.** 판심이 어디인지·열이 몇인지·본문 열의 키가 얼마인지는 전부 그
   책의 좌표에서 잰다. 문턱은 모두 «그 책에서 잰 값에 대한 비율»이다(D-119와 같은 원칙).
5. 좌표가 없는 행은 `unknown`이고 본문처럼 다룬다 — 텍스트로 가져온 문헌에서 아무것도 망치지 않는다.

### 실측 (네 책 눈가림)

| 책 | 판식이 일정한가 | 잰 행자수 | 뺀 행 | 제안 |
|---|---|---|---|---|
| 浩齋辰巳日錄 65쪽본 | **예** (61/64쪽에서 판심) | **반엽 10행** | 판심 자리 24 · 두주 83 | 615·583 (그대로) |
| 浩齋辰巳日錄 93쪽본 | **예** (73/77쪽) | **반엽 10행** | 판심 자리 9 · 두주 66 | L4가 1쪽뿐 |
| 天津談草 | 아니오 | — | 0 | 재현 36/36 · 정밀 36/42 (그대로) |
| 雲養集 1책 | 아니오 | — | 0 | 305·183 (그대로) |

**국립중앙도서관 KORMARC(KOL000019624)의 300▼b가 「四周雙邊 半郭 19.1 x 14.6 cm, 10行20字
註雙行, 上2葉花紋魚尾」다.** 좌표에서 잰 «반엽 10행»이 목록과 같다. 목록을 보지 않고 잰 값이므로
서로가 서로의 검산이 된다.

### 한계·다음

- 판심 띠 안에 든 행을 `pansim`이라 부르지만, 그중 일부는 판심제가 아니라 접은 자리 근처의 조각이다.
  라벨은 «판심 자리»라고만 말한다.
- 제안기(`propose_boundaries`)는 아직 이 라벨을 보지 않는다. 지금은 **규약을 찾을 때만** 뺀다 —
  그래서 네 책의 제안 수가 그대로다. 제안에도 반영할지는 실사용을 보고 정한다.
- `printing_info`(서지의 판식 칸)와 맞대어 검증하는 것은 다음 일이다. 지금 서고의 네 책은 그 칸이
  비어 있고, 파서도 국립중앙도서관 표기의 「註雙行」(注가 아니라 註)·「上2葉花紋魚尾」를 못 읽는다.

### ②·③ 목록과 맞대기 (2026-09-08)

D-120 ①은 좌표만 본다. Codex 교차검증(4회차)이 그 전제를 정확히 짚었다 — **«되풀이되는 넓은 틈»만으로
접은 자리를 확정할 수는 없다.** 반엽 하나씩 찍은 스캔에 빈 열이 규칙적으로 있으면 좌표가 같아 보인다.
그 반론을 코드로 없애는 대신 **독립된 근거를 들여왔다.** 목록(KORMARC 300▼b)의 행자수는 좌표와 무관하게
사람이 적은 값이라, 확인도 반증도 된다.

- **파서를 국립중앙도서관 표기까지 넓혔다.** 어미를 표로 나열하던 것을 조각의 조합(자리+향+잎 수+무늬)으로
  읽는다 — 나열표는 조합 하나가 빠질 때마다 못 읽고, 실제로 KOL000019624의 「上2葉花紋魚尾」가 통째로
  빠졌다. 「註雙行」(注가 아니라 註)도 읽는다. 기존 표의 출력은 그대로 재현한다.
- **서지 화면에서 판식을 붙여 넣는다.** 목록의 형태사항 한 줄을 그대로 넣으면 저장할 때 서버가 갈라
  담는다(파서가 한곳이라 화면과 어긋나지 않는다). 목록 레코드가 없는 필사본도 사람이 적어 같은 이득을 본다.
- **저장이 판식을 지운다.** 서지 저장 요청 모델에 `printing_info`·`publishing`·`extent`가 없어서, 화면에서
  제목만 고쳐도 그 셋이 사라졌다(`save_bibliography`는 받은 dict를 통째로 쓴다). 세 칸을 더하고, 보낸 칸만
  덮어쓰도록(`exclude_unset`) 고쳤다 — 화면이 모르는 칸은 지우지 않는다.
- **맞대기.** `compare_with_catalog`가 목록의 행 수와 좌표에서 잰 좌우 열 수를 견주어 한 줄로 답한다.
  어긋나도 고치지 않는다 — 어느 쪽이 틀렸는지는 사람이 판단할 일이고, 어긋났다는 사실 자체가 «이 문헌의
  OCR이 열을 놓쳤거나 붙였다»는 신호다. 편성 탭이 그 줄을 보인다.

실측(浩齋辰巳日錄 65쪽본, KOL000019624):

```
목록에서 갈라 담은 것 : 사주쌍변 · 19.1 × 14.6 cm · 반엽 10행 20자 · 주쌍행 · 상이엽화문어미
좌표에서 잰 것        : (10, 10)
맞댄 결과             : 같습니다
```

목록을 보지 않고 잰 값과 목록이 같다. 둘은 서로 독립이므로 이것이 검산이다.

### 판식 모듈 교차검증 (2026-09-08, Codex 4회차 — 크레딧 안에서 끝남)

일곱 건을 실패 시험 → 수정 순으로 받았다. 좌표가 뒤집히거나 넓이가 0인 행은 두주가 아니라 `unknown`으로
두고, 가로쓰기의 축·시작 변을 바로잡고, 길이·윗변 기준을 «긴 본문 후보»에서만 재고(두주가 많으면 기준이
오염된다), 같은 넓이의 틈이 둘이면 판심을 유보하고, 중복 좌표를 지우고 열을 세고, 판심 편차를 쪽 폭의
중앙값으로 잰다(좌표를 평행이동만 해도 판정이 뒤집혔다). 네 책 재현은 그대로다.

## D-121: 규칙 변경은 «재서 보이고 승인받는» 한 관문을 지난다 — 사람·통계·LLM이 같은 문으로

**날짜:** 2026-09-09
**상태:** 확정 (첫 단계만 구현)
**관련:** D-116~D-119(규약 도출), D-117 §4(모델의 말은 세어 확인한다)

### 무엇을 하려는 것인가

사용자가 여러 차례에 걸쳐 그린 그림은 이렇다. 편성은 책마다 사정이 너무 달라 체크박스만으로는
모자라니, **말을 주고받으며 단계적으로** 좁혀 가고 싶다. 그 대화는 두 방향이다.

- 시스템 → 사람: 「권점 기호 두 가지는 같은 것이니 합칩시다」
- 사람 → 시스템: 「談草로 끝나는 게 표제야」 — 연구자가 이미 아는 것을 말한다

두 방향은 결국 같은 것에 모인다. **`segmentation_rules`에 대한 변경 하나**다. 그래서 고리는 하나고
입구가 둘이다. 그리고 어느 입구로 들어오든 **같은 검산**을 받아야 한다 — 이 프로젝트는 이미 LLM의 말을
전문에서 세어 되풀이되지 않으면 버린다(D-117 `verify_pattern`). 사람의 말도 같은 예의로 대한다.

### 결정

`src/core/rule_preview.py`의 `preview_rule_change()`가 그 관문이다. 규칙 둘을 받아 제안을 각각 세고
견준다. **아무것도 저장하지 않는다.**

    후보 615 → 608 · 채택 583 → 575 · 빠지는 자리 8
    바꾸는 것: symbols −◯
    빠지는 자리 8: 12쪽 二 · 14쪽 十 · 17쪽 二十 …
    [이대로 적용]

말할 때의 규칙 둘:

1. **뜻 없는 차이는 말하지 않는다.** 목록은 «늘고 준 것»으로만 말한다 — 순서만 바뀐 것은 변경이 아니다.
2. **스위치는 «적힌 값»이 아니라 «듣는 값»으로 견준다.** 안 적힌 스위치는 켜진 것이므로,
   그대로 보이면 「None → False」처럼 사람이 못 알아볼 말이 된다.

### 단계

이번에 놓은 것은 **배관과 첫 입구**다.

- 배관: `preview_rule_change` + `POST /segmentation/preview` + 편성 탭의 「바꾸면?」(D-122에서 단추는 없앴다 — ③ 머리의 «바뀐 것»이 같은 관문을 자리로 견준다)
- 첫 입구: 화면의 체크 상태(사람이 손으로 고른 것)

남은 입구는 순서대로 이렇다. 셋 다 이 관문을 그대로 쓴다.

1. **사람이 말로**: LLM은 말을 규칙 변경으로 옮기기만 한다. 옮기지 못한 것은 **그대로 말해야 한다** —
   「권2부터는 卷頭로」(범위)·「날짜 앞에 ○가 있을 때만」(조건)은 지금 규칙으로 표현할 수 없다.
   조용히 비슷한 것으로 바꿔치기하면, 사람은 말했다고 여기는데 시스템은 다른 일을 한다.
2. **재서 아는 제안**: 기호 변이형 합치기 같은 것. 浩齋 실측에서 ○(217)과 ◯(158)을 합치면 간격의
   고름이 0.92·0.87 → 0.96으로 오른다 — LLM 없이 코드가 말할 수 있다.
3. **읽어야 아는 제안**: 「이 책은 일기라 날짜가 기사 경계다」·「「也」는 종결어미지 표제가 아니다」.
   여기서 LLM이 제 몫을 한다.

### 왜 «채팅으로 직접 고치기»가 아닌가

제안은 안전하고 실행은 위험하다. 모델이 경계를 직접 만들거나 지우면 D-117의 확인 관문과 D-092의
정본 계약이 함께 무너진다. 반면 **제안 → 코드가 셈 → 사람이 승인**은 지금 구조 그대로다. 채팅 화면은
이 관문을 감싸는 껍데기이지 그것을 대신하는 것이 아니다.

### 곁들여 고친 것

- 전역 이름 충돌: 이 저장소의 JS는 모듈이 아니라 전역 스크립트라, 파일이 달라도 이름이 겹치면 나중에
  읽힌 쪽이 이긴다. `_renderPreview`가 `punctuation-editor.js`의 같은 이름에 가려져 **아무 일도 일어나지
  않았다.** 새 전역 함수는 도메인을 붙여 짓는다(`_renderRulePreview`).

### 두 번째 입구 — 사람이 말로 (2026-09-09)

`src/core/rule_talk.py`가 연구자의 문장을 규칙 변경으로 옮긴다. 저장하지 않고 경계도 만들지 않는다 —
옮긴 결과는 `preview_rule_change`가 재고 사람이 승인한다(같은 관문).

- **모델은 정해진 칸 이름과 연산(add·remove·set)만 쓴다.** 아는 칸이 아니면 코드가 되돌린다.
- **옮기지 못한 말은 반드시 돌려준다.** 지금 규칙에는 범위도 조건도 없어 「권2부터는 卷頭로」·
  「날짜 앞에 ○가 있을 때만」을 표현할 수 없다. 비슷한 칸으로 조용히 바꿔치기하면 사람은 말했다고
  여기는데 시스템은 다른 일을 한다 — 이 대화에서 되풀이해 확인된 위험이다.
- **사람의 말도 세어 본다.** 어휘를 더하라는 말은 전문에서 몇 번 나오는지 함께 보인다. 0번이면 이
  책의 말이 아니다(오타이거나 다른 책의 기억). 버리지는 않고 사람이 보고 정한다.
- 화면은 「LLM에 묻기」 창의 넷째 선택지다 — **모델을 부르는 단추는 여전히 하나뿐**이다(D-117 §3). (D-122에서 ①의 칸으로 꺼냈다 — 흐름의 첫 자리이기 때문이다.)

### 화면 문구 걷어내기 (2026-09-09, 사용자 지적)

«이것저것 요소가 너무 많다». 실제로 재 보니 편성 패널 347자, 「LLM에 묻기」 창 663자가 설명 문단으로
차 있었다. **보이는 글자는 «무엇»만, «왜»는 툴팁으로** 옮겼다 — 편성 패널 145자, 창 200자.

화면에 찍히는 문장도 같이 줄였다. 「뺀 것」 줄이 네 마디를 이어 붙여 200자가 넘었는데, 수만 말하고
(「뺀 것 — 판심 문구 2가지 · 날짜뿐 1가지 · 판심 24행 · 두주 83행 · 반엽 10행 · 목록과 일치(10행)」)
어떤 행을 뺐는지는 툴팁이 보인다. `page_format.describe()`와 `compare_with_catalog()`의 문장도
짧게 고쳤다.

---

## D-122: 편성 탭은 ①②③ 한 흐름이다 — 단추 넷, 저장 자리 하나

**날짜:** 2026-09-09
**상태:** 확정
**관련:** D-088(경계 제안), D-116·D-117(규약 도출·답 한 줄), D-121(규칙 변경의 관문), D-092(경계가 정본)

### 왜

사용자가 새 화면을 직접 써 보고 한 말: 「되긴 되는데 처음 보는 사람한테는 어떻게 써야 할지 감이 안 올 것
같아. … 지금은 “그래서 이제 어딜 눌러야 되는 거지??”라는 느낌을 더 많이 준다.」 그리고 바라는 절차를
그대로 적었다 — ① 아는 것을 말로 넣는다(목차·제목이 없으면 없다고 먼저 전제하고 경계 표지가 무엇인지)
② 뽑힌 규칙 중 원하는 것만 체크 ③ 목록을 전체나 Shift 범위로 골라 **한 번에** 계층 조정(몇백 개를
하나씩은 비효율).

지난 화면(D-117 §3)은 «할 수 있는 것»을 다 늘어놓았다. 「경계 제안」을 눌러야 패널이 열리고, 머리에 단추
셋(닫기·골라 적용·전부 적용해 새로 세우기), 답 줄에 「LLM에 묻기」, 접힌 «자세히 · 고치기» 안에 「바꾸면?」
「후보 보기」, 후보 행마다 역할 select·깊이 input·「억제」. 저장 시점도 셋으로 흩어져 있었다(「후보 보기」가
규칙 저장, 「골라 적용」이 경계, 「전부 적용」이 둘 다 + 서버 auto). 각각은 이유가 있었지만 합치면 «지금
무엇을 해야 하는가»를 말하지 않는 화면이었다.

### 결정

한 패널에 번호 붙은 세 단계가 위에서 아래로 항상 보인다. **단추는 넷**: ①「말로 넣기」·「더 묻기…」,
②「후보 보기」, ③「적용」.

1. **① 이 책은 어떻게 나뉘나** — 탭을 열면 스스로 센다(`_startFlow`, 규칙만). 「찾은 규약: …」 한 줄
   아래에 «말로» 칸이 있다(D-121 두 번째 입구를 창의 넷째 선택지에서 여기로 꺼냈다 — 흐름의 첫 자리다).
   자리 표시자가 사용자가 말한 쓰는 법을 그대로 보인다: «목차와 제목이 없다. 기사는 ○ 뒤의 날짜로 시작한다».
   옮긴 것은 ②의 체크·손으로 넣은 행·참고 칸에 **바로** 들어가고(`_checkFromRules` — 처음 읽기와 같은 함수)
   ③이 다시 선다. 옮기지 못한 말은 그 아래 붉게 그대로. L4가 없으면 여기서 붉게 말한다.
2. **② 규칙 고르기** — 옛 «자세히 · 고치기»를 접기에서 꺼내 항상 펼친다. 「후보 보기」는 **저장하지 않는다**
   (propose 라우트는 규칙을 받기만 한다). 목차 줄의 체크가 곧 규칙의 `signals.toc`다(새 스위치 — 「목차가
   없다」는 전제를 규칙에 적을 자리. `_SIGNAL_KEYS`·스키마·rule_talk·propose·auto·preview가 다 같은 스위치를
   본다).
3. **③ 후보 고르기** — **체크(경계가 될 것)와 행 선택(한꺼번에 고칠 것)은 다른 상태다**(Codex 지적: 583개를
   체크해 둔 채 그중 12개만 골라 깊이를 바꿔야 한다). 행 클릭 = 선택, Shift = 보이는 순서로 범위, Ctrl = 하나씩.
   위 줄(붙박이)에서 역할·깊이·체크·해제·억제를 고른 행 전부에. 행의 역할·깊이는 글자로만(「기사 2단」).
   모든 상태는 index가 아니라 **자리 키**(`쪽:행:글자`)로 들어 규칙을 고쳐 다시 세어도 남는다.
4. **«바뀐 것»** — 저장된 규칙(없으면 권고)으로 센 채택 후보(`baseline`)와 지금 후보를 자리로 견주어
   «새로 잡히는 자리 N · 빠지는 자리 M»과 그 자리들을 ③ 머리에 보인다. 「후보 615 → 608」 같은 수만으로는
   자리가 바뀐 것을 못 잡는다(Codex 반대 의견을 받아들였다). 「바꾸면?」 단추는 없앴다 — 같은 관문이다.
5. **「적용」 하나가 저장한다** — 규칙과 경계를 **한 요청**으로(`/segmentation/apply`에 `rules`). 경계를 쓴
   뒤에 규칙을 저장하므로 경계가 실패했는데 규칙만 남는 일이 없다(두 요청이면 그렇게 된다 — Codex 지적).
   지울 수는 `dry_run`으로 먼저 세어 **실제 수**로 묻는다(«경계 584개를 세우고, 지금 경계 중 2개를 지웁니다»).
   같은 자리·깊이의 경계는 끼워 넣기가 옛것을 돌려주므로 역할만 바뀐 것은 옮겨 적는다(`role_changed`).
6. **낡은 후보는 적용하지 못한다** — ②의 칸이 후보를 셀 때와 다르면(규칙 JSON 견줌) 「적용」이 잠기고
   «규칙이 바뀌었습니다 — 「후보 보기」로 다시 세우세요»가 뜬다. 사람이 규칙을 고쳐 놓고 옛 후보를 적용하면
   «말한 것과 다른 일»이 된다.

없앤 것: 「경계 제안」 단추, 「닫기」, 「골라 적용」·「전부 적용해 새로 세우기」(→「적용」), 「바꾸면?」,
범례 문장, 「목차 항목만 ↔ 전부」 토글(→「전체」 체크박스), 행마다의 역할·깊이·억제, «자세히 · 고치기»
접기, 창의 넷째 선택지. «지금 경계»는 접이식 한 줄(수만).

### 잰 것

복사본 서고(浩齋 65) headless: ① 자동 셈 → ③ 583행. 행 2~6 Shift 범위 → 「선택 5」 → 깊이 3 「바꾸기」 →
다섯 행이 3단. 신호 하나 끄기 → 「적용」 잠김 → 「후보 보기」 → 후보 604 · «빠지는 자리 12»(자리 목록 포함),
깊이 3단은 그대로 남음. 「말로 넣기」 «목차와 제목이 없다. 기사는 ○ 뒤의 날짜로 시작한다» → «목차 끔 · 짧은
행 끔». 「적용」 → 확인창 «경계 584개를 세우고, 지금 경계 중 2개를 지웁니다» → 584개. 콘솔 오류 0, 4xx 0.

### 한계·남긴 것

- «규칙만 저장»은 없다 — 저장은 「적용」뿐. 사이드바 「자동 트리」는 저장된 규칙(없으면 다시 도출)으로
  서버가 세므로, ③에서 손본 체크·깊이는 재현하지 않는다. 손본 것을 남기려면 「적용」이다.
- 「전체」는 **보이는** 후보만(문턱 아래는 펼쳐야 든다). Shift 범위도 보이는 순서다.
- «바뀐 것»의 기준은 이 화면에서 처음 센 후보(저장된 규칙 또는 권고)이고 「적용」이 기준을 옮긴다 —
  서버 `preview_rule_change`는 API로 남아 있되 화면은 자리 키로 직접 견준다.
- 협주(註雙行)·D-121 ③④(재서 아는 제안·읽어야 아는 제안)는 그대로 남아 있다.

### 같은 날 고친 것 둘 (사용자 보고 2026-09-09)

- **교정 탭 「일괄」 «실행»이 적용되지 않았다.** `apply_batch_corrections`가 쪽마다
  `L4_text/corrections/*.json`에 항목만 적고 **확정본 본문(`L4_text/pages/*.txt`)은 그대로** 두었다.
  교정 탭이 보고 편집하는 것은 그 본문이라 화면에는 아무 변화가 없었고, 미리 보기는 그 기록으로
  «이미 고친 자리»를 빼서 두 번째부터는 찾지도 못했다. 이제 본문을 고치고(`save_page_text`) 기록은
  덧붙이며, 미리 보기는 본문만 본다(글자가 남아 있으면 안 고친 것). 실행 뒤 보고 있는 쪽의 편집기를
  다시 읽어 옛 글로 되돌려 저장하는 일을 막는다. `tests/test_alignment_page.py::TestBatchCorrectionWritesL4`.
- **90° 회전에서 가로·세로 맞춤이 안 맞고 왼쪽이 잘렸다.** 회전은 래퍼의 CSS transform인데 transform은
  배치를 바꾸지 않는다 — 래퍼는 여전히 w×h 자리를 차지한다. 보정 마진의 부호가 반대여서(세로 +, 가로 −)
  배치 상자가 그림보다 좁아지고, 가운데 정렬(`margin:auto`)이 그 상자를 기준으로 하니 그림이 왼쪽의
  스크롤로 닿지 않는 자리로 밀려났다. 맞춤도 돌리기 전 크기로 재고 있었다. 이제 마진은
  `-(h−w)/2 (h−w)/2`(세로 쪽·가로 쪽 모두 같은 식), 맞춤은 돌린 뒤 모양(`_rotatedSize`)으로 재며, 돌리면
  맞춤을 다시 잰다. 돌린 동안은 왼쪽 정렬이다. 회전은 **저장하지 않는다** — 화면의 것이다. 저장하려면
  권마다 각도를 manifest에 두고 OCR·좌표(L2 bbox·앵커)도 그 각도로 돌려야 하므로 따로 결정할 일이다.

### Codex 구현 리뷰(2026-09-09)에서 고친 것

설계 브리프에 이어 diff도 Codex(gpt-6-astra)에 맡겼다. Important 6·Minor 2, 전부 실측 근거가 있어 반영했다.
① 억제한 자리의 체크가 남아 화면만 꺼지고 적용은 되던 것 — 다시 셀 때와 일괄 체크에서 억제 자리를 뺀다.
② 요청 중 규칙을 고치거나 문헌을 바꾸면 옛 응답에 새 지문이 붙던 것 — 요청 세대·문헌·권을 고정하고
지문은 **보낸 입력**(규칙 + 목차 쪽)으로 만든다(`_formDigest`). ③ 「말로 넣기」가 바꾼 `min_confidence`·
`furniture`가 폼을 거치며 저장본 값으로 되돌아가던 것 — 초안 규칙(`signalState.draft`)을 바탕으로 쓴다.
④ 목차 쪽 칸을 고쳐도 캐시된 목차로 세던 것 — 칸이 바뀌면 다시 찾고 지문에도 든다. ⑤ 대조 0건인 책에서
preview는 목차 쪽을 본문에 넣고 propose는 빼던 것 — 둘 다 «항목이 있으면 뺀다». ⑥ `dry_run`이 실패해도
확인 없이 적용하던 것 — 세지 못하면 멈춘다. Minor: `indent_alone`만 켠 저장본을 화면이 권고로 덮던 것,
죽은 분기·옛 안내 문구. 남긴 것: 경계 저장과 규칙 저장은 한 요청이지만 **원자적이지는 않다**(경계 커밋 뒤
규칙 저장 실패는 경고로 알린다).

사용자 질문 «말로 물을 땐 LLM 뭘로 선택하는 거야?»(2026-09-09) — 모델 드롭다운이 「더 묻기…」 창 안에만 있어
「말로 넣기」만 쓰는 사람은 무엇이 도는지 볼 수 없었다. 두 단추가 공유하는 조건이므로 ①의 단추 줄로 옮겼다.

### v1.4.0 뒤 실측(2026-09-10) — «세는 중» 36초의 자리

릴리스 뒤 설치본에서 탭을 열고 ③이 뜨기까지 10~36초가 걸렸고, 헤드리스 검증은 90초 안에 행을 보지 못했다.
JS 긴 작업은 0이었고 전부 서버였다. 원인은 규칙이 아니라 **행 수집**: `collect_document_lines`가 권의
전체 쪽(196)을 돌며 쪽마다 `get_corrected_text` → manifest 4번 읽기 + `resolve()` 3번을 했다. 확정본은
20쪽뿐이었고, 편성 탭은 이 수집을 «내용 트리·경계 목록 둘·신호·목차·후보» 여섯 요청에서 되풀이했다.
고침: 확정본 파일이 없는 쪽은 열지 않는다(파일이 없으면 빈 글로 버려지므로 결과는 같다 — 행·후보·트리의
지문 동일 확인). 수집 2.5초 → 0.1초, 탭 클릭에서 후보까지 0.8초. 규칙 도출(`induce_signals`) 자체는
0.09초였다 — «느리면 알고리즘부터 의심»이 틀렸던 사례. 남긴 것: 쪽마다 manifest를 다시 읽는
`get_corrected_text`의 구조는 그대로다(확정본이 많은 권은 여전히 쪽 수에 비례한다).

---

### 덧붙임(2026-09-11) — 해제 칸을 ①로 합침 · 못 찾은 목차 항목 찾아 넣기 · ③의 말

사용자: 「해제 넣는 칸을 굳이 따로 두지 말까? "이 책은 어떻게 나뉘나"에 해제 등 자연어로 된 규칙을 모두 쓰라고
하는 게 나으려나」 — 그리고 「'체크'랑 '선택'이 별도인 거 이해하는 데 시간이 좀 걸렸어」, 「"목차에는 있으나
본문에서 못 찾음" 부분은 해당 부분 찾아서 경계 후보로 삽입할 수 있도록」.

1. **해제 칸은 없다 — ①의 칸 하나.** 해제(`reference_text`)는 세 LLM 경로에 «판단에만 쓸 것»으로 들어가는
   참고였고, 「말로 넣기」는 지시를 규칙으로 옮기는 입구였다. 둘을 다른 칸에 두는 유일한 이유는 «남의 서술과
   내 지시를 모델이 구분 못 한다»는 것인데, 그 위험은 이미 D-121의 관문이 막는다 — 어떤 변경도 ③의 «바뀐 것»을
   지나 「적용」을 눌러야 저장된다. 그래서 ①의 칸에 해제를 붙여 넣고 그 위나 아래에 «어떻게 나뉘는지»를 쓰게
   했다. 저장은 전처럼 `reference_text`(「적용」). rule_talk에는 «책 소개는 참고로만 읽고 나뉨을 말한 문장만
   옮기며, 소개 문장은 unsupported가 아니라 note 한 줄»을 더했고, 12,000자를 넘으면 양끝을 남기고 가운데를
   줄인다(`squeeze_said` — 사람이 쓴 문장은 맨 위나 맨 아래에 온다). 옮기지 못한 말이 넷 이상이면 접어 보인다
   (돌려주되 화면을 덮지 않는다). ②의 해제 칸과 「더 묻기…」 창의 「해제 칸으로」 단추는 없앴다 — 그 단추가
   생긴 이유가 «해제가 딴 데 있어서»였다. 화면에서 「해제」라는 말이 사라졌다.
2. **못 찾은 목차 항목은 찾아 넣는다.** `align_toc_to_body`는 순서를 지키고 0.6 이상만 받으므로 OCR이 제목을
   틀렸거나 순서가 어긋난 항목은 «못 찾음»으로만 남았다. 이제 propose 응답의 `unmatched[].near`에
   `locate_title`(느슨한 유사도 0.35↑·행 안 포함 0.8·순서 무시) 상위 3행이 오고, ③ 위의 노란 상자에서
   제목 칩 → 비슷한 행 → 「여기서 시작」을 누르면 그 자리가 ③의 후보로 선다(체크된 채, 표지 «목차·찾아
   넣음», 확신도 0.6, 역할은 목차 층위대로). 같은 항목은 한 자리만이고, 「후보 보기」로 다시 세어도 남는다
   (`proposeState.located`, D-125의 LLM 답과 같은 방식). 자동으로 경계가 되지는 않는다 — 저장은 「적용」.
   비슷한 행이 없으면 그렇게 말하고 사이드바 「경계 넣기」를 가리킨다.
3. **③의 말.** 「체크」·「해제」 단추는 「☑ 경계로」·「☐ 경계에서 빼기」로, 「전체」는 「전체 ☑」로, 머리에
   «☑ = 경계가 됨 · 행을 누르면 골라서 한꺼번에 바꿈» 한 줄을 붙였고, 고른 행이 있으면 «고른 행 N개 — 오른쪽
   도구가 이 행들에 듭니다»로 말한다. 「해제」는 解題와 «체크 해제»로 같은 화면에서 두 뜻이었다.

### 덧붙임 2(2026-09-11) — ③은 «지금 경계 + 새 후보»다 · 도구는 누른 행 옆에

사용자가 닫았다 다시 연 화면을 보였다: 사이드바 「내용」에는 경계 27개(목차·LLM 구조·찾아 넣은 것)가 있는데
③에는 규칙이 잡은 행머리 꼴 후보만 서 있었다. «구조를 통째로 묻기»의 답과 「여기서 시작」은 저장하지 않으므로
닫으면 사라지고, ③은 규칙으로 새로 센 목록이었기 때문이다. 이 상태에서 「적용」을 누르면 «전에 제안으로 만든
경계 중 이번 체크에 없는 것»을 지우므로, 확인창의 «지금 경계 중 20개를 지웁니다»를 무심코 지나면 세운 것을
잃는다. 사이드바와 ③이 다른 것을 보이는 화면은 «어딜 눌러야 하나»(D-122)에 답하지 못한다.

1. **③의 바탕은 저장된 경계다.** 후보를 셀 때 이 권의 살아 있는 경계를 읽어(`compState.currentBoundaries`)
   ③에 먼저 세운다 — 같은 자리의 후보가 있으면 그 행에 «지금 경계» 표지를 얹고 제목·역할·깊이는 저장된 것
   (사람이 확정한 것)으로, 없으면 행을 더한다(확신도 100%, 손으로 넣은 것은 «손으로 넣음» 표지). 저장된 행은
   처음부터 체크돼 있고, 새 후보는 저장된 경계가 하나라도 있으면 체크 없이 선다. **아무것도 안 건드리고
   「적용」하면 아무 일도 없다.** 닫았다 열어도 ③과 사이드바가 같다.
2. **«바뀐 것»은 저장된 경계와 견준다** — 저장된 경계가 있으면 «체크한 것 vs 저장된 것»으로 «새로 세울
   자리 N · 지울 자리 M»이고, 체크를 바꿀 때마다 다시 센다(적용하면 정확히 그것이 일어난다). 경계가 없는
   책은 전처럼 규칙 기준이다.
3. **「적용」은 지목한 것만 지운다** — `replace="listed"` + `drop`(체크를 뺀 저장된 행의 id). 손으로 넣은 것도
   체크를 빼면 지워지므로 「손으로 넣은 경계도 정리」 체크박스는 없앴다. 행 목록에 없는 자리(목차 쪽 등)의
   경계는 ③에 서지 않고 건드리지도 않는다. dry_run이 «새로 설 수»(`would_create`: 같은 자리·층위에 살아
   있는 경계가 없는 구간)를 세어 확인창이 «새로 N개를 세우고 M개를 지웁니다»로 말하고, 응답의 `new`가 실제로
   새로 선 수다(같은 자리를 다시 보낸 것은 세지 않는다). 커밋도 새로 선 것·지운 것·역할 바뀐 것이 있을 때만.
4. **도구는 누른 행 아래에.** 위 줄의 붙박이 도구 대신, 행을 누르면 그 행 바로 아래 한 줄로 «이 행에: 역할·
   깊이·바꾸기·☑ 경계로·☐ 경계에서 빼기·억제»가 뜨고, Shift·Ctrl로 여러 행을 잡으면 «고른 N행에:»가 된다.
   같은 요소를 DOM에서 옮기므로 단추의 이벤트는 그대로다(목록을 다시 그리기 전에 보관함으로 되돌린다).
   «선택»이라는 상태를 배우지 않아도 «누르면 고칠 수 있다»만 알면 된다.
5. **발이 행을 덮는다.** 실제 브라우저(720px 창)에서 «도구가 안 뜬다»(사용자 보고) — 붙박이 발(「적용」 줄)이
   마지막 보이는 행을 덮어 그 행을 누르면 발이 받았고, 도구가 그 행 아래 서면 발 밑에 가려졌다(headless 900px에서는
   위쪽 행만 눌러 못 봤다). 목록 끝에 발 높이만큼 여백을 두고, 도구는 `scroll-margin`을 알고 `scrollIntoView`로
   발·머리 줄 위로 굴려 보인다. 내장 브라우저에서 행 클릭 → 도구 588~613px, 발 627px — 가리지 않음을 확인했다.
   **진짜 원인은 따로 있었다**(사용자가 두 번째로 보고): 도구 요소를 고른 행 아래로 «옮겨» 두는데, 「후보 보기」·
   `_startFlow`·오류 경로가 목록을 `innerHTML`로 다시 쓰면서 그 안의 도구까지 지웠다 — 그 뒤로는 «고른 행 1개»만 뜨고
   도구는 영영 없었다. 목록을 비우는 세 곳이 먼저 도구를 보관함으로 거두고, `_ensureRowTools`가 없어진 도구를 원본
   markup으로 다시 만들어 손을 단다(안전망). 헤드리스 회귀: 「후보 보기」 뒤·「여기서 시작」 뒤·도구를 일부러 지운 뒤
   모두 행을 누르면 도구가 선다(`ui_d122_add2.py`).
6. **「단위 손보기」는 고른 기사를 잡는다.** 사용자: «기사 단위 하나를 선택하면 그 안에서 더 작은 조각을 나눠야 하지
   않나». 쪼개기는 이미 «원본 기사 안에 한 단 깊은 조각 경계를 넣는 일»이었지만(D-092), 사이드바 「내용」에서
   고른 것과 이어져 있지 않았고 패널의 「쪼개기」 단추는 창이 닫혀 있으면 아무것도 못 했다. 이제 `unit-selected`가
   그 기사를 손보기 대상으로 잡아 제목에 «고른 기사 #3 「…」 → 「쪼개기」로 그 안을 나눕니다»로 보이고, 단추가
   그 기사의 쪼개기 창을 연다(탭을 늦게 열어도 기억했다가 잡는다).

## D-123: 회전은 권의 속성이다 — 옆으로 스캔된 책은 돌린 이미지가 좌표계다

**날짜:** 2026-09-10
**상태:** 확정
**관련:** D-087(L2 image_width 배율), D-068(텍스트 레이어 산출물 검사), D-065(OCR 백업)

### 왜

옆으로 스캔된 책(족자·가로형)이 있다. 지금까지 회전은 뷰어의 CSS transform일 뿐이라 서버는 몰랐고,
OCR은 누운 이미지를 받아 깨졌다 — 엔진은 글자가 바로 선 것을 전제하므로 세로쓰기 책을 90° 누인 채로
넣으면 세로 열이 가로 줄로 검출되고 글자마다 90° 돌아가 인식이 무너진다. 사용자 결정: 회전을 저장하고
OCR도 돌린 이미지로 한다. 그러면 회전은 «화면 설정»이 아니라 **권의 속성**이고, 그 뒤의 모든 좌표
(L2 bbox·L3 레이아웃·앵커·내보내기)는 돌린 이미지를 기준으로 해야 한다.

### 결정

1. `manifest.parts[].rotation` ∈ {0, 90, 180, 270} — «바로 세우려면 시계 방향으로 몇 도». PDF /Rotate·
   PDF.js와 같은 부호다. 없는 옛 manifest는 0으로 읽는다(스키마 default는 런타임 값이 아니다).
   `core.document.part_rotation()`이 유일한 읽기 자리, `set_part_rotation()`이 쓰기(스키마 검증·원자적).
2. **서버의 쪽 이미지는 돌린 것이다.** `load_page_image_from_pdf`가 manifest를 읽어 PIL로 돌린다
   (`rotate_page_image` — PIL은 반시계가 양수라 부호를 뒤집는다). 개별 이미지 파일 경로·LLM 썸네일·
   레이아웃 감지 이미지도 같은 함수를 지난다. `native_render_scale`은 넓이와 긴 변으로 재므로 회전에
   불변이다.
3. **L2·L3에 `rotation` 도장을 찍는다.** 크기만으로는 180°나 정사각형의 90°를 가를 수 없다(Codex 지적).
   도장이 지금 회전과 다르면 파이프라인은 레이아웃을 늘여 쓰지 않고 거부하고(«다른 회전에서 만들어졌습니다»),
   부분 재OCR은 옛 L2를 섞지 않으며, 낡음 판정(`layout_changed_since_ocr`)이 재실행 대상으로 잡는다.
   **지우지는 않는다.**
4. **화면은 PDF.js `rotation`으로 그린다** — 캔버스 좌표가 곧 돌린 이미지 좌표라 오버레이·앵커가 그대로
   맞는다. 뷰포트를 만드는 곳(그리기·맞춤·레이아웃 편집기·OCR 저장)이 모두 `pdfViewport()` 하나를 쓴다.
   툴바 ↺↻은 임시(CSS)이고, 「저장」이 (저장값+임시값)을 권에 적은 뒤 CSS를 명시적으로 푼다. 임시 회전의
   마우스 좌표 되돌리기(`_canvasCoords`)는 AABB 폭·높이 바뀜을 먼저 되돌린 뒤 배율을 곱한다 — 전에는
   순서가 반대라 비정사각형에서 틀렸다.
5. **저장 단추는 먼저 묻는다** — `GET …/parts/{part}/rotation`이 L2·L3가 있는 쪽의 합집합 수를 주고, 있으면
   «N쪽은 다시 OCR해야 합니다 (지우지는 않습니다)»로 확인한다. `PUT`이 저장하고 커밋한다.
6. **텍스트 레이어 PDF는 출력 복사본의 /Rotate를 올린다** — 다른 뷰어에서도 저장한 방향으로 열리고, 표시
   공간이 곧 L2 좌표계가 된다. 글자는 표시 공간에서 자리를 잡고 `derotation_matrix`로 쪽 공간에 옮기며,
   글자 방향은 줄마다 TextWriter의 morph(그 점 둘레, 회전 부분의 역행렬)로 세운다. 부호는 실제 PDF에서
   잰 시험이 고정한다(M은 거꾸로 섰고 ~M이 맞았다). `_ink_check`도 bbox를 `rotation_matrix`로 옮겨 잰다.
   원본 L1은 그대로다.

### 잰 것

- `tests/test_part_rotation.py` — PIL 왕복으로 `unrotate_point` 부호 고정, 돌린 이미지 크기, 옛 manifest 0,
  옛 L3 거부, 낡음 사유, 내보내기 산출물에서 글자 자리·방향·잉크 비율(> 1.5).
- headless(복사본 서고): 임시 90° → 「저장」 → 캔버스가 PDF.js 회전으로 다시 그려지고 CSS transform이
  풀리며 라벨 «저장 90°», 다른 쪽에서도 유지, 되돌려 저장.
- 복사본에서 ndlkotenocr 1쪽: L2·L3에 rotation 90, 돌린 크기(4000×2799·1661×1162)로 저장. 글은 세로쓰기를
  누인 채 읽은 것이라 당연히 틀렸다 — 회전은 «세우는» 쪽으로 저장해야 한다는 뜻이다.

### 한계

- 회전을 바꾸면 옛 L2·L3는 화면 오버레이에 그대로 그려질 수 있다(거부는 파이프라인·낡음 판정에서만).
...
## D-125: 구조를 통째로 묻기 — 모델은 위치를 만들지 않고 고르기만 한다

**날짜:** 2026-09-10
**상태:** 확정
**관련:** D-117(4단 LLM은 표본만·답은 정해진 종류), D-121(규칙 변경의 관문), D-122(①②③ 한 흐름), D-092(경계가 정본), D-094(찍은 자리 → 행·글자)

### 사용자 요청

«OCR된 텍스트만 내보내기 해서 LLM에게 재구조화해 달라고 하면 훨씬 잘 할 것 같다. 그걸 CTB 안에서, 내보내기와
달리 **기존의 이미지-문자열 위치를 모두 보존하면서** 할 수 없을까?» (2026-09-10)

### 문제

규칙(D-116~D-119)은 되풀이되는 표지(○·날짜·卷·행 첫머리 어휘)가 있는 책에서 잘 듣는다. 문집·잡록처럼 표제가
제각각인 책은 표지가 없어 규칙이 못 잡고, 4단(D-117)도 «되풀이되는 표지»를 묻는 것이라 답이 비어 있다.
사람은 전문을 읽으면 어디서 글이 바뀌는지 안다. 모델도 그렇다. 그런데 텍스트를 «내보내기»로 뽑아 밖에서 물으면
답을 (쪽·행·글자) 자리와 이미지 위 점선으로 되돌릴 길이 없다.

### 결정

1. **모델이 위치를 만들지 않고 고르기만 하게 한다.** 행마다 `p8-L3` 번호를 붙여 텍스트만 보내고(좌표·이미지
   없음), 답은 «어느 행에서 새 글이 시작하는가»의 번호 목록이다. 코드가 실제 행인지 확인해 ③의 후보와 같은
   모양(쪽·행·글자 0·제목·깊이·역할·근거 `llm:structure`)으로 바꾼다. 후보가 지금 ③에 서는 것과 같은 자리이므로
   「적용」이 만드는 경계·앵커 글자·점선·재대조·되돌리기가 전부 그대로 된다 — 위치 보존은 «시키지 않아서» 된다.
2. **답은 검증을 지나야 후보가 된다.** 없는 행·빈 행·되풀이는 버리고 «버린 수»를 화면에 적는다. 제목이 그 행의
   글자가 아니면 버리지 않고 행의 글자로 바꾼다 — 자리가 정본이고 제목은 사람이 고친다. 확신도는 모델의 자신감이
   아니라 고정 0.6(중간)이다(D-117과 같은 태도). 규칙 후보와 같은 자리를 가리키면 그 후보에 근거만 보태고 0.8로
   올린다(두 출처가 가리켰다). 사람이 억제한 자리는 모델이 가리켜도 되살리지 않는다.
3. **자리는 「더 묻기…」의 네 번째 선택지다.** 규칙 ②와 별개로 돌고 결과만 ③에 합쳐진다. 저장은 여전히 「적용」
   한 곳(D-122). 보내기 전에 «권 전문 N행·M자·호출 K번»을 보인다 — 실행 게이트를 도구 층에(전역 규칙 11).
4. **긴 권은 쪽 경계에서 7,000자 묶음으로 나눠 묶음마다 한 번 부른다.** 로컬 모델의 문맥(8k 토큰)에도 들어가는
   크기다. 묶음 하나가 실패하면 그 묶음만 비고 나머지 답은 살린다. 묶음 경계의 글은 앞뒤에서 이어질 수 있다고
   모델에 말하지만, 겹침(overlap)은 두지 않았다 — 겹치면 같은 행이 두 답에 나와 세는 것이 흐려진다.
5. **확정본(L4)만 본다.** 제안·적용이 L4만 읽는다(D-088). L2로 보완하는 규칙 도출과 달리, 여기서 가리킨 자리는
   곧 경계가 되므로 확정본에 없는 행을 가리키게 해서는 안 된다.

### 하지 않는 것

- 행 중간 경계(D-090 2단계)는 만들지 않는다 — 행 첫머리만. 「○七日」처럼 열 중간에서 날이 바뀌는 판식은 규칙이
  잡는다.
- 모델의 답을 규칙(`segmentation_rules`)으로 저장하지 않는다. 이 답은 «이 권의 자리»이지 «이 책의 규약»이 아니다.
- 자동 트리(사이드바)는 이 길을 쓰지 않는다 — 사람이 창에서 고를 때만 모델을 부른다.

### 구현

`src/core/structure_llm.py`(묶음 나누기·번호 붙이기·검증·호출), `POST /api/documents/{doc}/segmentation/structure/llm`
(`dry_run`이면 크기만), 화면은 `proposeState.llm`에 답을 두고 `_proposeBoundaries`가 규칙 후보와 합친다
(`_mergeLlmProposals`). 근거 표지 «LLM 구조». 시험 `tests/test_structure_llm.py`(번호 왕복·쪽 경계 묶음·검증의
버림/제목 바꿈·묶음 실패 생존·라우트 dry_run·가짜 모델로 라우트).

### 실측 (2026-09-10, 속몽구분주 권4 — 확정본 20쪽, 글이 있는 행 225, 3,889자, 묶음 1개)

| 모델 | 시간 | 결과 |
|---|---|---|
| Ollama `kimi-k3:cloud` 1회 | 13초 | 가리킨 15자리 전부 실제 행. 손 경계 10편 중 표제 행 6개를 정확히, 나머지는 본문 첫 행 등 |
| Ollama `kimi-k3:cloud` 2회(화면) | 13초 | 가리킨 10자리 전부 실제 행. 표제 행 8개 정확, 1개는 규칙 후보와 같은 자리(근거 합침), ③ 후보 12 → 21 |
| Ollama `glm-5.3:cloud` | 69~109초 | 실패 — JSON 대신 영문 추론을 써 내려간다. D-118의 «사고 켜 다시 부르기»도 사고가 예산을 다 써 response가 빈다 |
| Ollama `gemma4:e4b`(로컬) | 39초 | 실패 — `{}`만 돌려준다 |

배운 것: **모델을 고르는 것이 결과를 가른다.** 짧은 표본에는 되던 glm-5.3:cloud가 권 전문에는 추론만 쓴다.
그래서 실패 문구가 「모델」 드롭다운으로 안내한다. D-118의 다시 부르기는 사고 예산을 답변 예산에 **더하도록**
고쳤다(`ollama.py` — 안 더하면 긴 입력에서 사고가 4096을 다 쓴다). 그래도 glm-5.3:cloud는 이 길에서 실패한다 —
모델 이름을 코드에 적지 않는다는 원칙(D-118)대로 화면이 다른 모델을 권할 뿐이다.

### 남긴 것

- Codex 설계 브리프는 크레딧 부족으로 받지 못했다(관례상 먼저 받는다 — [[ui-linear-flow-where-to-click]]).
- 답의 «깊이»는 모델이 권 전체를 한 번에 보지 못하면 묶음마다 어긋날 수 있다(1단 묶음이 앞 묶음에만 있는 경우).
  사람이 ③에서 범위 선택으로 한꺼번에 고친다(D-122).

---

## D-126: 회전은 쪽 범위로도 — 책 중간의 다른 구간, 그리고 쪽마다 알맞은 엔진을 훑어보기가 권한다

**날짜:** 2026-09-10
**상태:** 확정
**관련:** D-123(회전은 권의 속성), D-124(みんなで翻刻OCR), D-117·D-125(LLM은 정해진 답만, 코드가 확인), D-055(권 전체 OCR)

**덧붙임(2026-09-21) — 이름을 「판독 계획」으로 바꿨다.** 아래 본문의 「훑어보기」·「쪽 훑어보기」는
옛 이름이다(코드·화면·사용자 안내서는 모두 바꿨고, 이 기록만 그대로 둔다 — 결정문을 고쳐 쓰면
그때 무엇을 정했는지가 사라진다). 바꾼 까닭 둘. ① 추출 모드의 「쪽별 결과 훑어보기」(D-055)와 이름이
겹쳐 사이드바와 뷰어 툴바에 같은 말이 둘 있었다. ② 「훑어보기」는 **무엇이 나오는지**를 말하지 않는다 —
이 기능의 산출물은 «회전이 다른 구간»과 «구간별 엔진 계획»이고, 「권 전체 OCR」 옆의 체크도
«판독 계획대로»라야 읽힌다. 식별자(`page_survey`·`survey-*`·라우트 `rotation/suggest`)는 그대로다.
모달 안의 실행 단추만 「계획 세우기」다(창 제목이 「판독 계획」이므로 같은 말을 두 번 쓰지 않는다).

### 사용자 요청

«책 중간에 90도 회전을 번복해야 하는 부분이 존재하고… 훈점이 달린 건 honkoku ocr이 더 잘할 것 같기도 해서
고민이 돼» → «쪽 범위 지정도 좋고, LLM에게 스캔하게 한 후 제안해주는 것도 좋아» → «방향 판단이랑 추천 엔진 판단도
함께… 근현대 활자본이면 ndl ocr lite, 손글씨면 ndl koten ocr full, 훈점 등이 있으면 honkoku… 한글이 있다면 LLM 비전»
→ «한 페이지에 한글+훈점 텍스트가 다 있는 식으로 충돌한다면, 직접 해당 페이지에서 영역별 OCR 해야한다고 알려주도록»
(2026-09-10, 182쪽 강의 교재를 honkoku·ndlocr로 읽던 중).

### 문제

D-123은 회전을 권 하나에 값 하나로 두었다. 옆으로 스캔된 책은 그것으로 충분했지만, 책 중간에 접은 그림·가로 표·
뒤집힌 낙장이 있으면 세워 둔 90°가 원래 바로 선 쪽에도 적용되어 그 구간의 OCR이 엉뚱해진다. 또 한 권 안에 활자·
손글씨·훈점 단 쪽이 섞이면 엔진 하나로 권 전체를 읽는 것이 맞지 않는데, 어느 쪽이 어느 종류인지 사람이 182쪽을
넘기며 찾아야 했다.

### 결정

1. **회전은 권의 값 + 쪽 범위 목록이다.** `parts[].rotation`은 그대로 두고 `parts[].rotation_ranges:
   [{from, to, rotation}]`(양 끝 포함, 1부터)을 더했다. 읽는 함수는 `page_rotation(doc, part, page)` 하나 —
   범위에 들면 그 값, 아니면 권의 값. 쪽 이미지를 여는 곳·L2/L3에 도장을 찍는 곳·도장을 견주는 곳(파이프라인·
   낡음 판정)·썸네일·텍스트 레이어 내보내기가 **모두 이것**을 읽는다. `part_rotation`만 읽는 곳이 남으면 그 자리만
   범위 구간에서 좌표계가 어긋나므로, 호출자를 전부 바꿨다.
2. **저장은 같은 「저장」 단추, 옆에 쪽 범위 칸.** 비우면 권 전체(범위는 지운다 — «전체»가 새 기준이다), «37-52»면
   그 쪽만. 범위끼리 겹치면 새 것이 이기고, 권의 값과 같은 조각은 목록에서 빠지며, 인접한 같은 값은 합친다. 결과
   파일의 도장은 이미 쪽마다 찍히므로 나머지 장치(거부·다시 OCR 경고·백업)는 그대로 쓴다.
3. **훑어보기 — 방향은 코드가 재고, 모델은 글의 종류만 답한다**(D-117·D-125와 같은 태도). 쪽마다 두 가지를
   본다. (a) 방향: 잉크 투영 프로필의 **자기상관**(글줄 간격의 주기성)으로 «바로 섰다 / 누웠다»를 잰다 —
   모델을 부르지 않는다. 비전 모델에게도 방향을 묻지만, 투영이 모름(백지·그림·표)일 때만 그 답을 쓴다.
   (b) 종류: 비전 모델에 썸네일을 보여 여섯(활자·판본·손글씨·훈점·한글·백지, **목록**)만 받는다. 코드가 «지금
   회전과 다른 쪽»을 연속 구간으로 묶고(바로 선 쪽이 나오면 끊고, 모름은 끊지 않는다 — 모름은 «못 봤다»이지
   «바로 섰다»가 아니다), 종류를 **설치된 엔진** 중에서 대응시킨다: 활자 → NDLOCR, 판본 → 古典籍 Lite,
   손글씨 → 古典籍 Full, 훈점 → みんなで翻刻, 한글 → LLM 비전(없으면 다음 후보). 한 쪽에 종류가 둘 이상이면
   (한글+훈점) 엔진을 고르지 않고 «영역을 나눠 영역별로 OCR»하라고 따로 돌려준다 — 쪽 단위 엔진으로는 풀 수
   없는 일을 푼 척하지 않는다. 보내기 전에 «N쪽·호출 K번»을 묻는다(실행 게이트는 도구 층에).
   **누운 쪽은 90°인지 270°인지 투영으로 못 가린다** — «원래(0°)로», 권이 0°면 «시계 90°»를 추정으로 내고,
   화면이 그 구간의 첫 쪽을 **제안한 회전으로 돌려 보인 채** 묻고, 아니라면 반대쪽을 한 번 더 보인다. 확인한
   것은 같은 저장 길(`applySavedRotation`)로 적고, 엔진 구간은 고른 하나를 교감 탭 「권 전체 OCR」 칸에 넣는다.
4. **「권 전체 OCR」에 쪽 범위와 «된 쪽도 다시»를 두었다.** 배치 라우트는 전부터 `pages`·`skip_existing`을
   받았고 화면에 칸이 없었을 뿐이다. 훈점 구간만 みんなで翻刻으로 다시 읽는 일이 이것으로 된다.

### 하지 않는 것

- 쪽 하나 안의 영역별 회전은 두지 않는다 — 그 수준은 레이아웃 블록의 일이다.
- 훑어보기 결과를 저장하지 않는다. 다시 누르면 다시 본다.
- 모델의 «자신감»을 쓰지 않는다. 답이 정해진 값이 아니면 모름이다.

### 구현

`core/document.py`(`page_rotation`·`part_rotation_ranges`·`set_part_rotation(pages)`·`_merge_rotation_ranges`),
호출자 일곱(`image_utils`·`full_page_block`·`pipeline`×3·`layout_staleness`·`text_layer_pdf`·`llm_ocr` 썸네일·
`save_page_layout`), 라우트 GET/PUT `rotation`(`pages`·`from`·`to`), `POST …/rotation/suggest`(llm_ocr),
`core/page_survey.py`, 스키마 `rotation_ranges`, 화면(`savedRotationFor`·쪽 범위 칸·「훑어보기」·교감 배치의
쪽 범위·「된 쪽도 다시」). 시험 `tests/test_part_rotation.py::TestPageRanges`·`tests/test_page_survey.py`.

### 실측 (2026-09-10, 속몽구분주 권4 복사본)

- **비전 모델의 방향 판정은 믿을 수 없다.** 9·10쪽을 90° 눕혀 두고 물었더니 gemma4:cloud(자동 선택)는
  9쪽을 upside_down, 10쪽을 upright라고 답했다(둘 다 틀림). kimi-k3:cloud는 `max_tokens=200`이 사고에 다 소모돼
  잘렸다 — 800으로 올리고 `think=False`를 준다. 그래서 방향은 코드가 잰다.
- **투영의 변동계수(분산)는 반반이었다.** 쪽 위의 검은 띠 하나가 행 프로필을 흔들어 8쪽(바로 선 쪽)을 누웠다고
  했다. **자기상관(주기성)**은 6~25쪽을 0°·90°로 돌린 40판정을 모두 맞혔다(비율 1.9배 이상 vs 0.58배 이하 —
  문턱 1.5배). 오츠 문턱을 쓴다(고정 128은 누런 종이를 잉크로 센다).
- 화면 흐름: 9~10쪽을 90°로 저장 → 「훑어보기」(3쪽, 11초) → «9~10쪽을 0°로»를 미리보기와 함께 묻고 저장 →
  범위 비워짐 → 엔진 추천 «고서 판본 → 古典籍 Lite» 가 「권 전체 OCR」 칸에 들어감 → 그 칸으로 9~10쪽만
  paddleocr로 다시 읽음(2쪽 처리, 도장 0°). 콘솔 오류·4xx 없음.

### 남긴 것

- Codex 설계 브리프는 크레딧 부족으로 받지 못했다.
- 훑어보기는 쪽마다 모델을 한 번 부른다(종류 판정) — 182쪽이면 클라우드 모델로 10분쯤. `stride`(몇 쪽마다
  하나)를 라우트가 받지만 화면에는 아직 두지 않았다. 방향만 알고 싶으면 모델 없이도 되지만, 지금은 한 흐름이다.
- 종류 판정의 정확도는 모델에 달렸다(속몽구 3쪽은 «고서 판본»으로 맞았다). 훈점·손글씨의 구분은 더 재야 한다.

### 덧붙임(2026-09-10 저녁) — 구간별 엔진 계획, 180°, 확인창 한 번

사용자 지적 셋: «권 전체 OCR을 하면서 해당 쪽 범위는 권한 엔진이 수행하게 되는 거야?»(아니었다 — 한 번에 엔진
하나), «180도를 판별 못 하는 건 치명적», «일일이 “이대로 저장할까요?”가 굉장히 번거롭다».

1. **구간별 엔진 계획.** 배치 라우트가 `engine_plan: [{from, to, engine_id}]`를 받아 쪽마다 계획의 엔진으로
   돈다(없는 엔진은 기본 엔진으로 내리고 경고). 훑어보기가 계획을 만들어 「권 전체 OCR」 옆에 «훑어보기
   계획대로 · 1~36쪽 활자 → NDLOCR · 37~52쪽 훈점 → みんなで翻刻 · 지우기»로 보이고, 켜진 채 그 단추를
   누르면 계획의 쪽만 계획의 엔진으로 한 번에 돈다. 쪽 범위를 손으로 적으면 그 쪽이 우선이고 엔진은 여전히
   계획을 따른다. 종류가 섞인 쪽은 계획에서 빠진다. 진행 표시에 쪽마다 엔진이 찍힌다.
2. **180°와 90/270은 OCR 점수로 가린다.** PaddleOCR 3.x의 문서 방향 모델(`PP-LCNet_x1_0_doc_ori`)을 먼저 재
   봤는데 세로쓰기 한문에는 쓸모가 없었다 — 바로 선 쪽을 270°, 90°와 180°를 같은 값으로 답했다(40판정 중
   맞은 것 22, 그것도 우연). 대신 쪽 가운데 60% 조각을 후보 각도로 돌려 PaddleOCR에 넣고 «글자 수 × 평균
   신뢰도»를 견준다: 속몽구에서 바로 선 조각 88자·1.0, 180° 뒤집은 조각 36자·0.4로 또렷이 갈린다. PaddleOCR은
   세로쓰기의 0°와 270°를 스스로 바로잡아 그 둘은 같은 점수가 나오지만, 투영이 «섰다/누웠다»를 먼저 가르면
   후보가 둘로 준다 — 선 쪽은 0°·180°, 누운 쪽은 +90°·+270°(틀린 쪽은 곧 180°) — 그래서 넷 다 된다.
   PaddleOCR이 없으면(워커 포함) 전처럼 추정으로 내려간다. 쪽당 PaddleOCR 두 번이라 CPU에서 쪽당 20초쯤
   (실측 3쪽 76초, 비전 모델 포함). 보내기 전 창에 이 수를 보인다.
3. **확인창은 한 번.** 판정이 확실한 구간은 «회전이 다른 구간 N개: … 모두 저장할까요?» 하나로 묻는다.
   미리보기·반대쪽 묻기는 OCR 점수를 못 쓴 «추정» 구간에만 남는다. 엔진 계획은 묻지 않고 넣는다 — 실행은
   어차피 「권 전체 OCR」을 눌러야 한다.

4. **GPU 환경에서만**(2026-09-18 덧붙임의 `orientation_only`는 예외 — 방향만은 CPU에서도 연다).
   쪽마다 OCR을 두 번 돌리므로 CPU에서는 182쪽에 한 시간이다. 사용자 지시(«그 기능은
...
계획의 엔진(ndlkotenocr)으로 읽힘, 9쪽 도장 0°. 콘솔 오류·4xx 없음. 시험: `test_batch_follows_engine_plan_per_page`·
`test_ocr_scores_pick_the_readable_candidate`.

5. **모델은 고른다 — 창 하나로.** 처음 화면은 prompt(범위) → confirm(횟수)의 대화창 사슬이었고, 모델을 고를 자리가
   없어 `getLlmModelSelection()`을 id 없이 불러 언제나 «자동»(이 PC에서는 Ollama gemma4:cloud)으로 갔다. 사용자
   («모델 당연히 고르게 해야지… UI 너무 복잡할 것 같으면 모달 띄워서 모델 고르게 해», 2026-09-10). 도구 모음에
   드롭다운을 하나 더 두면 회전 단추 줄이 넘치므로 모달로 — 쪽 범위·모델(`llm-model-select`라 편성 ①과 같은
   목록, 비전 모델만)·«N쪽 — 비전 모델 N번 + PaddleOCR M번»(범위를 고칠 때마다 dry_run) 세 줄과 「훑어보기」 단추.
   대화창 사슬은 그 창이 대신하고, 결과 알림에 어느 모델이 답했는지(`provider:model`)를 적는다 — 라우트는 이미
   응답에 `provider`·`model`을 넣고 있었고 화면이 버리고 있었다.
   같은 날 레이아웃 탭의 계획 표시도 고쳤다 — 구간을 « · »로 한 줄에 이어 붙여 패널 폭을 넘어 옆으로 잘렸다(사용자
   지적). 단추 줄(`.ocr-buttons`)이 감싸게 하고 계획은 한 줄 전체를 차지하며 구간마다 한 줄로 보인다.

6. **모델 벤치마크와 그 뒤의 셋(2026-09-11).** 사용자가 gpt-5.5(OAuth)로 훑어보기를 돌리자 쪽당 15~30초였다 —
   추론 모델이라 25토큰짜리 답에 500토큰을 생각에 쓴다(실서고 로그 321건). «Ollama의 다른 모델은?»에 답하려고
   라우트와 같은 프롬프트·인자로 표본 10쪽(속몽구 판본 3·백지 1, 한글 논문 2, 일본어 활자 교재 2, 浩齋 석인본 2;
   정답은 축소판을 보고 매김)을 잰 결과:

   | 모델 | 종류 정답 | 쪽당(중앙값) | 두드러진 오류 |
   |---|---|---|---|
   | kimi-k3:cloud | 7/10 | 1.5초 | 활자·붓글씨를 판본으로 |
   | gemma4:cloud | 5/10 | 1.3초 | 백지를 손글씨로, 없는 종류를 덧붙임(한글 논문에 판본) |
   | minimax-m3:cloud | 5/10 | 2.2초 | 판본에 훈점을 지어냄(→ みんなで翻刻으로 감) |
   | qwen3-vl:235b-cloud·gemma3:27b-cloud | — | — | 은퇴(410) |

   그래서 셋을 바꿨다. (a) **훑어보기 기본 모델은 kimi-k3:cloud** — 화면 모달이 목록에 있고 은퇴가 아니면 미리
   고른다(`SURVEY_DEFAULT_MODEL`). 앱 전체 기본(gemma4:cloud, D-114)은 그대로다 — 벤치마크는 종류 판정만
   잰 것이고 OCR·레이아웃 경로는 재지 않았다. 설치 스크립트 5-1이 kimi-k3:cloud도 등록한다. (b) **은퇴 모델
   표시** — `/api/tags`는 은퇴한 클라우드 모델도 올려 두어 «●사용 가능»으로 보였고 고르면 쪽마다 410이었다.
   레지스트리 매니페스트 HEAD가 404면 은퇴(실측 0.2~0.4초, 프로세스 캐시, 오프라인이면 표시 안 함)로 보아
   `retired`를 붙이고 라우터가 «○ (은퇴한 모델)»로 내려 화면이 고를 수 없게 한다. (c) **섞임 판정의 짝** —
   종류가 둘이면 무조건 섞였다고 해서 한글 논문(활자+한글)이 매 쪽 «영역별로 읽으세요»였다. 한 엔진이 함께
   읽는 짝(`COMPATIBLE`: 활자+한글 → LLM 비전, 판본+훈점·손글씨+훈점 → みんなで翻刻)은 짝의 대표 종류로
   엔진을 고르고, 한글+훈점처럼 짝이 아닌 것만 섞임이다(사용자 지정 그대로).
   그리고 **진행 막대**(사용자 요청 «훑어보기도 프로그레스 바 표시를») — 라우트가 `stream: true`면 「권 전체
   OCR」과 같은 SSE(`start`·쪽마다 `page`·`complete`)를 보내고, 모달이 닫히지 않은 채 «3/20쪽 — 9쪽 고서 판본 →
   ndlkotenocr»로 올라간다. 완료 없이 끊기면 실패로 올린다(배치와 같은 이유 — 조용히 «다 된 것»처럼 보이면
   안 된다). stream을 켜지 않은 호출(시험·CLI)은 전처럼 JSON 하나다.

### 덧붙임(2026-09-18) — 「권 전체 OCR」의 «돌아간 쪽은 세워서»: 방향 잡기를 OCR 단추 안으로

사용자 질문: «하나의 PDF 안에 90도 돌려서 봐야 할 내용과 그렇지 않은 내용이 병존한다면 어떻게 해야 쉽고 간편하게
OCR을 진행할 수 있을까?» 답은 이미 있었다(쪽 범위 회전 + 훑어보기). 그런데 «좀 덜 복잡하게 하는 방법은 없을까?».

**복잡한 진짜 이유**: 방향 잡기가 OCR과 다른 단추·다른 창에 있었다. 훑어보기는 «방향»과 «글 종류(엔진)»를 한
창에 묶었는데, 방향은 코드가 재고 종류만 비전 모델이 본다. 방향만 필요한 사람도 모델 고르기·LLM 호출·GPU
게이트를 다 지나야 했다.

**설계 한 줄**: 「권 전체 OCR」에 «돌아간 쪽은 세워서» 체크(기본 켬)를 두고, 켜져 있으면 실행 직전에 훑어보기
라우트를 `orientation_only`로 한 번 불러(LLM 없음·GPU 게이트 없음) 구간을 기존 저장 경로로 저장한 뒤 OCR을 잇는다.

1. **라우트** `rotation/suggest`가 `orientation_only: true`를 받는다 — 비전 모델을 부르지 않고
   (`calls: 0`, `engines: []`, `mixed: []`; 라우터 객체는 엔진 목록 초기화가 llm_vision에 붙이느라 생길 수
   있으나 호출은 없다), 투영으로 «섰다/누웠다»만 가른다. **누운 쪽에서만** PaddleOCR 점수로 90/270을
   가르고, 그것도 **GPU에서만**(`gpu_runtime()`) — CPU거나 점수가 못 가르면(1.3배 안·PaddleOCR 없음)
   추정(`guess`)으로 두어 화면이 미리보기로 묻는다. 바로 선 쪽의 180° 검사는 하지 않는다: 쪽마다 OCR 둘은 CPU에서 한 시간이고(2026-09-10
   사용자 지시), 이 기능의 이름이 «세우기»다. 라우트 수는 그대로다(D-079).
2. **화면** — 레이아웃 탭 「권 전체 OCR」 옆 «돌아간 쪽은 세워서»(기본 켬). 누르면 배치 확인창 한 번 → 진행 막대에
   «3/20쪽 — 9쪽 누움» → 찾은 구간을 훑어보기와 **같은 창**으로 묻고(`_confirmRotationRanges`로 뽑아 둘이
   공유 — 묻는 말이 두 곳에서 달라지면 사람은 다른 기능으로 여긴다) → 저장 → OCR. 취소하면 회전 없이 OCR을
   잇는다. 쪽 범위를 «3,7,10-12»처럼 적었으면 [최소, 최대]로 잰다 — 회전은 권의 속성이라 사이 쪽에 저장돼도 옳다.
   미리보기는 뷰어(`viewerState`·`pdfState` 둘 다)가 그 권을 보고 있을 때만 — 그 사이 다른 문헌으로 갔으면
   확정 구간만 묻고 추정 구간은 건너뛴다(저장하지 않는다). 화면 코드(`autoOrientForOcr`)를 못 불러오면
   조용히 건너뛰지 않고 토스트로 알린다.
3. **전면 블록의 회전 도장** — 회전을 저장한 쪽을 배치가 다시 돌 때, 옛 전면 블록(L3)이 남아 있으면 파이프라인이
   «다른 회전의 레이아웃»이라 거부하고 사람이 손으로 지워야 했다. `ensure_full_page_block`이 **전면 블록
   하나뿐인 레이아웃**(`is_full_page_layout`, 형태 판별)의 도장이 지금 회전과 다르면 지금 회전으로 **치수만
   다시 잰다** — 폭·높이·bbox·도장만 새것이고 블록의 쓰기 방향·종류·skip과 `analysis_method`는 옛것을 옮긴다
   (Codex 지적: 형태로만 판별하므로 사람이 만진 전면 블록도 걸린다 — 통째로 다시 만들면 그 손질이 사라진다).
   블록이 여럿인 레이아웃은 그대로 두고 거부되게 둔다(D-123) — 잃을 것이 있는 쪽이다. 같은 이유로
   `layout_changed_since_ocr`는 L3가 없고 L2만 있는 쪽도 L2의 도장이 지금 회전과 다르면 «바뀜»으로 본다 —
   아니면 옛 좌표계의 L2가 «이미 결과가 있다»로 건너뛰어졌다.
4. **`sideways_target`의 180° 결함(기존)** — 권이 180°일 때 누운 쪽에 0°를 제안하면 후보 둘(0°·180°)이 모두
   여전히 누운 것이었다. 누움은 지금 회전 ±90°이므로 권이 90°·270°일 때만 0°를, 아니면 90°를 제안한다.

**하지 않은 것**: 한 쪽 **안**에 세로 본문과 가로 표가 섞인 경우 — 쪽 회전으로는 못 풀고 블록 단위 회전은 없다.
실제 자료에 나오면 그때 정한다.

**검증**: `tests/test_page_survey.py`(CPU — 모델을 부르면 실패하는 가짜, 점수 안 잼·guess / GPU — 누운 쪽에서만
후보 둘·sure / manifest에 page_count 없음), `tests/test_full_page_block_rotation.py`(전면 블록 재생성·손 레이아웃
보존). **headless Chrome E2E**(격리 서고·`CTB_CONFIG_DIR`, .venv CPU, PaddleOCR): 세로 줄무늬 3쪽 중 2쪽만 가로로
만든 PDF → 「권 전체 OCR」 한 번 → 확인창 둘(배치·2쪽 미리보기 «90°로») → `rotation_ranges [2~2 → 90°]` 저장 →
세 쪽 OCR 완료, L2·L3 도장 0/90/0. 이 E2E가 잡은 것: `add_document`로 만든 문헌은 manifest에 `page_count`가
없어 훑어보기 라우트가 «쪽 수를 몰라» 400이었다(배치 라우트의 `_resolve_page_count`로 통일 — 훑어보기 단추도
같은 400을 냈을 자리). 실제 고서 PDF로 누운 구간을 세운 실측은 아직 없다.

## D-127: Codex 교차검증(2026-09-16) 반영 — L7 주석 경로의 계약 다섯

**날짜**: 2026-09-16
**상태**: 확정

**배경**: a0f47a6 기준 Codex 독립 리뷰(`korean-humanities-dist/_codex-review-2026-09-12/out-ctb.md`, 11건)를
건마다 재현 시험을 먼저 쓰고 고쳤다. 11건 전부 실재했다(해당 없음 0). 여기에는 «무엇을 고쳤나»보다
**앞으로 지킬 계약**을 적는다 — 같은 자리에서 되풀이되지 않게.

1. **저장은 «지금 파일» 위에 병합한다** (Critical ①). LLM을 기다리는 라우트는 기다린 뒤 파일을 **다시 읽고**
   그 위에 병합해 저장한다(`_merge_generated_into_page`). 아까 읽은 데이터를 저장하면 기다리는 동안 들어온
   수동 주석이 오류 없이 사라진다. 잠금(asyncio.Lock)과 견줘 재읽기를 택했다 — 라우트는 `async def`이고
   재읽기~저장 사이에 `await`가 없어 단일 이벤트 루프에서 끼어들 수 없다. 그 사이에 await를 넣게 되면 그때
   잠금으로 올린다. 2·3단계 생성기는 `merge=False`로 LLM 항목만 돌려주고 병합은 라우터가 한다.

2. **L5·L6·L7 저장도 `write_json_atomic()`** (Critical ②). 표점·현토·번역·주석 저장 함수 넷이 `write_text`로
   덮어썼다. D-069의 규칙이 해석 저장소에는 적용되지 않은 채 남아 있었다.
   `tests/test_interp_layers_atomic_write.py`가 넷 모두 «갈아 끼우는 순간 죽어도 예전 파일이 남는지» 본다.

3. **주석 좌표(`target.start/end`)는 코드포인트가 정본이다** (③). 서버(Python `len`)가 그렇게 세고, 화면은
   `_annChars()`(= `Array.from`)로 맞춘다 — `.length`·`[i]`·`.slice`는 UTF-16 단위라 확장 한자(𠀀, CJK 확장 B)가
   앞에 하나만 있어도 그 뒤 주석이 한 칸 어긋나고 서로게이트 반쪽이 찍힌다. 렌더링·선택·표점 합성·AI 범위 보정
   전부 같은 자를 쓴다. `indexOf` 결과(UTF-16)는 `_annCpIndex`로 되돌린다. 시험은 node로 실제 함수를 돌린다
   (`tests/test_annotation_editor_js.py`, `tests/js_harness.py` — 첫 프론트 시험이다).
...
5. **LLM 답의 항목 목록은 공통 파서 하나로 꺼낸다** (⑥·⑦·⑪). `core/llm_json_items.parse_llm_items()` —
   ...
   (`ok`·`recovered`·`no_json`)와 버린 개수를 돌려준다. 사전형 주석 파서와 일반 주석 파서는 이것의 껍질이다
   (전에는 갈라져 같은 잘린 답에 1건·0건). 생성 단계 셋은 `GeneratedAnnotations`(list 하위형)에 진단을 얹어
   돌려주고 라우터가 응답 `diagnostics`에 실어, 화면이 «완료»와 «부분 완료(답이 잘려 건짐·기형 N개 버림)»를
   가른다. 좌표 검증(`_build_annotation_from_raw`)은 예외 대신 None — 항목 하나가 정상 항목 전부를 500으로
   끌고 가면 안 된다. 정규화 순서는 뒤집기 → 음수 자르기((0,−1)이 (−1,0)이 되던 것).

그 밖에 — **범주가 바뀌면 유형도 따라간다**(⑧: 병합·수정 둘 다, 범주가 그대로면 사람이 고른 유형을 지킨다),
**단위 계산의 행 색인은 권마다 한 번**(⑨: `boundaries.LineIndex`, 시험은 `==` 호출 횟수를 세어 O(N²)을 잡는다),
**사전 펼침 카드의 모든 문자열은 이스케이프**(⑤), **사전 저장 실패는 화면에**(⑩: 4xx/5xx·네트워크, 폼은 그대로).
...
전달 방식은 가져오지 않는다.** neuPrint는 서버·다중 데이터셋·다중 사용자 환경이라 규약을
사용자 입력으로 강제하는데, 이 도구는 `install.bat`·`installer/`로 배포되는 **로컬 단일 사용자**
도구다. 같은 목적을 사용자 부담 0으로 달성하는 쪽을 택한다. neuPrint 자체(API·계정·토큰·
`neuprint-python`)는 이 저장소에 들어오지 않는다 — 개인 실험 환경에만 둔다.

1. **데이터 버전은 «입력»이 아니라 «출력»에 각인한다.** neuPrint는 `Client(..., dataset='male-cns:v1.0')`
   으로 버전을 안 주면 연결조차 안 된다. 목적은 옳다 — 「이 결과가 어느 시점 데이터에서 나왔는가」를
   나중에 지목할 수 있어야 논문에 쓴다. 그러나 코퍼스를 하나 쓰는 사람에게 매번 스냅샷 ID를 요구하는 것은
   **얻는 것 없이 진입 비용만 늘린다.** 그래서 **내보내기·인용·보고서에 코퍼스 스냅샷 해시를 자동으로
   붙이고** 사용자는 아무것도 입력하지 않는다. `core-schema-v1.3`은 **스키마** 버전이지 **데이터**
   버전이 아니다 — 지금 없는 것이 후자이고, 그 자리를 출력 쪽에서 채운다.

2. **Concept 승격·병합 때 구 ID를 죽이지 않는다.** 커넥톰은 뉴런이 병합·분할되면 구 bodyId → 신 bodyId
   매핑을 릴리스 노트에 남긴다. 남기지 않으면 그 ID를 인용한 과거 논문의 참조가 조용히 끊긴다.
   `Promotion Flow`에서 Concept이 승격·병합될 때 **정확히 같은 일**이 일어나는데 지금 스키마에는 그
   매핑의 자리가 없다. `Relation`에 `supersedes` 계열 서술어를 두거나 별도 매핑 테이블을 둔다.
   사용자에게 보이지 않는 내부 장부이므로 부담은 0이다.

3. **질의 API는 좁게, 그러나 «좁게»는 저장이 아니라 질의에서만이다.** neuPrint는 노드 17,087·엣지
   46,700 규모를 `fetch_neurons`·`fetch_adjacencies` 둘로만 노출한다. 임의 순회를 열어주지 않는 것이
   핵심이다 — 스키마가 커져도 사용처가 터지지 않는다. 다만 이것은 **접근 경로**의 이야기이고, 아래
   4~6번이 말하는 **저장 내용**과 혼동하면 안 된다. `core-schema-v1.3.md` §6.1
   `Predicate Rules (Core Stability Rule)`이 서술어 축에서 같은 일을 한다.

### 연결 구조에서 가져올 것 — 세 계통 실측(2026-09-20)에 근거한다

neuPrint API로 **씨앗 60개씩 조건을 맞춰** 세 계통의 출력 엣지를 받아 쟀다. 처음에는 하행뉴런만
보고 결론을 냈다가, «그 계통이 유비로 적절한가»라는 물음에 답하지 못해 두 계통을 더 쟀다.
**그 확인이 결론 하나를 뒤집었다** — 아래 6번.

> **이 항의 모든 수치는 재생산 가능하다.** `jt725/scripts/cns_structure_stats.py --all` 이 아래 표
> 넷(발산·수렴·구획화·극성)을 그대로 다시 뽑는다. 2026-09-20 대조에서 36개 셀이 전부 일치했다
> (「뉴런당 ROI 수」만 방향별 표기 대 통합 중앙값 차이). neuPrint 토큰이 필요하고 읽기 전용이다.
> **작성자를 믿는 대신 돌려서 확인하라는 뜻이다** — 이 항은 세 번 고쳐졌고 그중 둘은 측정 오류였다.
> 4·5항의 구현·검증 수치는 `jt725/scripts/graph_two_stage.py --suite` 가 같은 방식으로 다시 뽑는다
> (약 3분, 외부 LLM 호출 0건). 보고서는 `jt725/brain/sessions/reviews/2026-09-21-graph-two-stage.md`.

아래 표는 **타입 전수**다(2026-09-20 재측정). 초판은 계통당 씨앗 60개였는데, 그 표본이
DNge\*의 파트너 수를 2배 부풀렸다(발산 541→279, 수렴 610→258) — 앞쪽 60개가 우연히 고차수
뉴런이었다. KCg-m·Tm9은 60개와 전수가 사실상 같았다. **표본이 흔든 것은 파트너 수뿐이고
무게 분포는 세 계통 모두 안정적이었다.**

| 측정 (전수) | DNge\* (운동출력) n=429 | **KCg-m (버섯체 연합) n=1,342** | Tm9 (시각전달) n=1,771 |
|---|---:|---:|---:|
| 출력 파트너 수 중앙값 | 279 | 282 | 45 |
| 무게 1~2인 엣지 비율 | 56.4% | **82.6%** | 54.4% |
| 상위 3 파트너 무게비중 | 13.1% | 17.4% | 31.9% |
| 상위 10% 엣지 무게비중 | **59.4%** | 45.3% | 42.6% |
| 최빈 ROI 무게비중 | (아래 단서 참조) | (아래 단서 참조) | (아래 단서 참조) |

### 구획화 — primary ROI 기준 전수 (확정)

ROI는 세 번 쟀다. ① 60씨앗 `fetch_adjacencies` ② 전수 `roiInfo` 무제한 파싱 → 26~42%로 떨어짐
③ 전수 `roiInfo` **primary ROI 제한**. ②가 틀린 것이었다 — neuPrint ROI는 계층 구조라 부모·자식이
중복 계상되고, 계통당 비primary 키 22만~118만 개가 분모를 부풀렸다. ③이 ①과 몇 %p 안쪽으로
일치하므로 **초판 수치가 옳았다**.

| 최빈 primary ROI 무게비중 (전수) | 발산 | 수렴 | 뉴런당 ROI 수 중앙값 |
|---|---:|---:|---:|
| KCg-m (연합) n=1,342 | **83.2%** | **62.1%** | 5~6 |
| DNge\* (운동출력) n=429 | 43.0% | 68.7% | 8~9 |
| Tm9 (시각전달) n=1,771 | 87.2% | 85.8% | 2 |

**계통마다 비대칭의 방향이 반대다.** 연합 구조는 62.1% 넓게 모아 83.2% 좁게 내보내고, 하행뉴런은
68.7% 좁게 모아 43.0% 넓게 뿌린다. 시각 전달은 양쪽 다 좁다(85.8% / 87.2%, ROI 2개).
**구획 규칙은 계통의 임무를 따른다** — 연합은 수집을 열고, 출력 경로는 배포를 연다.

**이 저장소의 유비는 KCg-m이다.** 버섯체 케니언세포는 연합·기억 구조이고, 지식 그래프가 하는 일과
성격이 같다. 하행뉴런은 뇌→신경삭으로 **내보내는 통로**라 유비가 아니다.

4. **약한 관계를 지우지 말고 무게를 붙인다.** 세 계통 모두 무게 1~2인 엣지가 과반이고(55.6~82.0%),
   연합 구조에서 가장 높다(82.0%). 정교함은 «연결을 적게 두는 것»이 아니라 **«많이 두고 무게로
   가르는 것»**에서 나온다. 「그래프가 비대하다 → 병합해서 줄인다」는 본능과 정반대다.
   `Relation`에 `weight`를 두고 **순회 시 임계로 거른다** — 저장 시 병합하지 않는다.
   병합은 되돌릴 수 없고 임계는 되돌릴 수 있다. **세 계통에서 모두 성립하는 유일한 성질이다.**

   > **구현·검증(2026-09-21).** `jt725/scripts/graph_two_stage.py --mode ablation`. 약한 엣지를
   > 실제로 **지웠을 때** 잃는 것을 도달률·고립 노드·최대 도달집합으로 쟀다. 위키 `wiki/`
   > 전수 4,485쌍에서 무게 1 링크(전체의 56%)를 지우면 도달률이 **70.6% → 23.2%**(무방향
   > 98.6% → 79.4%), 고립 노드가 5 → 31 로 는다. 4항은 이 저장소의 실데이터에서 재현된다.
   >
   > **다만 「세 계통에서 모두 성립」은 도달률까지 확장되지 않는다.** 무게 분포(약한 다수)는
   > 세 계통 모두에서 성립하지만, **그것이 도달률을 만드는지**는 갈린다 — 같은 제거를 했을 때
   > 손실이 위키 −47.4%p · DNge\* −6.0%p · **KCg-m 0.0%p** 다. KCg-m 동종 induced subgraph 는
   > 엣지의 87%를 지워도 도달률이 안 변한다(평균 출력차수 197 — 우회로가 남는다).
   > **반증이 아니라 범위 문제다**: 4항의 처방(지우지 말고 임계로 거른다)은 그대로 유효하고,
   > 손해가 가장 큰 쪽이 하필 우리처럼 성긴 그래프(위키 평균 차수 4.6)라는 것이 요지다.
   > 「약한 연결이 도달률을 만든다」를 커넥톰의 보편 성질로 적으면 안 된다.

5. **2단 순회.** 강한 엣지로 주 경로를 잡고 막히면 약한 엣지로 떨어진다. 임계 하나를 인자로
   노출하면 같은 저장소가 «정밀 탐색»과 «폭넓은 탐색» 둘 다가 된다. 단, 상위 10%가 가진 무게가
   계통마다 42~62%로 달라 **임계값 자체는 데이터에서 정해야 한다** — 상수로 박지 않는다.

   > **구현·검증(2026-09-21).** 같은 파일의 `two_stage()`. 「강한 엣지로 주 경로, 막히면 약한
   > 엣지로」를 **사전식 최단경로**로 구현했다 — 키가 `(약한 엣지 사용 수, 홉 수)` 인
   > Dijkstra다. 약한 엣지를 **버리지 않고 비싸게** 만드는 것이 핵심이고, 그래서 도달률이
   > 떨어지지 않는다. 최적성은 무작위 소규모 그래프 300개에서 전수 탐색과 대조해 확인했다
   > (`--selftest`, 불일치 0건).
   >
   > | 그래프 (θ) | 단순 BFS(상한) | 강한 엣지만 **버림** | **2단 순회** | 경로 강한엣지 비율 |
   > |---|---:|---:|---:|---|
   > | 위키 `wiki/` (θ=2) | 70.6% | 23.2% | **70.6%** | 45.2 → **72.5%** |
   > | KCg-m (θ=6) | 49.7% | 33.0% | **49.7%** | 1.3 → **76.8%** |
   > | DNge\* (θ=19) | 100.0% | 21.3% | **100.0%** | 8.5 → **60.8%** |
   >
   > 기준선을 함께 두면 「굵은 것만 따라가면 된다」가 얼마나 나쁜지가 보인다 — 최대무게 우선은
   > 위키 11.0% · KCg-m 1.0% · DNge\* 7.0% 밖에 못 닿는다.
   >
   > **임계를 상수로 못 박는다는 것은 이렇게 나타난다**: 같은 분위수 q=0.9 가 위키에서 θ=3,
   > KCg-m 에서 θ=3, DNge\* 에서 **θ=19** 다. 해석된 θ 는 출력에 함께 찍는다.
   >
   > **사람이 고르지도 않는다 — 규칙이 고른다 (2026-09-21).** 처음 구현은 분위수 `q` 를
   > 사람이 주고 「훑기」 표를 보며 골랐는데, 그러면 5항을 분위수까지만 지키고 **분위수 자체는
   > 사람이 박는** 셈이다. 게다가 쓸 만한 q 가 위키 0.75 · DNge\* 0.9 · KCg-m 0.99 로 갈려
   > 공통 상수가 없다 — q=0.75 를 커넥톰에 쓰면 θ 가 1~2 로 잡혀 엣지 대부분이 «강함»이 되고
   > 2단 순회가 단순 BFS 로 퇴화한다. 그래서 선정을 규칙으로 옮겼고 그것이 이제 **기본값**이다
   > (`--q auto`). 규칙은 **「2단 경로의 강한엣지 비율이 과반인 가장 큰 θ」** —
   > 「주 경로가 실제로 강한 엣지 위를 지나는, 가장 엄격한 임계」라는 뜻이다.
   >
   > | 그래프 | 규칙이 고른 θ | 그 전에 손으로 고른 값 |
   > |---|---:|---|
   > | 위키 `wiki/` | 2 | q=0.75 → 2 |
   > | 위키 볼트 전체 | 2 | q=0.75 → 2 |
   > | KCg-m 동종 induced | 6 | q=0.99 → 6 |
   > | KCg-m 2홉 폐포 | 41 | q=0.99 → 41 |
   > | DNge\* 동종 induced | 19 | q=0.9 → 19 |
   > | KCI 인용망 | 1 | 퇴화(무게 전부 1) |
   >
   > **여섯 값을 모두 재현한다.** 규칙도 기댓값도 한 사람이 만들면 자기채점이므로, 다른 세션이
   > 자기 훑기 표에서 독립으로 5/5 를, 선정기를 따로 구현해 값싼 세 그래프 3/3 을 확인했다.
   > 재현 산출물(`--suite`)도 **규칙 경로로 돈다** — 손으로 골랐던 상수는 실행에서 빠지고
   > 기대값으로만 남아 어긋나면 표 머리말에 찍힌다. 산출물이 규칙을 안 쓰면, 규칙이 망가져도
   > 재현이 침묵한다.
   >
   > 위키에서 쓸 만한 θ 가 2 하나뿐인 사정은 그대로다 — θ=3 이면 강한 엣지가 11.7% 밖에
   > 안 남아 주 경로를 만들 재료가 없다. 규칙이 고르는 값이 바로 그 2 다.
   >
   > **11항과 충돌한다는 것을 여기 적어 둔다.** 이 구현에는 부호가 없다. 11항이 경고한
   > 「반박이 가장 밀집한 자리를 지지와 똑같이 세는」 상태에 그대로 걸려 있고, 위키 링크에
   > 부호가 없어 지금은 잴 수도 없다. `Relation` 에 부호가 생기면 사전식 키의 3번째 자리에 넣는다.

6. **출력 구획은 실제로 벽에 가깝다 — 3번을 살린다. (전수 확정)** 하행뉴런만 봤을 때(43.0%)는
   «구획은 사전확률일 뿐»으로 읽혔으나, 그것은 구획을 건너 배포하는 것이 임무인 계통이라 그렇다.
   연합·감각 계통은 **83.2%·87.2%로 출력 무게의 대부분이 자기 구획에 머문다.** 따라서 범위는
   순위 가중치가 아니라 **주소의 일부**로 두는 쪽이 유비에 맞는다. 다만 아래 9번대로 **이것은
   출력 쪽 규칙이다.**

7. **파트너 수·집중도는 일반 원칙이 아니다.** 전수 기준으로 출력 파트너 수가 45~282로 6배,
   상위 3 집중도가 13.1~31.9%로 2.4배 차이난다. 「커넥톰은 이렇다」고 말할 수 있는 값이 아니므로
   설계 근거로 쓰지 않는다. (초판에서는 12배·3배로 적었는데, 60씨앗 표본이 DNge를 부풀린 값이었다.
   **차이가 줄어도 결론은 같다** — 계통 간 편차가 커서 못 쓴다.)

### 수렴(입력) 측정 — Concept 승격 규칙의 근거

같은 씨앗으로 들어오는 엣지도 쟀다. 발산과 다른 이야기를 한다.

전수 기준이다(수렴 → 발산 순으로 적는다).

| 측정 (전수) | KCg-m 수렴 → 발산 | DNge\* 수렴 → 발산 | Tm9 수렴 → 발산 |
|---|---:|---:|---:|
| 파트너 수 중앙값 | 269 → 282 | 258 → 279 | 34 → 45 |
| 무게 1~2 비율 | 84.0% → 82.6% | 55.9% → 56.4% | 68.8% → 54.4% |
| 상위 3 무게비중 | 19.2% → 17.4% | 14.9% → 13.1% | 46.0% → 31.9% |
| 상위 10% 엣지 무게비중 | 47.1% → 45.3% | **62.6% → 59.4%** | 48.7% → 42.6% |

8. **수렴 임계는 «개수»가 아니라 «무게»로 잡는다.** 「출처가 몇 개 모이면 Concept으로 올리는가」를
   개수로 정하면 안 된다는 것이 이 표의 요점이다. 연합 구조는 입력 파트너가 276개나 되지만 그중
   83.5%가 무게 1~2, 즉 잡음 바닥이다. 개수를 세면 아무 개념이나 승격되고, 무게 상위 꼬리를 보면
   실제 기여자만 남는다(상위 10%가 입력 무게의 48.3%). `Promotion Flow`의 승격 조건은
   **출처 수가 아니라 가중 기여도**여야 한다.

9. **넓게 모아서 좁게 내보낸다 — 연합 구조에 한한 규칙이다. (전수 확정)** 연합 구조에서 최빈
   primary ROI 비중이 **수렴 62.1% · 발산 83.2%**로 21%p 갈린다. 입력은 구획을 넘어 들어오고
   출력은 자기 구획에 머문다. Concept 승격이 하는 일과 같은 형태다 — 출처(Layer 1)는 어디서든
   오고, 합성된 개념은 한 영역에 속한다. **6번은 출력 쪽 규칙이고 입력 수집에는 적용하지
   않는다**; 수집을 구획으로 막으면 연합이 일어나지 않는다.

   **비대칭의 방향이 계통마다 반대라는 것이 이 항의 근거다.** 하행뉴런은 수렴 68.7% · 발산
   43.0%로 **좁게 모아 넓게 뿌린다** — 연합 구조와 정확히 거꾸로다. 시각 전달은 양쪽 다 좁다
   (85.8% / 87.2%, 뉴런당 ROI 2개). 즉 «구획을 어디에 두는가»는 커넥톰의 보편 규칙이 아니라
   **그 계통의 임무를 따른다.** 이 저장소의 임무는 연합이므로 연합 구조의 방향을 따른다.

10. **어느 계통도 «깔때기»가 아니다 — 초판의 대조를 철회한다.** KCg-m은 입출력이 거의
    대칭이고(269↔282, 84.0%↔82.6%, 19.2%↔17.4%) 그 부분은 전수에서도 유지된다. 그러나 초판은
    하행뉴런을 「610→541로 모아서 내보내는」 대조군으로 삼았는데, **전수로는 수렴 258 · 발산 279로
    오히려 발산이 크다.** 60씨앗이 만든 착시였다. 파트너 수로 «모은다/편다»를 말할 수 있는 계통은
    셋 중 없다.

    남는 차이는 **무게 집중도**다 — 상위 10% 엣지가 가진 무게가 DNge 59~63%인 데 비해
    KCg-m 45~47%, Tm9 43~49%다. 하행뉴런은 파트너를 줄이는 것이 아니라 **소수 파트너에 무게를
    몰아준다.** 따라서 Concept 층은 «요약해서 줄이는 단계»가 아니라 같은 폭을 유지하며
    **가중치가 재배치되는** 단계로 본다 — 결론은 초판과 같되 근거가 파트너 수에서 무게로 바뀐다.

### 극성 측정 — Relation 에 부호가 필요한 이유

`consensusNt` 로 입력 시냅스 무게의 구성을 쟀다. 초파리 기준으로 ACh는 흥분, GABA·히스타민은 억제,
글루탐산은 주로 억제이나 맥락 의존이라 따로 둔다. 도파민·옥토파민·세로토닌은 조절계다.
«강연결»은 무게 5 이상으로 잘랐다(임의 절단이며 값 자체가 결론은 아니다).

| 입력 무게 구성 | KCg-m (연합) | DNge\* (운동출력) | Tm9 (시각전달) |
|---|---:|---:|---:|
| 흥분 | 74.7% → 강연결 65.1% | 58.0% → 58.3% | 53.3% → 52.9% |
| 억제 | 12.0% → **27.7%** | 27.2% → 27.5% | 35.0% → **42.5%** |
| 글루탐산(모호) | 0.5% | 12.1% | 11.1% |
| 조절 | **12.5%** | 0.2% | 0.0% |

11. **Relation 에는 부호가 있어야 하고, 부호는 «강한 엣지»에서 가장 중요하다.** 연합 구조에서 억제는
    전체 무게의 12.0%인데 강연결만 보면 27.7%로 두 배 넘게 뛴다(시각 전달도 35.0%→42.5%).
    **약한 연결은 대체로 흥분성이고 강한 꼬리에 반대 방향이 몰려 있다.** 4·5번대로 «강한 엣지로 주
    경로를 잡는» 순회를 부호 없이 구현하면, 반박이 가장 밀집한 자리를 골라 지지와 똑같이 세는 셈이
    된다. 무게와 부호는 **함께** 저장한다.

12. **지지/반박 외에 «조절» 종류가 따로 있다.** 조절계 입력은 연합 구조에만 유의미하게 존재한다
    (KCg-m 12.5%, 나머지 0%대). 이것은 내용을 주장하지 않고 **다른 연결의 무게를 바꾸는** 입력이다.
    이 저장소에도 대응물이 이미 있다 — 출처 신뢰도, 「이 판정은 폐기됨」, holder 표시 같은 것들이
    Relation 이 아니라 각 층에 흩어져 있다. 연합 층(Concept)에만 조절 입력이 붙는다는 것이
    관찰이므로, **Concept 층에 한해** 조절형 Relation 을 1급으로 두는 것이 유비에 맞는다.

13. **부호를 이진으로 강제하지 않는다.** 글루탐산이 두 계통에서 11~12%인데 초파리에서 그 극성은
    실제로 맥락 의존이다. 「지지 아니면 반박」으로 몰아넣으면 그 11%가 거짓 신호가 된다.
    부호는 «지지 / 반박 / 맥락의존 / 불명»을 담을 수 있어야 한다.

**측정의 한계**: 극성 표는 **처음부터 타입 전수**였다(Cypher 집계, KCg-m 입력 엣지 360,579개).
초판에 「계통당 씨앗 60개」라고 적었던 것은 발산·수렴에만 해당하고 극성에는 틀린 단서였다 —
바로잡는다. `consensusNt` 는 시냅스별 실측이 아니라 뉴런 단위 예측·합의값이다. 글루탐산의 극성은
학계에서도 확정적이지 않으므로 위 표에서 따로 두었고 억제에 합산하지 않았다.
강연결 절단(무게 5)은 임의값이다.

**이 항목 전체의 방법론 교훈**: 60씨앗 표본이 한 계통(DNge\*)에서 파트너 수를 2배 부풀렸고,
그 위에서 «모아서 내보낸다»는 대조를 세웠다가 전수 재측정으로 철회했다. **씨앗을 편의로 고르면
그 편의가 결론이 된다.** 계통 간 대조를 근거로 쓸 때는 전수이거나, 최소한 무작위 표본이어야 한다.

**가져오지 않는 것**: neuPrint의 계정·토큰 계층(로컬 단일 사용자 도구에 인증 계층은 비용만 는다),
데이터 버전의 필수 인자화(1번의 이유), 원격 API 의존(이 저장소는 로컬에서 완결되어야 한다).

> **2026-09-21 개정 — 「원격 API 의존 없음」을 부분적으로 되돌린다.**
> 배포되는 앱은 여전히 로컬에서 완결된다. 다만 **만드는 쪽**에는 커넥톰을 다시 잴
> 길이 필요하다 — 기준값이 낡았는지 확인할 방법이 「작성자를 믿는 것」뿐이면 안 되기
> 때문이다. 그래서 기능을 둘로 갈랐다(`src/core/connectome.py`):
>
> - **기록된 기준값 대조** — 의존성 없음. 배포본에서도 돈다. 서고의 관계 분포를
>   연합 구조의 실측값과 나란히 놓는다. **판정하지 않는다** — 「82% 닮았다」 같은
>   점수를 내놓으면 그 숫자를 올리는 것이 목표가 되고, 커넥톰은 참고 좌표이지
>   목표가 아니다.
> - **살아 있는 재측정** — extra `connectome`(`neuprint-python`) + 토큰이 있어야 한다.
>   **`install.ps1`의 $pick 1~4 어느 가지에도 그 extra가 없다.** 설치 파일을 받은
>   사람에게는 선택지로도 보이지 않고, 없으면 라우트가 400에 「이 설치에 포함되지
>   않았습니다」를 담아 답한다(판독 계획이 CPU 환경에서 하는 것과 같은 모양).
>
> 배포에서 빠지는 것을 시험이 지킨다(`tests/test_connectome.py::TestExcludedFromDistribution`) —
> extra 이름을 설치 선택지에 슬쩍 넣는 것은 diff 한 줄이라 사람 눈으로는 지나간다.
>
> 같은 날 `jt725/scripts/cns_structure_stats.py`로 네 표를 다시 뽑아 **D-128의 24개
> 수치가 전부 일치**함을 확인했다(발산·수렴 13초, 구획화·극성 30초 — 읽기 전용).
> 이 항의 「작성자를 믿는 대신 돌려서 확인하라」는 단서가 실제로 지켜진 셈이다.
...
7. **「원래 깨져 있는 시험」은 없었다 — 시험이 설치되지 않은 extra에 의존하고 있었다.**
   전체 슈트의 실패 넷(`test_ocr_honkoku` 3 · `test_page_survey` 1)을 두 세션이 각각
   「기존 실패」와 「동시 실행 경합」으로 오진했다. **둘 다 틀렸다.** 원인은 둘:
   ① `honkoku_engine._process`가 잡을 예외(`ModelSetupError`)를 가져오려고
   `honkoku_ocr`을 **무조건** import해서, 「모델 없이 계약만 잰다」고 적어 둔 시험이
   가짜 OCR을 넣었는데도 `ModuleNotFoundError`로 죽었다.
   ② `test_page_survey`의 `FakeVision`에 `providers`가 없어 `llm_vision`이 조용히
   «사용 불가»로 떨어졌고, 그래서 훈점 엔진 후보가 전부 사라져 **선택 extra가 설치된
   환경에서만 통과하는 시험**이 됐다.
   둘 다 고쳤다(없으면 «결코 맞지 않는» 예외 대역 · 비전 지원 프로바이더 대역).
   교훈은 **「환경이 다르면 답이 다르다」를 먼저 의심하는 것**이다 — main 체크아웃
   `.venv`에는 onnxruntime이 있고 워크트리 `.venv`에는 없어서 두 환경이 다른 답을 냈다.
...
### 독립 검증 (2026-09-21) — Codex는 두 번 다 끊겼고, 서브에이전트가 대신했다

**Codex 교차검증은 두 번 다 결론을 내지 못했다.** 1차는 프롬프트의 「모지바케가 보이면
멈추라」가 너무 넓어 PowerShell 오류 출력이 깨진 것을 보고 중단했고(74,059 토큰),
프롬프트를 소스 파일로 한정해 다시 돌린 2차는 **크레딧 소진**으로 끊겼다(73,507 토큰).
중단 직전 「승격 가중치의 대체 계산」과 「병합 후 엔티티 저장 경로」를 의심 지점으로
지목했는데, **앞의 것이 실제 결함이었다.**

크레딧이 회복되기 전이라 **격리 워크트리의 서브에이전트**에게 같은 네 축 질문을 주되
작성자의 결론은 감추고 맡겼다(작성자 ≠ 검증자를 다른 방법으로 지킨 것이다).
그 검증이 아래를 찾았고 전부 고쳤다.

| 발견 | 무엇이 틀렸나 | 고침 |
|---|---|---|
| **① 승격이 «개수»로 열렸다** (Important) | 확정본이 없으면 무게가 `Tag 수`로 떨어졌다. **한 단위에 같은 표면형 Tag가 셋이면 무게 3.0**으로 임계를 그대로 넘는다 — 8항이 막으려던 바로 그것이 복원된다. 확정본이 **있어도** 표면형이 문자 그대로 일치하지 않으면(이체자·신자체) 같은 자리로 떨어지므로 예외가 아니라 상시 경로였다 | 대체 계산을 없앴다. 못 재면 `measured: false` · 무게 0이고 승격 근거가 되지 않는다. 「재 봤더니 0」과 사유 문구를 나눴다 |
| **② 순환 병합** (Important) | B→A 뒤 A→B를 코어가 막지 않아 장부에 순환이 섰다. 조회는 멎지 않지만 **«대체되지 않은» 개념이 하나도 남지 않아** 화면에서 되돌릴 길이 사라진다(삭제 금지라 장부를 지울 수도 없다). 화면 필터가 유일한 방어였는데, 라우트 직접 호출과 낡은 탭에서 뚫린다 | `merge_concepts`가 이미 대체된 목표를 거부한다 — 장부가 본체이므로 방어도 장부 쪽에 둔다 |
| **③ 고리 따라가기 복제** (Minor) | `resolve_id`와 `_annotate_superseded`가 같은 규칙을 각각 구현했고 **이미 갈라져 있었다**(한쪽만 순환에 경고) | `entity_id_map.follow_chain` 하나로. 복제가 되살아나면 시험이 잡는다 |
| **④ 깨진 장부를 덮어쓴다** | 검증자는 「장부가 깨지면 2항이 조용히 꺼진다」까지 봤는데, 그보다 나쁜 것이 있었다 — 그 뒤 첫 병합이 `record_mapping`으로 파일을 새로 써서 **남아 있던 매핑을 영영 지운다** | 읽기는 빈 장부로 잇되(서고는 열려야 한다) **쓰기를 막는다** |
| **⑤ 잔여** | `metadata.weight` 오타가 승격 전체를 영어 traceback으로 떨어뜨림 · `source_ids` 중복이 「2건 합쳤다」로 거짓 보고 | 각각 감싸기·중복 제거 |

**가장 값진 발견은 「시험이 초록인데 동작이 틀린 자리」였다.** `test_without_confirmed_text_it_fails_closed`가
Tag 30개를 **서로 다른 단위 30개에** 하나씩 흩어 놓고 「닫힌다」를 확인하고 있었다. 그
배치에서만 무게가 바닥에 깔린다 — 시험이 고정한 것은 코드의 성질이 아니라 **픽스처의
성질**이었고, 독스트링은 「그 성질을 여기 고정한다」고 적고 있었다. 지금은 개수가 위력을
발휘할 수 있는 배치(한 단위에 몰아 주기)로 잰다.

검증자가 `rejected`로 돌려보낸 것도 기록해 둔다(되풀이 올라오지 않게): 주입 키
(`superseded_by`)가 디스크에 닿는 경로 없음 · `_digest`는 플랫폼을 넘어 안정적 ·
`?scope=`의 빈 문자열과 없음은 구분됨 · 6항과 9항이 서로 새지 않음 · 새 모듈 사이 순환
import 없음.

### 사각지대 실측 (2026-09-22) — 초록은 「돌았다」가 아니다

Codex 교차검증이 또 크레딧에 막혀(세 번째) 기다리는 동안, **사람 눈이 아니라 기계로**
같은 질문의 일부에 답했다. 시험 전체(1,495건)가 도는 동안 `sys.monitoring`의 `PY_START`로
D-128 모듈 여섯 + `entity.py`의 **함수 진입**을 기록했다. 이 저장소가 古典籍 Lite·Full의
사각지대를 찾은 것과 같은 방법이다(CLAUDE.md 「올릴 때 절차」 3).

줄 커버리지로는 보이지 않는 것을 묻는다 — 「import돼서 `def` 줄이 실행됐다」가 아니라
**「그 함수의 본문이 한 번이라도 돌았는가」**다. 결과는 69개 중 60개 진입, 안 도는 것 아홉.

| 안 도는 것 | 성격 | 한 일 |
|---|---|---|
| `connectome.measure_reference`·`_share` | **시험에서는 안 돈다**(자동 시험이 neuPrint 에 붙지 않는다). 「neuprint가 없어서」라고 적었다가 **틀린 것으로 드러났다** — 이 PC 에는 extra 와 토큰이 있어 게이트가 열린다 | 손으로 돌렸고 **결함 둘이 나왔다**(아래 절). 시험은 질의문과 상수를 검사한다 |
| `connectome._token_from_file` | **덮을 수 있었는데 안 덮었다.** neuprint 없이도 도는 코드다(파일을 읽고 파싱할 뿐). 비밀을 다루는데 시험이 환경변수 쪽만 덮고 있었다 | 시험 7건. 그중 하나는 `_token_present`와 **두 파서가 어긋나는지**를 잰다 |
| `concept_scope.address_of`·`resolve_by_label` | **죽은 코드.** 부르는 곳이 어디에도 없다 | 걷어냈다 — 아래 |
| `entity_id_map.successors` | **죽은 코드.** `predecessors`는 시험이 쓰는데 이쪽은 아무도 안 쓴다 | 걷어냈다 |
| `entity.py:153 _get_source_head_commit` | **중복 정의** — `interpretation.py:232`와 본문이 같다(Phase 8부터, D-128과 무관) | 백로그로 넘긴다. `entity.py`가 `interpretation.py`를 들이면 순환 import 위험이 있어 한 줄로 끝나지 않는다 |
| `entity.auto_create_units_from_text` | **죽은 코드** — 부르는 곳 없음(D-128과 무관) | 백로그 |
| `entity.create_unit_from_source` | 라우트(`interpretations.py:676`)가 부르는데 시험이 한 번도 안 들어간다(D-128과 무관) | 백로그 |

**`resolve_by_label`을 지운 이유를 남긴다.** 이 모듈의 계약은 「**거르기만 하고 정렬하지
않는다** — 정렬하는 순간 범위가 순위를 만들어 6항이 버린 쪽으로 간다」인데, 같은 모듈의
그 함수가 `sorted(hits, key=…scope_document…)`로 **정렬하고 있었다.** 독스트링은 「범위 밖은
애초에 목록에 없으니 걸러진 뒤의 순서 이야기다」라고 변론했고 그 변론에도 일리가 있다.
그래도 지웠다 — **죽은 코드가 규칙의 예외를 품고 있으면, 다음 사람이 변론을 읽지 않고
배선한다.** 「같은 라벨이 문헌마다 다른 개념일 수 있다」는 문제는 여전히 참이므로,
필요해지면 6항을 다시 열고 결정으로 남긴 뒤 만든다.

**그리고 시험이 초록이면서 놓치고 있던 것이 하나 더 있었다 — 라우트는 있는데 화면이
없었다.** `connectome-comparison`을 부르는 곳이 `src/app/static/` 전체에서 0건이었다.
D-128 후속을 「구현됨」으로 적은 것은 거짓이 아니지만(라우트는 돈다), 이 저장소의 사용자는
비개발자 연구자이고 **URL을 손으로 치는 것은 기능이 아니다.** 게이트가 400에 담는 한국어
사유도 「화면에 그대로 띄울 수 있어야 한다」고 적어 두었는데 띄우는 곳이 없어 그 목적이
미달이었다.

**단추를 처음에는 해석 도구 바(`interp-toolbar`)에 붙였다가 옮겼다 — 그 도구 바가
죽어 있었기 때문이다.** 붙여 놓고 headless Chrome 으로 눌러 보니 보이지 않았고,
조상을 거슬러 올라가 보니 `#interp-panel` 자체가 **어느 모드에서도 `display:none`**
이었다. 그 패널을 여는 것은 `_switchMode("interpretation")` 하나인데
`data-mode="interpretation"` 탭이 index.html 에 없다 — 해석 편집기가 표점·현토·번역·
주석·인용 다섯으로 갈라지면서(D-096) 남은 자리로 보인다. 그래서 그 도구 바의 기존
단추 둘(「단위 만들기」·「LLM에게 요청」)과 공용 「저장」까지 함께 보이지 않는다.
**이것은 D-128과 무관한 기존 결함이고 백로그로 넘긴다**(B-008). 커넥톰 단추는 살아
있는 엔티티 사이드바의 「+ 새 엔티티」 옆에 둔다.

여기서 배운 것: **「시험이 JS 에서 라우트 이름을 찾았다」는 「사람이 누를 수 있다」가
아니다.** 앞선 병합 검증이 `interpState` 를 손으로 넣는 우회를 썼기 때문에 이 죽은
패널을 보지 못했다. 화면 검증은 사용자가 걷는 길 — 문헌 펼치기 → 쪽 열기 → 패널 열기 —
을 그대로 걸어야 한다.

시험 11건을 더했고 **여덟 가지를 실제로 깨뜨려 빨간불을 확인했다** — 화면이 라우트를 안
부르면 · 단추 id가 사라지면 · 사유를 한 줄로 뭉개면 · 항목 이름을 화면이 스스로 적으면 ·
기준값을 화면에 박으면 · 줄을 합쳐 총점을 만들면 · 두 토큰 파서가 갈라지면 · 따옴표를 안
벗기면. 전부 빨강으로 바뀌었다.

### 살아 있는 재측정이 정본과 다른 자로 재고 있었다 (2026-09-22)

화면이 생기고 나서야 **「기준값 다시 재기」를 실제로 눌러 볼 수 있었다.** 그때까지
`measure_reference`는 시험에서 한 번도 돌지 않았고(neuprint 미설치를 이유로 «못 재는
자리»로 적어 두었다), 실은 이 PC 에는 extra 와 토큰이 **있어서** 게이트가 열린다.
눌러 보니 숫자가 이렇게 나왔다.

| 칸 | 기록된 값 | 고치기 전 재측정 | 고친 뒤 재측정 |
|---|---:|---:|---:|
| 지지 | 74.7% | **22.0%** | 74.73% |
| 반박 | 12.0% | 12.1% | 12.05% |
| 모호 | 0.5% | 0.2% | 0.47% |
| 조절 | 12.5% | **65.2%** | 12.53% |
| 강연결 지지 | 65.1% | (재지 않음) | 65.06% |
| 강연결 반박 | 27.7% | (재지 않음) | 27.65% |

질의가 정본(`jt725/scripts/cns_structure_stats.py::polarity_table`)과 **네 군데** 달랐다.

| 자리 | 정본 | 이쪽 | 결과 |
|---|---|---|---|
| 신경전달물질 | `consensusNt`(합의값) | `predictedNt`(기계 예측) | 조절 12.5% → 65.2% |
| 무게 | `w.weight IS NOT NULL` | 조건 없음 | — |
| 계통 대조 | `=~`(전체 일치) | `STARTS WITH`(접두) | `KCg-m*` 변종까지 끌어옴 |
| 분모 | `unclear`도 센다 | nt가 null인 행을 버림 | 「아는 것들 사이의 비율」이 됨 |

**그리고 더 나쁜 것이 있었다 — 거짓 이름표.** `measure_reference`가
`{**RECORDED_REFERENCE, "source": "직접 질의"}`로 돌려주어, 실제로 다시 잰 것은
`polarity` 하나인데 `noise_share`·`top10_share`·`polarity_strong` 세 줄에까지
「방금 쟀다」가 붙었다. 대조 다섯 줄 중 **셋이 거짓**이 되는 자리다. 무게 분포는
부호 집계 질의로 뽑을 수 없으므로 애초에 이 함수가 잴 수 없는 값이다.

고친 뒤 다시 재니 여섯 칸 모두 **±0.0005 안**에서 기록된 값과 맞는다(기록값이 소수
3자리로 반올림된 수준). 이것으로 두 가지가 동시에 확인됐다 — 질의가 틀렸었고,
**D-128에 기록된 기준값은 정확하다**(2026-09-22 살아 있는 데이터베이스로 독립 재확인).

배운 것을 코드에 남겼다. 주석에 「정본과 같아야 한다」고 적어 두었지만 네 군데가
달라진 것을 아무도 몰랐다 — **주석은 코드를 지키지 못한다.** 그래서 질의를
`POLARITY_QUERY` 상수로, 분류를 `NT_CLASSES`·`STRONG_CUT`으로 꺼내고 시험이 그것을
검사한다(정본 스크립트는 이 저장소 밖에 있어 의존할 수 없다). 다시 잰 칸은
`LIVE_FIELDS`로 밝히고, `compare()`가 줄마다 `reference_live`를 붙여 화면이 ◆로
표시한다 — 「다시 재기」를 누른 사람이 다섯 줄 전부 새 값이라고 여기지 않게.

여기에도 **열 가지를 깨뜨려 빨간불을 확인했다**(기계 예측값으로 되돌리기 · 무게 조건
빼기 · 접두 대조 · 분모에서 모르는 것 빼기 · 질의 칸 이름 어긋나기 · 글루탐산을
억제에 합산 · 강연결 절단 바꾸기 · 기록된 값에 거짓 이름표 · 줄이 출처를 안 밝힘 ·
화면이 표시를 안 그림).

**이 측정의 한계를 밝힌다.** 「들어갔다」는 「제대로 잰다」가 아니다 — 한 번 들어갔어도
분기 절반이 안 돌 수 있고, 시험이 값을 확인하지 않아도 진입은 기록된다. 이 측정이 짚는
것은 **초록인데 한 줄도 안 도는 자리**뿐이고, 그 이상을 주장하면 안 된다. 그리고 **다른
모델의 눈은 아직 없다** — Codex 교차검증은 2026-09-22 기준 세 번째로 크레딧에 막혀
예약 대기 중이다(프롬프트는 발견을 즉시 파일에 적게 고쳐 두었다 — 두 번 다 죽으면서
아무것도 남지 않았기 때문이다).
---

## D-129: 경계 판정을 편성에 붙인다 — 두 길을 다 돌리고, 어느 쪽도 문지기로 쓰지 않는다

**날짜**: 2026-09-21(설계·구현) · **2026-09-22 확정**
**상태**: 확정 · **구현됨**(미결은 아래 「아직 안 한 것」 — 실서고 화면 검증)

> 설계 초안은 `docs/design-boundary-judge.md`, 실측 전문은 `docs/sessions/session_jev_structure.md`.
> 이 항은 그 초안을 결정으로 받아들인 것이고, **초안에 없던 것을 더하지 않았다** — 더한 것은
> 「무엇이 그 주장을 지키는가」(맨 아래 표)와 그것을 얻으려다 드러난 구멍 하나뿐이다.

**배경**: 판정 모델(TypeSafe Jev)은 글을 만들지 않고 «예·아니오 확률»과 «고르기»만 돌려준다.
지어낼 자리가 형식에 없다는 뜻이라, D-125가 생성 모델에게 맡긴 «글이 시작하는 행 고르기»를
이 모델로 다시 물을 수 있다. 문제는 그 답을 기존 규칙 층계(D-116·D-117·D-119)와 **어떻게
합치느냐**였고, 두 책에서 재 보니 합치는 방식이 결과를 뒤집었다.

**한 줄**: 두 길(규칙·모델)을 다 돌리고, 어느 쪽도 문지기로 쓰지 않으며, 둘이 얼마나
어긋나는지를 사람에게 보이고, 고르는 일은 사람이 한다.

### 채택한 것

**1. 규칙 후보로 모델의 답을 거르지 않는다 — 후보 집합은 합집합이다.**
천진담초(일기류)에서는 «규칙 ∩ 모델»이 가장 좋았다(정밀 0.729 → 0.875, 재현 0.972 유지).
그 결과만 보고 교집합을 택했으면 문집에서 무너졌다 — 운양집_01에서 같은 구성은 **재현
0.880 → 0.176**이다. 규칙 후보로 거르는 순간 규칙의 재현(0.183)을 그대로 물려받기 때문이다.
**규칙이 못 보는 책에서는 모델도 함께 눈이 먼다.** 규칙은 가산점이지 문지기가 아니다.
합치는 자리는 화면의 `_mergeLlmProposals` 하나다 — 같은 자리면 근거만 보태고(규칙 근거를 지우지
않는다), 새 자리면 후보로 더하며, **사람이 억제한 자리는 모델이 가리켜도 되살리지 않는다.**

**2. 문턱은 상수가 아니라 이 책에서 뽑는다.**
운양집_01에서 0.8이 들었던 것은 0.8이 특별해서가 아니라 **0.94와 0.77 사이가 비어 있었고**
0.8이 그 빈 구간에 떨어졌기 때문이다. `derive_toc_threshold()`가 확률 높은 것부터 내려가며
**자기 검증**(고른 행에서 제목이 실제로 시작하는가 — 코드가 공짜로 확인한다)이 깨지는 직전에서
자른다. 같은 책에서 0.94를 내놓고 고정 0.8과 **같은 14건**을 고른다(오답 0). 달라지는 것은
절벽이 다른 곳에 있는 책에서다. 답이 다섯 개도 안 되면 절벽을 말할 근거가 없으므로 그때만
0.8로 물러서고, 요청이 숫자를 주면 그 값으로 고정한다(비교·재현용).
왜 분위수가 아닌가: 백분위는 분포와 무관하게 **언제나 같은 비율을 자르므로** 고른 것이 대부분
맞는 책에서는 맞는 자리까지 잘라 낸다.

**3. 자기 검증은 확률과 다른 관문으로 따로 선다.**
확률 문턱 하나만 두면, 답이 적어 문턱이 0.8로 물러설 때 **자기 검증에 실패한 답이 그대로
들어온다**. 반대 배치를 시험에 걸어 찾았다(답 4개에서 엉뚱한 행 하나가 통과했다). 그래서
`toc_picks_to_proposals`가 확률과 무관하게 `sim < 0.85`인 답을 세우지 않는다.
한계는 남긴다 — 「續昇平館集」이 「昇平館集」 행을 고르는 접두 관계는 여기서 떨어지고,
그 자리는 규칙·본문 판정 후보로 사람에게 남는다.

> **얼마나 «다른» 관문인지 쟀다(2026-09-22, 호출 0건, 정렬 세션의 방법 — 라벨 없이 순위 상관만
> 보면 두 지표가 같은 순서를 내는지 알 수 있다).** 운양집_01의 고름 25건에서 **`prob ↔ sim`
> 순위 상관 +0.804**로 **둘은 꽤 같이 움직인다.** 상위 8건 집합 일치도는 4/8이라 정작 중요한
> 상단에서는 갈린다.
>
> **그리고 이 책에서는 자기 검증이 한 번도 구속하지 않았다** — 확률 0.8을 통과했는데 자기
> 검증에서 떨어진 것이 **0건**이다(반대는 3건: 「續昇平館集」·「○雲嶽飮泉集」 등이 sim은 높은데
> 확률에서 떨어졌다). 이 관문이 실제로 무언가를 막은 것은 **반대 배치 시험 안에서뿐**이다.
> 그러니 「확률만 두면 샌다」는 **성질로는 참이지만 이 책에서 자주 일어나는 일이 아니다.**
> 없애자는 뜻은 아니다 — 답이 적어 문턱이 물러서는 책에서 도는 안전장치이고 공짜다. 다만
> **«두 관문이 늘 서로 다른 일을 한다»고 읽으면 안 된다.**
>
> 덤으로 `sim ↔ top_sim` 상관이 **+0.996**이다 — 사실상 한 값이다. 모델이 대개 1위 후보를
> 고르기 때문이다. `top_sim`을 함께 기록하는 것은 «없음»으로 답한 항목을 볼 때만 뜻이 있다.
>
> 해상도: n=25라 **4%보다 드문 쌍은 이 상관이 못 본다.**

**4. 후보는 코드가 만들고 모델은 고르기만 하되, «없음»을 반드시 선택지에 둔다.**
없는 것을 고르게 하면 지어내기를 강요하는 셈이다. 실제로 잘못 고른 4건은 전부 «첫 글자가 같은
행»이었다. 선택지 밖의 답을 말하면 코드가 «없음»으로 되돌린다. 목차 대조를 choice로 바꾸니
운양집_01에서 **9 → 14항목**이 되었고 그 14개는 제목이 고른 행에서 실제로 시작했다(불일치 0).
본문에 없는 75항목에는 «없음»을 골랐다(문턱 0.8에서 100%). 83회 호출 · $0.0022.

**대조군으로 확인했다(2026-09-22, 84회 · $0.0022).** 제목 A를 묻되 후보는 다른 항목 B의 것을
주면 정답은 거의 언제나 «없음»이다. 달성 가능한 최대치(빌린 창에 A의 진짜 행이 든 쌍을 뺀 값)가
98.8%인데 **관측이 98.8% — 천장에 붙었다**(천장 쌍을 빼면 83/83 = 100%). 실제 조건의 «없음»
비율 69.9%와 **+30.1%p** 갈리므로, 모델이 유사도 1위를 기계적으로 집는 것이 아니다. 자기검증과
확률을 **둘 다** 통과한 헛것은 0건이다. 유일하게 «없음»이 아닌 답은 바로 그 천장 쌍이었고
(「擊磬集」의 진짜 행이 빌린 창에 우연히 들어 있었다) 모델이 그것을 맞게 골랐다.
전문은 `docs/sessions/session_jev_structure.md`의 「대조군」 절.

**5. 목차가 답하는 층과 본문 판정이 답하는 층을 나눈다.**
총목이 답하는 것은 권·集(층위 1~2)이고 본문 판정이 답하는 것은 그 안의 낱글이다. 같은 층에
두면 트리가 «集과 작품이 형제»인 꼴이 된다. `nest_under_toc`이 각 본문 후보 앞에 선 가장 가까운
목차 후보의 층위 + 1을 준다. **목차 후보가 하나도 없으면 아무것도 바꾸지 않는다**(일기류에서는
본문 판정이 곧 낱글의 층이다).

**6. 사람에게는 문턱이 아니라 «상위 몇 개»를 묻는다.**
0.7이라는 숫자는 사람에게 뜻이 없다. 손잡이는 개수로 묻고 코드가 확률 내림차순으로 자른다.
움직이면 목록이 그 자리에서 늘고 준다(D-121의 «미리 보기»와 같은 태도). 그러려면 확률이
브라우저에 있어야 하므로 응답에 행마다의 확률을 실어 보낸다 — **다시 부르지 않는다.**

**7. 모델의 확률은 `confidence` 축이지 `weight` 축이 아니다.**
D-128 11항에서 `Relation`의 두 축이 갈라졌다 — `weight`는 «관계가 얼마나 굵은가», `confidence`는
«그것이 참인가»다. 판정 모델의 확률은 뒤엣것이다. 화면의 확신도 칸(규칙의 고정 0.6·가산점)과도
섞지 않는다. 척도가 다른 둘을 한 칸에 넣으면 사람이 비교할 수 없다 — 모델 확률은 배지와
정렬에만 쓴다. 임계도 같은 이유로 다른 질문이다: `suggest_strong_threshold()`(무게 분포의
90분위)가 답하는 것은 «두꺼운 꼬리가 어디부터인가»이고, 여기의 문턱이 답하는 것은
«어느 확신도부터 믿을 만한가»다.

**8. 두 길이 얼마나 어긋나는지 한 줄로 보이되, 그 값으로 아무것도 자동으로 바꾸지 않는다.**
밀도비·합의율 둘을 화면이 계산한다(서버에 새 상태를 두지 않는다 — 규칙 후보 수·모델 후보 수·
겹친 수가 이미 브라우저에 있다). 이 줄은 **판정이 아니라 모름의 표시**다: 어느 쪽이 틀렸는지
그 값은 말해 주지 않는다. 경계값(2배·80%)은 **임시다** — 책 둘에서 나온 값이라 아직 가설이고,
정답이 없어도 재는 값이라 서고의 아무 문헌에서나 모을 수 있으므로 모아서 정한다.

**9. 라우트를 늘리지 않고 저장하지 않는다.**
기존 `POST …/segmentation/structure/llm`(D-125)이 `engine: "llm" | "jev"`(기본 `"llm"`)를 받는다.
라우트가 늘면 CLAUDE.md·AGENTS.md의 숫자와 기계 검사(D-079)까지 함께 고쳐야 하는데 얻는 것이
없다. 확률도 경계도 저장하지 않는다 — 「적용」이 만드는 것은 지금과 같은 경계 목록뿐이다(D-122).
보내기 전에 «몇 행·몇 질문·몇 번·얼마»를 세어 보이고 사람이 누른다(전역 규칙 11).

### 버린 것 (되풀이 올라오지 않게 적는다)

| 버린 길 | 왜 |
|---|---|
| **규칙 후보 ∩ 모델**(교집합) | 1항 — 천진담초에서만 좋고 문집에서 재현이 0.18로 무너진다 |
| **Jev를 `LlmRouter`에 넣기** | 라우터의 계약은 «프롬프트 → 글»이고 Jev에는 글이 없다. 화면 「모델」 드롭다운도 생성 모델을 고르는 자리다 |
| **`llm_pipeline`의 클라이언트를 가져다 쓰기** | 이 앱은 `CTB-Setup.exe`로 배포되는 독립 앱이라 남의 저장소를 의존성으로 가질 수 없다 — 아래 참조 |
| **책 종류를 분류해 설정을 고르기**(일기류·문집…) | 고전 문헌의 경우의 수는 열거되지 않는다. 분류가 틀리면 설정이 통째로 틀린다 |
| **어긋남 값으로 무언가를 자동으로 켜고 끄기** | 8항 — 그 값은 누가 틀렸는지 말해 주지 않는다 |
| **Jev에게 층위·제목을 만들게 하기** | 만들지 않는 모델을 쓰는 이유가 사라진다. 제목은 그 행의 글자, 깊이는 사람 몫 |
| **Jev를 판독 계획(D-126)에 쓰기** | 벤더 문서가 **텍스트 전용·CJK 정확도 낮음**을 밝힌다. 비전 자리에는 못 쓴다 |
| 새 단추·새 저장 자리·새 라우트 | D-122의 한 흐름을 깬다 |

### 클라이언트가 두 벌이라는 것 (2026-09-22 확인)

`src/llm/jev.py`는 `llm_pipeline`의 `llm_runtime/jev_decisions.py`와 **같은 계약을 따로 구현한
것**이다(다른 세션이 알려 왔다). 지울 수 없다 — 이 앱은 `CTB-Setup.exe`로 배포되는 독립 앱이라
남의 저장소를 의존성으로 가질 수 없고, `importlib`로 읽어 오는 길도 설치본에는 없다. 벤더링이
불가피한 중복이므로 **한쪽만 고치면 안 된다**는 것을 여기 적어 둔다.

지금 갈리는 자리는 하나다: **429·529 백오프가 이쪽에만 있다**(1초·×2·3회). 다만 그 값은
**실측이 아니라 안전빵**이다 — 이쪽 누적 306회, 저쪽 111회 모두 간격 없이 쏘고 **429를 한 번도
보지 못했다.** 처음 429가 나면 그때의 누적 호출 수·경과 시간을 남겨야 값을 고칠 근거가 생긴다.
방어 다섯(`cost`가 bool일 때·토큰이 문자열일 때·키 파일의 줄 끝 주석·`KEY=" "`·`uncosted_calls`)은
**양쪽이 독립으로 같은 자리에 닿았다**.

### 아직 안 한 것 · 열린 질문

- **실서고 화면 검증(①→②→③→적용)을 못 했다.** 확정본(L4)이 있는 문헌이 지금 서고에 없다.
  화면 경로는 아래 표의 js 시험 다섯이 함수 단위로 지키고 있으나, 그것은 **브라우저에서 한
  흐름으로 도는 것**을 본 것이 아니다. 문헌이 생기면 그때 본다.
- 층위를 «목차가 1~2, 본문 판정이 3»으로 나누는 것은 운양집 1책에서만 쟀다(9 → 14).
  총목이 集을 나열하지 않는 책, 총목이 두 층인 책에서는 다시 재야 한다.
...
- 모델을 한 번도 부르지 않은 문헌에서는 ①의 어긋남 줄이 아예 없다. 그 상태를 «모름»으로 보일지
  아무 말도 하지 않을지.
- **자기검증은 한 방향으로만 선다.** 후보를 고르는 자(`toc_candidates`)와 검증하는 자
  (`SELF_CHECK_MIN`)가 같은 `title_similarity`이고, 측정기의 항목 분류(`PRESENT`)도 같은 값
  0.85를 쓴다. 그래서 「잘못 고른 것」은 잡지만 **정답 행이 상위 6개 밖에 있으면 영영 보이지
  않는다.** 위 대조군도 **지어내기**만 재고 **놓침**은 재지 못한다.
  **놓침은 서지로 쟀다**(2026-09-22, 호출 0건): `db/unyangjip_works.csv`의 «권» 열은 ITKC
  메타데이터라 본문 글자와 무관하다. 운양집_01(卷1·2)에서 **서지가 «이 책»이라는데 «없음»이라
  한 것은 0건**이고, «없음» 58건 중 23건은 서지가 «다른 권»이라고 확인해 준다. 즉 실제 조건의
  «없음» 69.9%는 «수확량 30%»가 아니라 **이 책에 없는 것을 없다고 한 것**이다
  (`eval_boundary_judge.py biblio`). **다만 그 주장이 서는 범위는 83건 중 65건(78.3%)**이고,
  나머지 18건은 서지가 아무 말도 하지 않는다 — 놓침이 숨을 수 있는 곳이 거기다. n≈84라 **최소
  검출 단위가 1.2%p**이므로 「0건」은 「0%」가 아니라 **「1.2% 미만」**이다. 양성 팔도 심은 것이
  없다(우연히 생긴 1건 + 서지가 확인해 준 16건뿐) — 「**창에 있는데도** 못 찾는가」는 창이
  겹치는 쌍을 설계로 20~30개 심어야 재진다. 그때 **총목에 두 번 적힌 이름 아홉은 뺀다**:
  우연히 생긴 그 1건(「擊磬集」)이 하필 중복 항목이라 본문 대응 자리가 여럿이었고, 그런 항목은
  아무 창을 빌려도 걸릴 확률이 높아 대표성이 없다.
- **자동 대조를 한 겹 더 쌓지 않는다.** 서지가 말하지 않는 18건을 편집 거리로 메우고 싶어지는데,
  이 자료가 반례다 — 「昇平館集」과 「續昇平館集」은 거리가 작고 **서로 다른 작품**이다(3항의 접두
  한계와 같은 자리). 이 코퍼스에서 한 글자가 의미를 나르므로(續·附·又·并) 편집 거리는 판독
  오류와 별개 작품을 구별하지 못한다. 사각지대를 줄이는 대신 그만큼의 오탐이 생긴다.
- **덮개를 넓히면 새로 덮인 부분의 품질이 낮다.** 서지 대조를 그룹 한 열에서 그룹+기사명 두 열로
  넓히자 사각지대가 줄면서 **그 새 층에서 헛것이 나왔다**(이름 2,077개라 「三首」가 우연히 걸려
  卷6 표제가 «이 책»으로 분류됐다). 그래서 `biblio`가 판정마다 **어느 열이 덮었는지**를 함께
  찍는다 — 느슨한 층에서만 잡힌 것은 약한 증거다.
- **문턱이 서지와 같은 답을 냈다(2026-09-22).** 서지가 «다른 권»이라는데 모델이 고른 3건
  (0.70·0.72·0.73)이 **전부 도출 문턱 0.94 밖**이라 후보로 서지 않았다. 유사도만 보는 문턱과
  본문 글자를 안 보는 서지가 **독립인데 같은 답**을 냈다 — D-129의 문턱이 가진 유일한 외부
  검증이다. 셈 단위 주의: 위 4항의 「14건」은 **항목** 수이고, 총목이 같은 集을 두 권에 적어
  자리로 접히므로 ③에 서는 후보는 **8자리**다(0.8과 0.94가 항목·자리 모두 같다).
- `starts`의 «모르는 자리» 제외는 무작위가 아니다(앵커 CSV가 나쁜 자리에 몰려 있고 그 자리가
  어려운 사례와 상관될 수 있다). 제외 건수·비율을 표에 늘 함께 찍고, 제외 없이 채점한 값을
  민감도 띠로 병기하는 것이 남은 일이다(커넥톰 세션 권고, 2026-09-22).

### 주장과 그것을 지키는 시험 — **전부 깨뜨려 빨간불을 보고 적었다**

산문으로 적은 결정은 코드가 바뀌어도 문장만 남는다. 그래서 항마다 시험 이름을 붙이되,
**이름만 붙이지 않고 그 주장을 실제로 깨뜨려 빨간불을 한 번 보고 나서 적었다**
(규율의 근거는 `docs/maintenance.md`의 「주장을 지킨다는 시험은 깨뜨려 빨간불을 본 뒤에 적는다」).
2026-09-22에 18가지로 깨뜨렸고 **18가지 모두 빨간불**이었다.

**이 표를 만들다가 구멍이 하나 나왔다.** 1항 — 이 설계의 핵심 결정 — 을 지키는 시험이 **없었다.**
합치는 자리 `_mergeLlmProposals`에 시험이 한 건도 없어서, 교집합으로 바꿔도 아무것도 깨지지
않았다. 초안의 검증 계획(§7-1)이 «합집합 시험을 더한다»고 적어 둔 것이 그대로 남아 있던 것이다.
시험을 먼저 쓰고(`test_screen_keeps_rule_places_when_the_model_answers`) 세 가지로 깨뜨려
빨간불을 본 뒤에 이 표를 적었다. **계획에 적힌 것과 코드에 있는 것을 견주는 일을 사람이 하지
않으면 아무도 하지 않는다.**

| 항 | 주장 | 시험 (`tests/test_structure_jev.py`) |
|---|---|---|
| 1 | 규칙 후보를 거르지 않는다(합집합) · 억제한 자리를 되살리지 않는다 | `test_screen_keeps_rule_places_when_the_model_answers` |
| 2 | 문턱을 이 책의 절벽에서 뽑는다 | `test_threshold_is_derived_from_this_book_not_a_constant` · `test_threshold_finds_a_low_cliff_where_the_constant_keeps_nothing` |
| 2 | 답이 다섯도 안 되면 0.8로 물러선다 | `test_threshold_falls_back_when_there_is_too_little_to_go_on` |
| 2 | 전부 통과하면 아무것도 자르지 않는다 | `test_threshold_keeps_everything_when_nothing_fails_the_self_check` |
| 3 | 자기 검증이 확률과 **다른 관문**으로 선다 | `test_self_check_gate_holds_even_when_the_threshold_falls_back` · `test_nothing_is_kept_when_even_the_best_answer_fails_its_own_check` |
| 4 | «없음»이 선택지에 늘 있다 | `test_toc_matching_refuses_when_nothing_fits` |
| 4 | 선택지 밖의 답은 «없음»으로 되돌린다 | `test_toc_matching_ignores_an_answer_that_is_not_one_of_the_choices` |
| 5 | 본문 후보가 목차 경계 아래 층으로 내려간다 | `test_body_proposals_nest_under_the_nearest_toc_boundary` |
| 5 | 목차가 없으면 층위를 건드리지 않는다 | `test_without_a_toc_the_levels_stay_as_they_are` |
| 6 | «상위 몇 개»는 확률 내림차순으로 자른다 | `test_screen_ranks_model_candidates_by_probability` |
| 7 | 판정 모델·목차 대조의 자리를 «모델이 가리킨 자리»로 센다(배지·정렬) | `test_screen_treats_judge_places_as_model_places` |
| 8 | 많이 다를 때만 «많이 다릅니다»라고 한다 | `test_screen_tells_when_the_two_paths_disagree` |
| 9 | `engine="jev"`가 **같은 라우트**로 들어오고 답을 합칠 자리에 둔다 | `test_screen_asks_the_judge_engine_and_keeps_the_answer` |
| 9 | 보내기 전에 «몇 번»을 세고, 상한을 넘으면 한 건도 쏘지 않는다 | `test_size_counts_calls_before_sending` · `test_gate_refuses_before_the_network` |
| — | 묶음 하나가 실패해도 나머지 답을 잃지 않는다 | `test_one_failed_call_does_not_lose_the_others` |
...
증거가 못 되는 드문 자리다. 전체 시험 1,578건도 초록이었다 — 시험은 자기
...
**조용히 쌓인 빚 — 같은 판에서 갚았다.** v1.4.0 태그 트리는 `ruff` 0건인데
HEAD 는 **64건**이었다(직접 세었다: `git archive`로 두 트리를 풀어 각각 검사).
그 사이 86커밋 동안 **ruff를 부르는 자동 관문이 없었다** — 릴리스 체크리스트
2단계에만 손으로 적혀 있었고, 전체 시험 1,578건 중 어느 것도 ruff가 아니었다.
「검사를 몇 번이나 돌렸는데 왜 이제 나오느냐」의 답이 이것이다.

처음에는 기준선을 박아 «늘어나는 것만» 막으려 했다(`tests/test_lint_does_not_grow.py`).
같은 날 64건을 다 고쳐 기준선은 비었고, **빈 기준선은 「어느 파일이든 0」을 뜻한다.**
고치는 방법은 한 가지가 아니었다 — 코드 줄은 `ruff format`, 주석·docstring 은 접기,
코드 안의 긴 문자열은 암시적 이어붙이기(값은 그대로), **시험이 비교하는 문자열은
손대지 않고 그 문자열 하나만 면제**. 마지막 것이 요점이다: 줄 길이를 맞추려고
검사 대상을 바꾸면 본말이 뒤집힌다.

접는 데도 함정이 둘 있었다. ruff 는 글자 수가 아니라 **표시 폭**으로 잰다(한자·한글은
2칸) — `len()`으로 세면 66자짜리 줄을 「이미 짧다」로 보고 한 줄도 접지 못한다.
그리고 **여러 줄 문자열 안의 줄에는 `# noqa` 를 붙일 수 없다** — 그 자리에서는 주석이
아니라 문자열의 내용이 되어, 검사는 그대로 걸리고 문자열만 더러워진다. 다만 거기서
「파일 전체를 끈다」로 가면 과하다: ruff 는 그 진단을 **닫는 따옴표 줄로 옮겨** 보므로
닫는 줄에 붙이면 그 문자열 하나만 면제된다(교차검토 지적, 실측으로 확인).

### D-130 덧붙임 — 전 과정을 걸어 보고 찾은 둘 (2026-09-22)

「아무것도 모르는 사람이 처음부터 끝까지 쓸 수 있는가」를 실제로 걸었다(격리 서고에
표본 PDF 를 넣고 등록 → 레이아웃 → OCR → 확정본 → 편성 → 해석 → 내보내기). **그 전까지
이 경로 전체를 한 번에 걸어 본 적이 없었다.**

**① 끌어다 놓기가 영문 이름 PDF 를 거절했다.** `create-from-files` 는 이미지 묶음
PDF 를 `<doc_id>.pdf` 로 저장하려고 그 이름을 예약하는데, **이미지가 없어도** 예약했다.
`doc_id` 를 비워 보내면 서버가 첫 파일 이름에서 만들므로 `report.pdf` → doc_id=`report`
→ 예약 `report.pdf` → 올린 PDF 도 `report.pdf` → 자기 자신과 충돌.

왜 안 드러났나: 기존 시험이 전부 **한글 이름**(`舊注蒙求.pdf`)을 썼다. 한글이면 저장명이
`doc_YYYYMMDD_pdf1.pdf` 로 바뀌어 충돌하지 않는다. **시험 표본이 한쪽으로 치우쳐 있으면
그 반대쪽은 없는 것과 같다.**

**② 한글 제목 해석을 JSON 으로 내보내면 500 이었다.** HTTP 헤더는 latin-1 로 인코딩된다
(starlette `init_headers`). `Content-Disposition` 파일명에 제목을 그대로 넣어
`UnicodeEncodeError` 가 났다. 이 저장소의 문헌은 제목이 거의 다 한글이라 **사실상 늘
실패하던 기능**인데 시험이 한 건도 없었다. RFC 5987(`filename*=UTF-8''…`)로 고쳤다.

**부수:** 「서고가 설정되지 않았습니다」를 500 → **409**(133곳). 서고가 없는 것은 처음 켠
사람의 정상 상태다(PDF 를 떨어뜨리면 `quick-start` 가 자동으로 만든다). 500 은 서버
잘못이라는 뜻이라, 처음 화면이 늘 빨간 콘솔을 내면 **진짜 오류 하나가 묻힌다.**

**걸어 보며 나온 「실패」 열 건 중 여덟은 내 시험 스크립트의 잘못이었다** — 없는 라우트,
빠뜨린 필수 칸, 틀린 id 형식. 제품의 오류 메시지가 그때마다 정확히 짚어 주었다
(`'id' is a required property`·`'b1' does not match '^p\d+_b\d+$'`). 검증 도구가 낸
실패를 제품 결함으로 보고하지 않으려면 **라우트와 모델을 코드에서 읽고** 걸어야 한다.
