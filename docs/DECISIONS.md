...
3. **층위**: 1 = 卷·편(Work 아래 큰 단위), 2 = 기사(글), 3 이상 = 기사 안 조각(문답·협주·연).
   층위 n 경계를 넣고 지워도 층위 < n 의 id는 변하지 않는다. 내용 트리는 층위대로 접힌다.
...
- D-093 이름 마이그레이션(위 2).
- 층위 1(卷) 경계를 D-089 목차 대조에서 자동으로 만드는 것(자동 트리가 목차 상위 항목을 container로 놓는 것까지는 됨).
- 해제에서 LLM이 표제 어휘를 뽑아 고급 규칙을 채우는 것.
- 경계 넣기 폼은 쪽·행·글자를 손으로 적는다. 교정 텍스트에서 글자를 눌러 넣는 편집기가 다음 형태다.
