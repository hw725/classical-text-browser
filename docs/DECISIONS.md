...
> **ROI 수치는 보류한다.** 초판의 최빈 ROI 비중(40.7% / 78.9% / 86.8%)은 `fetch_adjacencies`의
> primary ROI 기준 60씨앗 값이다. 전수로 다시 재려고 `roiInfo` JSON을 파싱했더니 26~42%로
> 떨어졌는데, **이는 방법 차이일 가능성이 크다** — neuPrint ROI는 계층 구조라 부모·자식이
> 중복 계상되면 최빈 비중이 기계적으로 낮아진다. 두 값은 비교 대상이 아니다.
> **6번·9번은 이 수치에 기대고 있으므로, primary ROI 기준 전수 측정으로 확정하기 전까지
> 잠정으로 둔다.**
...
6. **구획은 실제로 벽에 가깝다 — 3번을 살린다.** 하행뉴런만 봤을 때(최빈 ROI 40.7%)는 «구획은
   사전확률일 뿐»으로 읽혔으나, 그것은 구획을 건너가는 것이 임무인 계통이라 그렇다. 연합·감각
   계통은 78.9%·86.8%로 **출력 무게의 대부분이 자기 구획에 머문다.** 따라서 범위는 순위 가중치가
   아니라 **주소의 일부**로 두는 쪽이 유비에 맞는다.
...
9. **넓게 모아서 좁게 내보낸다.** 연합 구조에서 최빈 ROI 비중이 입력 59.8% · 출력 78.9%로 갈린다 —
   입력은 구획을 넘어 들어오고 출력은 자기 구획에 머문다. Concept 승격이 하는 일과 같은 형태다:
   출처(Layer 1)는 어디서든 오고, 합성된 개념은 한 영역에 속한다. **6번(구획은 주소의 일부)은
   출력 쪽 규칙이고, 입력 수집에는 적용하지 않는다** — 수집을 구획으로 막으면 연합이 일어나지 않는다.
