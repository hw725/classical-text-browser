...
- [x] 경계 색인 — 경계가 정본, TextBlock은 파생. 행 단위 먼저, 글자 단위는 다음 → D-090
...
- 천진담초 실제 확정본으로 제안 정확도 측정: DB의 36회차 대비 재현·정밀도. 그 결과로
  가중치(날짜 0.5·어휘 0.3·짧은 행 0.2·내려쓰기 0.25·역행 −0.35)를 조정한다.
...
- 운양집 중간본·운양속집으로 판별·대조 정확도 측정. 첫 실측 결과로 문턱(0.7·0.6)을 조정한다.
- 대조된 卷 경계로 Work를 자동 생성해 트리를 2단으로 만드는 것(지금은 kind 표시까지).

## D-090: 경계 색인 — 경계가 정본이고 TextBlock은 파생이다

**날짜**: 2026-09-02
**상태**: 확정 (1단계 행 단위 구현. 글자 단위는 2단계)

### 배경

사용자: 「인덱싱은 쪽 번호에 그치면 안 된다. 정확도 높은 위치에서 TextBlock의 경계를
찾아야 하고, 그 자리를 색인한다는 개념이다.」 D-088·089의 경계 제안은 승인하면 TextBlock의
`source_refs`(쪽·char_range)에 위치가 남지만, 경계 자체는 독립 데이터가 아니었다. 그래서
확정본(L4)을 고치면 경계가 어디로 갔는지 다시 잇는 근거가 없고, 경계만 따로 내보내거나
연구자의 다른 DB(운양 텍스트 DB의 `article_index.csv`: 시작쪽·시작행·끝쪽·끝행)와 맞출 수
없었다.

### 결정

1. **경계 색인 항목(Boundary)을 코어 엔티티로** (`schemas/core/boundary.schema.json`,
   `core_entities/boundaries/`). 시작·끝 앵커 = {쪽, 행, char(선택)}. kind(toc·date·어휘·manual·
   front·volume), level, status(proposed·approved·manual·deprecated), 신뢰도·근거, L2 행 bbox
   캐시, `l4_commit`(어느 확정본 기준인지), `text_block_id`.
2. **경계가 정본, TextBlock은 파생.** 「적용」은 경계를 먼저 만들고 그 구간의 본문을 이어
   TextBlock을 만든다(`metadata.boundary_id`로 서로 가리킨다). 경계를 옮기면 파생 TextBlock의
   본문·출처가 다시 이어진다.
3. **행 단위 먼저.** 시작·끝은 행이고 `char`는 선택 필드로 예약만 한다. 사용자 결정: 「이미
   복잡하니 전자로 구현한 다음 후자로 튜닝」. 글자 단위가 필요한 판식은 확인됐다 — 澹齋日錄류
   일기는 「○七日…」처럼 ○ 표지와 날짜가 열 중간에 오고 개행이 없다. 2단계의 첫 시험 자료.
4. **경계 옮기기는 이웃과 함께.** 이 경계의 시작을 한 행 뒤로 밀면 앞 경계의 끝이 그만큼
   늘어난다(구간이 겹치거나 비지 않는다). 내용 트리의 ▲▼ 단추가 이것이다.
5. **앵커를 이미지 위에 표시.** 내용 트리에서 블록을 누르면 시작 행의 L2 bbox를 점선으로
   그린다. L2·L3의 좌표계가 다르므로 image_width 비로 환산한다(D-087). L4 행 수와 L2 행 수가
   어긋나면 bbox를 만들지 않는다 — 틀린 자리를 보여 주는 것보다 안 보여 주는 게 낫다.
6. **CSV 내보내기**(`GET …/boundaries/export.csv`): 기사id·문헌·권·순서·유형·층위·제목·시작쪽·
   시작행·끝쪽·끝행·상태·신뢰도·근거. 연구자 DB의 article_index와 열 이름을 맞춰 서로 검증
   자료가 되게 한다. 가져오기는 다음.

### 남은 것 (2단계)

- 글자 단위 앵커: 행 안 「○」+날짜 같은 표지를 찾아 `char`를 채우고, 적용·재잇기가 행 중간에서
  자르게. 이미지 표시는 행 bbox 위의 비율 위치로.
- L4가 바뀐 뒤 앵커 재대조(`l4_commit`이 다르면 경계 행 텍스트로 다시 찾기).
- CSV 가져오기(article_index → 경계 색인), 목차 스킬 패키지(D-089 후속).
