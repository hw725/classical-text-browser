...
| 측정 | DNge\* (운동출력) | **KCg-m (버섯체 연합)** | Tm9 (시각전달) |
|---|---:|---:|---:|
| 출력 파트너 수 중앙값 | 541 | 283 | 44 |
| 무게 1~2인 엣지 비율 | 58.1% | **82.0%** | 55.6% |
| 상위 3 파트너 무게비중 | 10.9% | 17.8% | 32.5% |
| 상위 10% 엣지 무게비중 | 61.5% | 45.7% | 42.3% |
| 최빈 ROI 무게비중 | 40.7% | **78.9%** | 86.8% |
...
7. **파트너 수·집중도는 일반 원칙이 아니다.** 출력 파트너 수가 44~541로 12배, 상위 3 집중도가
   10.9~32.5%로 3배 차이난다. 「커넥톰은 이렇다」고 말할 수 있는 값이 아니므로 설계 근거로 쓰지 않는다.

### 수렴(입력) 측정 — Concept 승격 규칙의 근거

같은 씨앗으로 들어오는 엣지도 쟀다. 발산과 다른 이야기를 한다.

| 측정 | KCg-m 입력 → 출력 | DNge\* 입력 → 출력 | Tm9 입력 → 출력 |
|---|---:|---:|---:|
| 파트너 수 중앙값 | 276 → 283 | 610 → 541 | 34 → 44 |
| 무게 1~2 비율 | 83.5% → 82.0% | 53.7% → 58.1% | 69.1% → 55.6% |
| 상위 3 무게비중 | 19.3% → 17.8% | 10.1% → 10.9% | 46.0% → 32.5% |
| 상위 10% 엣지 무게비중 | 48.3% → 45.7% | 65.7% → 61.5% | 49.2% → 42.3% |
| 최빈 ROI 무게비중 | **59.8% → 78.9%** | 65.9% → 40.7% | 84.8% → 86.8% |
...
10. **연합 구조는 깔때기가 아니다.** KCg-m은 입출력이 거의 대칭이다(276↔283, 83.5%↔82.0%,
    19.3%↔17.8%). 「여러 개가 하나로 줄어든다」는 그림은 하행뉴런(610→541, 상위 10%가 입력
    무게의 65.7%)의 것이지 연합 구조의 것이 아니다. Concept 층을 «요약해서 줄이는 단계»로
    설계하면 유비가 어긋난다 — 같은 폭을 유지하되 **가중치가 재배치되는** 단계로 본다.
...
**측정의 한계**: 계통당 씨앗 60개, 양방향 엣지. `consensusNt` 는 시냅스별 실측이 아니라 뉴런 단위
예측·합의값이다. 글루탐산의 극성은 학계에서도 확정적이지 않으므로 위 표에서 따로 두었고 억제에
합산하지 않았다. 강연결 절단(무게 5)은 임의값이다.
