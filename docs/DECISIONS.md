...
**결정 1 — 「가벼운 설치」를 정식 경로로 안내한다.**

```
uv sync --no-install-package paddlepaddle --no-install-package paddleocr
```

약 **320MB**. 읽기(인식)는 LLM Vision이 하고 PaddleOCR는 **검출(위치)** 만
맡으므로, 없어도 텍스트는 그대로 들어가고 복사·Ctrl+F·참고문헌 추출이 된다.
잃는 것은 **형광의 위치 하나**다(실측: 502줄 중 433줄 제자리 → 0줄).

**paddle을 다시 extra로 내리지는 않는다.** 기본값이 「형광이 엉뚱한 데 뜨는
상태」가 되면 안 된다는 것이 D-055의 판단이고 그대로 유효하다. 기본은 안전한
쪽에 두되, **덜어내는 길을 명시적으로 알려 준다.**
