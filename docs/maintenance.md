...
| **OCR 엔진의 실제 모델** | 엔진 **변환 코드**는 전부 시험이 돌린다(古典籍 둘은 2026-09-21 `tests/test_ocr_kotenocr_contract.py`로 메웠다 — 그 전엔 `is_available()`뿐이었다). 그러나 **모델 가중치가 내는 인식 품질**은 어느 시험도 보지 않는다. 배치·파이프라인은 `DummyOcrEngine`을 쓴다(뼈대만) | 엔진을 올린 뒤 **실제 이미지로 1쪽** + `scripts/eval_cer.py`. 사각지대가 어디인지는 짐작하지 말고 **다시 잰다** — 아래 「무엇이 실제로 실행되는지 재는 법」 |
| **다권본** | 시험 문헌이 대부분 단권이다 | 2권짜리로 **같은 쪽 번호의 결과가 다른지** 확인 |
| **테스트 픽스처가 만드는 manifest** | 테스트는 `page_count`를 채워 둔 서고만 쓴다. `add_document`로 만든 실제 문헌은 그 칸이 없어 훑어보기 라우트가 400을 냈는데 테스트는 전부 초록이었다(2026-09-18, D-126 덧붙임) | 쪽 수는 `_resolve_page_count`로만 읽고, **실제로 등록한 문헌**으로 headless E2E 한 번 |
...
### 주장을 지킨다는 시험은 깨뜨려 빨간불을 본 뒤에 적는다 (2026-09-21)

「이 주장은 이 시험이 지킨다」고 문서에 적는 것은 주장을 **추적 가능하게** 만들 뿐
**참으로** 만들지 않는다. 같은 날 세 사례가 나왔다.

- `core-schema-v1.3.md`가 「L4가 없으면 닫힌다」 옆에 시험 이름을 적어 두었는데, 그 시험은
  주장이 거짓이 되는 배치(한 단위에 Tag 셋)에서 아무 말도 하지 않았다.
- 이 문서의 「검출은 순수 함수만 검증한다」는 그 기능을 만든 커밋과 **같은 날** 쓰였고 처음부터
  반쪽만 맞았다. 두 달 동안 드리프트 검사는 계속 초록이었다.
- 새로 만든 `tests/test_ocr_kotenocr_contract.py`의 「읽기 순서」 단언도 처음에는 공허했다 —
  정렬을 통째로 빼도 초록이었다.

**초록은 «이 배치에서 이렇다»는 뜻이지 주장을 지킨다는 증거가 아니다. 빨간불만이 증거다.**
한 번만 하면 되고 비용이 거의 없다: 주장을 코드에서 실제로 깨뜨려 그 시험이 빨개지는지 보고,
무엇을 깨뜨렸을 때 빨개졌는지 시험 독스트링에 적는다. 위 세 번째 사례는 그렇게 해서
«정렬 한 줄»이 아니라 «XY-Cut을 거친다»가 진짜 지켜지는 것임을 알았다(정렬을 빼면 초록,
XY-Cut을 빼면 빨강).

### 무엇이 실제로 실행되는지 재는 법 (2026-09-21)

위 표의 「OCR 엔진 API」 줄은 오래 **틀린 채로** 있었다 — 「검출은 순수 함수만 검증한다」고
적혀 있었지만 실제로는 `detect_lines`·`_get_detector`가 실행되고, 정작 아무것도 안 도는 것은
古典籍 둘이었다. 문서가 코드를 앞질러 늙은 것이고, **사각지대를 기억으로 말하면 이렇게 된다.**

`coverage`는 이 저장소에 없다(의존성을 늘리지 않았다). 대신 파이썬 3.12의 `sys.monitoring`으로
**시험이 도는 동안 함수 진입만** 기록하면 된다 — 스크래치패드에 작은 pytest 플러그인을 쓰고
`-p <모듈이름>`으로 불러 쓴다(저장소에 파일을 남기지 않는다).

- `sys.monitoring.use_tool_id(4, ...)` → `register_callback(4, events.PY_START, cb)` → `set_events`
- 콜백에서 `code.co_filename`이 관심 파일이 아니면 `return sys.monitoring.DISABLE`
  (한 번 본 코드도 DISABLE — 그래야 느려지지 않는다)
- `pytest_unconfigure`에서 모은 `code.co_qualname`을 파일별로 찍는다

엔진 시험 묶음(153건)에 3분이면 돈다. **사각지대를 주장하기 전에 이걸로 다시 잰다.**
...
- 새 엔드포인트는 해당 도메인의 라우터 파일에(현재 9개, 226 라우트).
