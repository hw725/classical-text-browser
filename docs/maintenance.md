...
| **OCR 엔진 API — 古典籍 Lite·Full 둘** | 이 둘만 `is_available()` 말고는 **인식 코드가 한 줄도 실행되지 않는다**(2026-09-21 실측). 나머지 엔진은 모델이 가짜여도 자기 변환 코드가 실행되므로 API 변경이 잡힌다. 배치·파이프라인은 `DummyOcrEngine`을 쓴다(뼈대만) | 엔진을 올린 뒤 **실제 이미지로 1쪽**. 사각지대가 어디인지는 짐작하지 말고 **다시 잰다** — 아래 「무엇이 실제로 실행되는지 재는 법」 |
