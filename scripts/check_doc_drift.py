...
# 성공 출력이 무엇을 보증하는지 **출력 자신이** 말한다 (2026-09-21).
#
# 왜: 이 검사기가 「문서와 코드가 일치한다 ✓」를 찍는 동안 같은 CLAUDE.md 안에
# 거짓 동작 주장이 있었다 — 「L4가 없으면 자동 승격이 닫힌다」(실제로는 Tag 수로
# 떨어져 열렸다)와 「검출은 순수 함수만 검증한다」(사각지대는 古典籍 Lite·Full
# 둘뿐이었다). 둘 다 셀 수 있는 수치가 아니라 **동작 주장**이라 이 검사기의 범위
# 밖인데, 초록이 문서 전체를 보증하는 것처럼 읽혔다.
#
# 검사 로직은 그대로다. 고치는 것은 **초록이 무엇을 뜻하는가**뿐이다 —
# 범위를 말하지 않는 초록은 범위를 넘어 읽힌다.
SCOPE_NOTE = """셀 수 있는 5종(라우트·라우터·JS 모듈·스키마·테스트 파일)이 일치한다. ✓
  동작 주장은 검사 대상이 아니다 — 그것은 시험 슈트가 본다."""
