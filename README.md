...
| 영역 | 기능 |
|------|------|
| **원본 관리** | PDF/이미지 뷰어, 레이아웃 분석, OCR(NDL古典籍OCR Full/Lite + NDLOCR + LLM 비전 + PaddleOCR), HWP/HWPX 가져오기 (준비중), PDF 참조 텍스트 추출 |
...
| **추출 모드** | 작업 모드(교감/추출) 전환, 텍스트 레이어 진단, 권 단위 일괄 OCR(중단·재개·부분 재실행), **텍스트 레이어를 텍스트 레이어 PDF 내보내기** |
