...
왼쪽에 서고 탐색기, 가운데에 원본 PDF 뷰어, 오른쪽에 텍스트 작업 패널이 배치된 **VSCode 스타일 3단 레이아웃**입니다. 상단 탭 10개(열람 → 레이아웃 → 교정 → 편성 → 표점 → 현토 → 번역 → 주석 → 인용 → 이체자)로 작업 단계를 전환합니다.
...
탭 줄 오른쪽 끝의 「추출 모드로 →」 버튼으로 «교감 모드»와 «추출 모드»를 오가면
교감 전용 탭 7개(편성·표점·현토·번역·주석·인용·이체자)가 숨습니다
...
> **v1.2.0 이전에 만든 텍스트 레이어 PDF는 다시 만들어야 합니다.**
> 텍스트가 쪽 구석에 2pt 크기로 박혀 있어 드래그도 검색도 되지 않습니다(D-068).
> 추출 패널의 「텍스트 레이어 PDF 다시 만들기」를 한 번 눌러 주세요.
...
| [**maintenance.md**](docs/maintenance.md) | 개발자 | **고치기 전에 볼 것** — 되돌릴 수 없는 것, 되풀이하지 말 것, 테스트 사각지대 |
| [DECISIONS.md](docs/DECISIONS.md) | 개발자 | 설계 결정 근거 (D-001~D-069) |
...
| [releases/v1.1.5.md](docs/releases/v1.1.5.md) | 전체 | v1.1.5 릴리스 노트 |
